...
things. `tools/ci/gallery-baseline.py` held the one that BUILDS — it strips the
three non-campaign directories, so what `delvec` compiles is the campaign and
nothing else. `tools/ci/check-gallery-coverage.py` held one that only ever had to
...
byte, the tree the baseline compiles and `tools/ci/gallery-build.py` writes to disk.
