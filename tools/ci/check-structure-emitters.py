...
3. **Every package in `prefabs/` is built, and every generator in it is run
   ...
   generator. `fmt` is derived (`tools/ci/fmt-workspaces.sh` takes its population
...
NOT_EMITTERS = {
    ...
    "docs/experiments/m2-jigsaw-seed-stability/generator/src/main.rs": (
        ...
        "recorded method (same rule as `tools/ci/check-live-commands.py`)."
    ),
}
    ...
    for rel in emitters:
        ...
        if any(m in s for m in CONNECTION_RULE_MARKERS for s in scope):
            ...
            findings.append(
                ...
                f"NOT_CONNECTION_EMITTERS in tools/ci/check-structure-emitters.py with the reason."
            )
    ...
    for rel in tracked(".rs"):
        ...
        if any(m in s for m in BLOCK_RULE_MARKERS for s in scope):
            ...
            findings.append(
                ...
                f"tools/ci/check-structure-emitters.py with the reason."
            )
...
def sweep_excluded_prefix() -> str | None:
    """The path prefix `tools/ci/fmt-workspaces.sh` excludes from its own population.
    ...
    than an assumption.
    """
    script = ROOT / "tools" / "ci" / "fmt-workspaces.sh"
...
def check_generators_are_wired() -> tuple[list[str], int]:
    ...
    a different mechanism:
      ...
      * `cargo fmt`                 — enumerated nowhere.
                                      `tools/ci/fmt-workspaces.sh` derives its
    ...
    established = {
        ...
        "the job)": bool(loops),
        "`cargo fmt` (an invocation of tools/ci/fmt-workspaces.sh --check)": sweep_invoked,
    }
    ...
    if prefix is None:
        findings.append(
            "tools/ci/fmt-workspaces.sh's EXCLUDED_PREFIX could not be read, so this check cannot "
    ...
    for m in sorted(swallowed):
        findings.append(
            f"tools/ci/fmt-workspaces.sh's exclusion prefix '{prefix}' swallows prefabs/{m}. An "
