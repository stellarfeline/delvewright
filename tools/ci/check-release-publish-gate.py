...
WHAT COUNTS AS A PUBLISHING ACT, and why each is one
  ...
  cargo-publish       `cargo publish`, or `tools/ci/crates-io-publish.sh` invoked
...
TWO POPULATIONS, because one of them is the way around the other
  ...
  B. every tracked shell script, plus every composite-action YAML under
     ...
     legitimately lives inside `tools/ci/crates-io-publish.sh`, and it is that
...
Usage:
  python3 tools/ci/check-release-publish-gate.py [--workflows DIR] [--repo DIR]
