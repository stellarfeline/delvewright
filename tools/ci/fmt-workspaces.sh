...
# `tools/ci/check-shell-pipe-shortcircuit.py` and `tools/ci/check-shell-redirect-dirs.py`
...
# note stands in `tools/ci/build-release-binaries.sh` and `validation/packtest-all.sh`
...
if [ "${#FAILED[@]}" != 0 ]; then
  if [ "$MODE" = check ]; then
    echo
    echo "Run 'bash tools/ci/fmt-workspaces.sh' to fix all of these at once."
