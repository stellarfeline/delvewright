...
#    that found anything wrong leaves nothing behind. `tools/ci/crates-io-publish.sh`
...
# `tools/ci/crates-io-publish.sh` reads it back to decide what crates.io is
...
echo
echo "== 4. sha256 written beside each tarball, for tools/ci/crates-io-publish.sh =="
...
# right after check 1 — but `tools/ci/check-shell-redirect-dirs.py` reads redirects
...
SUCCESS=1
echo "check-publishable: OK — \`cargo install $CRATE\` has everything it needs; tarballs kept at $PKG (sha256 beside each) for tools/ci/crates-io-publish.sh"
