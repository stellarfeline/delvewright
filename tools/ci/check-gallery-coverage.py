...
def materialise(base: Path, overlay: Path, dest: Path) -> None:
    ...
    `tools/ci/gallery-baseline.py` builds and what `tools/ci/gallery-build.py` writes
...
def main() -> int:
    ...
    if not prefabs.is_dir():
        die(
            ...
            "`python3 tools/ci/gallery-prefabs.py --out <dir>` first: it runs the generator "
...
def write_index(
    ...
    lines = [
        ...
        "",
        "Generated by `tools/ci/check-gallery-coverage.py --index`. Every row is a",
