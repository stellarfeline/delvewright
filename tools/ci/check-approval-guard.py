...
`tools/ci/assert-run-approved.sh` is the repository-side answer: it reads the run's
...
2. the FIRST `run:` step invokes `tools/ci/assert-run-approved.sh`;
...
Usage:
  python3 tools/ci/check-approval-guard.py [--workflows DIR]
...
GUARD = "tools/ci/assert-run-approved.sh"

# `bash tools/ci/assert-run-approved.sh crates-io`, `sh tools/…`, `./tools/…` or the
