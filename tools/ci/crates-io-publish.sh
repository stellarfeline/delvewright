...
# and `tools/ci/check-publishable.sh` must have packaged it first — the release path
...
# CURRENT sha256 does not match the sha256 `tools/ci/check-publishable.sh` wrote
...
#
# Usage:
#   tools/ci/crates-io-publish.sh --plan      # what WOULD happen; touches nothing
#   tools/ci/crates-io-publish.sh --publish   # do it (needs $CARGO_REGISTRY_TOKEN)
#   tools/ci/crates-io-publish.sh --plan --only delvewright-dsl
#   tools/ci/crates-io-publish.sh --publish --only delvewright-dsl [--allow-dirty]
#
...
# Where tools/ci/check-publishable.sh packages, and where `--only` packages: its
...
# The sha256 `tools/ci/check-publishable.sh` wrote beside the tarball, right after
...
local_cksum() { # <crate-name> <version>
  ...
  [ -f "$f" ] || { echo "crates-io-publish: no packaged tarball at $f — run tools/ci/check-publishable.sh first" >&2; exit 2; }
  ...
  # of it EXCEPT `tools/ci/check-publishable.sh`, so THAT boundary is real: the
  ...
  if [ -z "$ONLY" ]; then
    ...
    [ -f "$hash_file" ] || {
      echo "crates-io-publish: no verified sha256 at $hash_file — run tools/ci/check-publishable.sh first" >&2
    ...
    if [ "$actual" != "$expected" ]; then
      echo "crates-io-publish: $f now hashes $actual, but tools/ci/check-publishable.sh verified $expected." >&2
...
if [ -n "$ONLY" ]; then
  ...
  # already packaged by `tools/ci/check-publishable.sh`; a single selected crate has
  ...
  # Same lifetime rule as tools/ci/check-publishable.sh's own packaging: keep the
