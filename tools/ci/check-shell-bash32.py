...
`tools/ci/fmt-workspaces.sh`, `tools/ci/build-release-binaries.sh`,
`tools/ci/check-publishable.sh`, `tools/lib/rcon.sh`, `validation/packtest-all.sh`,
...
cargo workspaces and, in the content repository, excluded 27 tracked files:
  ...
  * every tracked `*.yml`/`*.yaml` under `.github/`, plus any tracked
    ...
    `tools/ci/check-python-shell-newlines.py`, which covers workflow `run:` for the
...
`tools/planner/planner-state.sh`, which is in the population. Should a hook ever grow an
...
# tools/ci/check-shell-pipe-shortcircuit.py, tools/ci/check-shell-redirect-dirs.py and
# tools/ci/fmt-workspaces.sh — and, like fmt-workspaces, it is COUNTED every run: an
