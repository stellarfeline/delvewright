...
gate related the two. `tools/ci/check-dw-codes.py` keeps the DIAGNOSTICS catalog
...
first: the row restated an accepted set that had moved on. `tools/ci/check-dw-codes.py`
...
(`tools/lib/publishable.py`, shared with `tools/ci/check-crates-io-readmes.py`). A
...
root:
    python3 tools/ci/check-reference-versions.py
...
def fail_shape(what: str, path: pathlib.Path, knob: str) -> int:
    ...
    sys.stderr.write(
        ...
        f"       tools/ci/check-reference-versions.py. Do NOT loosen the check.\n"
    )
    return 2
...
def write_claims(root: pathlib.Path, real_delvec: str, real_dsl: str, real_mc: str) -> int:
    ...
    bump is `python3 tools/ci/check-reference-versions.py --write`, and the same
