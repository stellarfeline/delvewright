...
#      (tools/planner/worktree-reclaim.py), which runs here rather than being remembered
...
# does not merely report; it RUNS `tools/planner/worktree-reclaim.py --apply`, which is
...
section "worktree reclamation — RUN here, not remembered (tools/planner/worktree-reclaim.py)"
if [ -x "$ROOT/tools/planner/worktree-reclaim.py" ]; then
  python3 "$ROOT/tools/planner/worktree-reclaim.py" --apply 2>&1 | sed 's/^/  /'
else
  cat <<EOF
  REFUSED — $ROOT/tools/planner/worktree-reclaim.py is missing or not executable.
