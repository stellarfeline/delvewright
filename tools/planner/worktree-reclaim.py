...
avoid: it is run at the merge, and it is run again from `tools/planner/planner-state.sh`,
...
tree `tools/planner/worktree-new.sh` creates is claimed at dispatch, and the only
...
def main(argv: list[str] | None = None) -> int:
    ...
    # its build output from (`tools/planner/worktree-new.sh`). Deleting it does not free
    ...
    donor = "the main checkout, which is the donor tools/planner/worktree-new.sh clones from"
    ...
    if args.after_merge or args.tree:
        ...
        #
        # `tools/planner/worktree-new.sh` takes a lease on every tree it creates and
