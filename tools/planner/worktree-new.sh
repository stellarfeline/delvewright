...
e.g.:
  ...
  tools/planner/worktree-new.sh --path "\$DELVEWRIGHT_WORKTREES_DIR/<name>" ...
...
LEASED=0
if [ "$DO_LEASE" = 1 ]; then
  if [ ! -x "$HERE/tools/planner/worktree-reclaim.py" ]; then
    echo "   lease      : NOT TAKEN — $HERE/tools/planner/worktree-reclaim.py is missing or not executable."
    echo "                An unclaimed tree is one the sweep has to judge on its own."
  elif python3 "$HERE/tools/planner/worktree-reclaim.py" --lease "$NEW" --holder "$HOLDER" \
    ...
    if [ -n "$BRANCH" ]; then
      ...
      echo "                tools/planner/worktree-reclaim.py --after-merge $BRANCH --apply"
    else
      echo "   lease      : held by $HOLDER — this tree is DETACHED, so no pull request"
      echo "                can end its lease. End it deliberately:"
      echo "                tools/planner/worktree-reclaim.py --release $NEW"
    fi
  else
...
echo
echo "Format with 'bash tools/ci/fmt-workspaces.sh' — it states how many workspaces it"
