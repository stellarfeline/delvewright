...
datafixer that renamed `PatrolTarget` to `patrol_target`.)
    ...
    tools/maintenance/check-patrol-types.py                # check; non-zero on disagreement
    tools/maintenance/check-patrol-types.py --print        # also print the full type table
...
`tools/maintenance/dump-block-light.py` this is a maintenance tool: CI reads the committed tag
...
JAVA_SRC = ROOT / "tools" / "maintenance" / "patroltypes" / "PatrolTypeDump.java"
