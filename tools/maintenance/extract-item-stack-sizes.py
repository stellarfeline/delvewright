...
canonical-form sweep (`tools/ci/check-json-canonical.py`). Every 1.21.11 item declares
the component explicitly, so nothing is defaulted or inferred here — a missing
one is an error, not a silent 64.

    python3 tools/maintenance/extract-item-stack-sizes.py item_components.min.json \
