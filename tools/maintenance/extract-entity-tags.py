...
stdlib). Same shape and provenance as `tools/maintenance/extract-sound-registry.py`.
...
sweep (`tools/ci/check-json-canonical.py`). Values are already namespaced in
...
the `#minecraft:<tag>` a datapack would write. 46 tags for 1.21.11.
    ...
    python3 tools/maintenance/extract-entity-tags.py entity-type-tags.min.json \\
