...
Usage:
    tools/creator/refimg.py --prompt "a sea-gate barbican at dusk, ..." --out .refimg/z1
    tools/creator/refimg.py --prompt-file zone2.txt --style-code A1B2C3D4 --seed 42
    tools/creator/refimg.py --prompt "..." --dry-run     # show the request, call nothing

    # a multi-view reference: view 1 from the prompt alone, every later view
    # anchored on VIEW 1 and framed for what it shows
    tools/creator/refimg.py --prompt-file v1-front.txt --aspect-ratio 16:9 --out .refimg/v1
    tools/creator/refimg.py --prompt-file v3-plan.txt  --aspect-ratio 1:1 \
...
class MissingConfig(Exception):
    ...
    what `tools/creator/refscore.py` — which states the same convention in the same
