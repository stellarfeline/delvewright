...
So palette selection is a **query**, not a recollection:
    ...
    tools/creator/block-appearance.py --id minecraft:packed_mud          # what colour IS it
    tools/creator/block-appearance.py --near '#6b6b6b' -n 12             # what is this colour
    tools/creator/block-appearance.py --list --full-cube-only            # the whole shelf

and it ends in a screen, a measurement of the mix, and a LOOK:

    # "a pale, cool ashlar for a nave wall": 1146 blocks down to 14
    tools/creator/block-appearance.py --screen --where full_cube --where 'L>=0.75' \\
        --where 'L<=0.95' --where 'C_mean<0.02' --where 'texture_range<=0.30'

    # what a weighted paint actually reads as — four numbers, never a mean
    tools/creator/block-appearance.py --mix 'sandstone=3,smooth_sandstone=3,andesite=4'
    tools/creator/block-appearance.py --mix 'deepslate[axis=y]=3,stone=1'   # states, properties and all

    # a whole program's palette, and a count of how much of it was read
    tools/creator/block-appearance.py --program zone.json
...
(`tools/maintenance/extract-block-classification.py`).
...
def load_classification() -> dict:
    ...
    if not CLASSIFICATION.exists():
        raise SystemExit(
            ...
            f"Regenerate it: python3 tools/maintenance/extract-block-classification.py "
