#!/usr/bin/env python3
"""Nothing in this repo may write a prefab `.nbt` without stating every block-state property.

The rule itself lives elsewhere (`prefabs/invariants.rs::complete_state` for the
generator workspaces, `BlockRegistry::complete` for the compiler workspace).
...
non-production code — did not, and shipped `hello-room.nbt` with 2 palette
entries over 10 cells that only a running 1.21.11 server can read. A seventh
would have arrived the same way, because nothing anywhere asked what the set
was.
...
1. **Every site that turns a value into NBT bytes is accounted for.** The
   ...
   A candidate passes by naming the completion rule — that is what an emitter
   does. A candidate that is not an emitter must be listed in [`NOT_EMITTERS`]
   below with a reason, which is PRINTED on every run. A NEW file that
   serialises NBT and does neither is red, and the message says which of the two
   it needs.
...
# How a file shows it applied the completion rule. Both spellings of the same
# rule: the source-included one the generator workspaces share, and the
# registry method the compiler workspace uses.
COMPLETION_MARKERS = (
    "invariants::complete_state(",
    "registry.complete(",
)

# Sites that serialise NBT and are NOT prefab emitters. Each is named
# individually, with the reason it is not obliged to complete a palette — and
...
NOT_EMITTERS = {
    "crates/admit/src/structure.rs": (
        ...
        "template. Completing here would rewrite a PRE-PIN piece under a registry that "
        "describes 1.21.11 alone and has no authority over it — the same scoping DW0734 "
        "already states."
    ),
...
def check_emitters() -> tuple[list[str], int]:
    """Every NBT-serialising file completes its palette, or says why it need not."""
    findings: list[str] = []
    candidates = 0
    emitters: list[str] = []
    honoured: list[str] = []
    ...
    for rel in tracked(".rs"):
        ...
        if any(m in s for m in COMPLETION_MARKERS for s in scope):
            emitters.append(rel)
        elif rel in NOT_EMITTERS:
            honoured.append(rel)
        else:
            findings.append(
                f"{rel} serialises NBT ({INGREDIENT}) and never completes a block state.\n"
                f"    If it writes a prefab `.nbt`, route its palette through the completion "
                f"rule — `invariants::complete_state` in a `prefabs/*` generator, "
                f"`BlockRegistry::complete` inside the workspace — and assert the "
                f"post-condition, or a state it writes means whatever a 1.21.11 server "
                f"decides and no other reader of the file can resolve it.\n"
    ...
    print(f"  emitters (complete their palette): {len(emitters)}")
    ...
    for f in stale:
        findings.append(
            f"{f} is listed in NOT_EMITTERS but no longer serialises NBT (or now completes). "
...
def emitter_scope(rel: str, text: str) -> list[str]:
    ...
    neighbour's completion is not this file's, and widening the scope that far
...
def main() -> int:
    findings: list[str] = []
    print("== every site that serialises NBT completes its block states ==")
    f, _ = check_emitters()
    findings += f
    print()
