...
2. **A version ledger** (§7.1/§7.4). Every optional field is listed in
   `docs/reference/grammar.md` §2d with the document version that introduced it,
...
def ledger() -> dict[tuple[str, str], tuple[str, str]]:
    """The §2d table of `docs/reference/grammar.md`.
    ...
    start = text.find("## 2d.")
    if start < 0:
        sys.exit("FAIL: docs/reference/grammar.md has no `## 2d.` section to hold the ledger")
    end = text.find("\n## ", start + 1)
...
def main() -> int:
    ...
    for key in fields:
        if key not in rows:
            failures.append(
                f"{key[0]}.{key[1]} is #[serde(default)] with no row in grammar.md §2d. "
    ...
    for key in rows:
        if key not in fields:
            failures.append(
                f"grammar.md §2d has a row for {key[0]}.{key[1]}, which is not a "
