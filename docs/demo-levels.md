...
| Document-level composition — one program file composes another (`include`, grammar.md §5c) | generation-time, so its demo is a corpus program rather than a level: a map document that composes two zone documents into one derivation, expanded at the region its splits allocate. The point it makes is that the map writes what neither zone can know — the boxes they get, the whole-map datum bound into one of them, and the mass between them — and that the composition is byte-for-byte the Rust composition it replaces. Its second half is the bound, which is the part a reader will otherwise assume away: the same piece composed under a host that draws first re-textures, so a composed piece's look is certified by the composed render and never by its own | pending |
