...
Everything knowable from the inputs alone — the id, the region against the
48-block cap — is refused up front, with nothing written. A verdict line is
printed only once the prefab exists, so a `pass` never sits above a failure.
