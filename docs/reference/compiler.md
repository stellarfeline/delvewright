...
| `DW0344` | **A piece that stands in the sea answers for where it meets it.** Two arms, one code. *The placement arm* (`compiler::plan::check_ocean_waterline`, build tier, exit 3): in a `horizon: ocean` world a placed piece whose metadata declares `waterline_y` must land that waterline at sea level (`piece.y + waterline_y = 62`) — otherwise the piece floats above the sea (its shore an unclimbable cliff, its authored water pocket hanging in the air) or is drowned under it. Since the area's origin is DERIVED from the piece set's own `walk_y` (spec-0060 §3.2), the two declarations are claims about one building, and the message names the walk plane that would seat this waterline on the sea. *The walk-plane arm* (`compiler::plan::check_ocean_walk_plane`, build tier, called from `emit` where the walk region and the placed boxes are both in hand): **is any walk cell of a placed piece at or below the sea plane?** A walk cell is where a body's feet go, and one under the sea is a party wading its critical path. **What this arm no longer asks, and why.** It used to ask whether the placement BOX reached the sea plane, and refuse a piece with no declaration when it did. The box's minimum y IS the area origin, so under one global ocean datum that was true of every piece in every ocean world whatever its bytes held, and the refusal's own second remedy — *raise the piece clear of the sea* — could not be performed by any amount of authoring. That is an over-refusal by construction. The quantifier moved onto the cells the rule was always about, and the RAISE move is now observable: the count in the refusal is what the author's own edit moves (`crates/delvec/tests/remedy_reachability.rs`). The static half of the same question, which needs nothing placed, is `DW0886`; this arm is the backstop for what the documents could not know — a piece the campaign edits after placement above all. *The document arm* (`compiler::seating::walk_plane_over_waterline`, validation tier, exit 1, and reported by `delvec prefab seating`): the placement arm's own question, asked of the two declarations. The origin is `walk_ref - walk_y`, so a declared waterline lands on the sea plane exactly when `waterline_y == walk_y - 1` — a shore stands its walk plane one course above the water it can be climbed out of. Both numbers are in the document, so nothing has to be placed to know it. It carries **this code and not `DW0886`** deliberately: it is the same fact one stage earlier, and giving the earlier statement a different number would leave a creator with two codes for one answer. Measured on the shipped library before its generators were repaired: `island-beach-camp` (`walk_y: 2`, `waterline_y: 2`) and `cave-shore` (`walk_y: 1`, `waterline_y: 1`), both refused at build for a fact two declarations already held. **Binding**: `waterline binding: horizon base …` states pieces placed and pieces held to sea level on every build, ocean or not; `sea walk-plane binding: horizon base …` states walk cells judged out of walk cells examined, and how many stand at or below the sea. |
