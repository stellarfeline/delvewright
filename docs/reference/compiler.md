...
Methodology; CI enforces the DW-code subset — see `tools/check-dw-codes.py`).
...
- The compiler is the module `delvec::compiler` of the one package
  ...
  [`tools.md`](tools.md).
- Versions (as of this doc): `delvec 1.4.0`, `dsl 0.23.0`, `mc 1.21.11`.
  `dsl 0.23.0` is the **one** `dsl_version` this engine accepts (ADR-0024): every
...
| `time` (**required**) | `day`/`noon`/`dusk`/`night`/`midnight`/`dawn` (`sunrise` is accepted as a synonym of `dawn`). **No default, and no `Default` impl on the enum** (spec-0061 §4): "this delve is played at noon" is a design decision, and `#[default] Noon` is what let a delve whose whole approved look was night build, light-check and render under a blue noon sky. An omitted `time` is `DW0100` like any other missing required field. It is also the world half of `DW0890`'s comparison against the approved design, which needs two sides to have one. Dimension-global initial state, emitted in the sealing baseline. Vanilla's `/time set` takes either a keyword or a raw tick count, so the DSL is not limited to the four keywords: the four vanilla states emit their **keyword verbatim** (`time set night`) and the other two emit the equivalent **tick form** (`dusk` -> `time set 12000`, `dawn` -> `time set 23000`). One table maps every state to its argument and its `time query daytime` read-back (`WorldTime::spec`), so the sealed-state PackTest asserts the right value and no shipped campaign's bytes move. `dusk` is the **sunset onset** (12000, the sky visibly going orange), deliberately not 13000 — 13000 is the sun already down, which is exactly what the `night` keyword sets, so it would make `dusk` a synonym rather than its own beat; `dawn` (23000) is the sunrise onset. `dusk`/`dawn` count as night for the sky-light model (`DW0210`), which is the conservative direction (both skies are in fact brighter than midnight). Same enum for the `set-time` effect. | 0.5 / dusk+dawn 0.5 |
| `weather` (**required**) | `clear`/`rain`/`thunder`. Dimension-global, emitted after sealing (`weather <kw>`). Rain/thunder attenuate the assembled-light sky term. **Required, with no default**, for the reason `time` gives. The emission rule is unchanged — the declared state is emitted — and what moved is that there is no longer a campaign that declares none, so a world document that used to omit it now declares `clear` and emits `weather clear`. | 0.5 / required 0.22 |
...
**`flask`** (bool, DSL v0.8, spec-0016 §1): this kit entry is the class's **recovery item**, and resting at
...
**`contents`** (obj, DSL v0.8, spec-0016 §1): **what is in the bottle** — vanilla's
...
| Effect `move-npc{npc,to_anchor,speed?,on_arrive[]?}` | `to_anchor` resolves through `plan::body_station` (`BodyScope::Beat`: the area the firing quest bundle plays in, then the npc's home, then an unambiguous crossing) — a name two areas answer to and neither settles is `DW0859`. A*-planned per-tick tp through walkable space; unroutable → `DW0307`. `on_arrive[]` (v0.6) fires once on the driver's final-waypoint tick — **exact parity with `move-actor.on_arrive`**: same arrival detection, same execution context (`mv_arrive_<key>` mirrors `ma_arrive_<key>`), and every deep effect walker (flag/wave producer scans, consumer-ref checks, checkpoint/stealth collector, l10n inventory + localization, nav flattening, emission) recurses into it via the shared `nested_effect_lists` authority. Lets content gate a beat on walk *completion* (`on_arrive` → `set-flag`) instead of fire-and-forgetting the walk. | 0.4 / `on_arrive` 0.6 |
...
as a zero and which `tools/staging-gate.py` refuses — a build the owner walks
...
no `dsl_version`; `tools/check-dw-codes.py` resolves both forms.
...
beat behaved differently from its second would be a trap, and a `seq_…` is itself
...
| — | `weather <kw>` (declared `world.weather`, **required since 0.22**; the declared state is emitted, whatever it is — a campaign that declares `clear` emits `weather clear`, and there is no longer a campaign that declares none) |
...
- **`OCEAN_BASE_Y` and `ISLAND_WATERLINE_Y` are retired as world constants.**
  The old datum placed every ocean area at y=60 = `SEA_LEVEL − 2`, which is one
  tileset's authoring convention promoted to a world constant by being the only
  convention that existed when the ocean was built. It is correct for exactly
  those pieces whose walk plane is at local y=3, and it is why every other piece
  in every other library landed with its floor under the sea and met a refusal
  whose second remedy could not be performed by any amount of authoring. A
  tileset convention is a fact about a tileset, so it now lives in that
  tileset's pieces. `SEA_LEVEL` stays: the sea plane is a property of the
  horizon. spec-0013's "areas sit at y=64+" is retired rather than
  reinterpreted — under it an island's walk plane would stand four blocks above
  its own shore.
- **`void` and `valley` keep their ORIGIN datum**, `BASE_Y` = 64
  (`horizon::walk_ref_y` returns `None` for both). `void` declares nothing
  outside the placed geometry, so there is no outside relationship to hold a
  walk plane against; `valley`'s gap floor is pinned one block under `BASE_Y` so
  that a piece placed at that origin stands beside it. Every campaign on either
  base is byte-identical across this change.
...
- **`delvec prefab seating --horizon <base>`** answers the pairing over a whole
  ...
  `tools/check-seating-agrees.py` is where that sentence is a check: it
  enumerates every pool of a library against every base the schema declares,
  compares the command's verdict with the engine's, and perturbs toward both
  disagreement shapes.
...
- `<out>/validation/lethal-gate.json`: the lethal-volume proofs' **binding
  ...
  count and its `shown_by` as declared. **A volume that catches nothing prints
  its zero beside the population**, so a pit whose keep-out lies under the rim
  reads as *checked and clear* rather than as unbound. The build prints the same
...
- `<out>/validation/piece-mating.json`: the `DW0780`/`DW0781` **binding ledger**
  ...
  reads a library that declares nothing as an honest zero, and that is the
  sentence this check printed for as long as it read only `spatial_contract`.
...
- **`lighting` stamp** (POV + interior shots, `crate::render_plan::area_lighting_stamp`):
  ...
  emitters render), so the stamp tells `delvec render` exactly which shots need its
...
rules the moment the graph exists. Measured on the gallery's site-plan point with
`layout-graph.json` removed: 15 refusals to 2.
...
| `DW0102` | The document's `dsl_version` is not the one this engine accepts, `0.23.0`; the message names it (ADR-0024). Raised per stage document by `dsl::validate::envelope`, and for an l10n sidecar under `DW0180`. |
...
| `DW0340` | Trap declaration structurally invalid (v0.6, spec-0011): a malformed/duplicate `trap/<id>`, an `at`/`disarm.via` that no area's prefab provides, or a `disarm.via` that collides with the trap's own trigger anchor. The `at` remedy names what a trap actually needs of a piece — **one point anchor, under any name** — and says that a spec-0022 command `payload` needs nothing beyond that cell, because the compiler emits the detection; the `dispenser` socket belongs to a legacy `dispense` effect and the `trigger_block` to a flag-gated trap (`DW0363`). Sending an author to carve trap hardware for a payload trap sends them to build something no build reads. Renumbered off the spec's stale reserved number (0197). |
...
| Code | Meaning |
|------|---------|
| `DW0326` | A `play-sound.sound` (v0.6) or `narrate.sound` (v0.4) id is not a known 1.21.11 sound event (validated against the vendored `sound_event` registry, `crates/delvec/data/sounds-1.21.11.json`; `minecraft:` prefix optional). |
...
| `DW0344` | **A piece that stands in the sea answers for where it meets it.** Two arms, one code. *The document arm* (`compiler::seating::walk_plane_over_waterline`, **validation tier, exit 1**, and reported by `delvec prefab seating`): in a `horizon: ocean` world a piece whose metadata declares BOTH `walk_y` and `waterline_y` must stand its walk plane one course above its own waterline. The area's origin is DERIVED from the walk plane (spec-0060 §3.2), so `pos.y + waterline_y == 62` is exactly `waterline_y == walk_y - 1`, and both numbers are in the document: nothing has to be placed to know it, and the message names the walk plane that would seat this waterline on the sea AND the waterline the bytes would have to hold, because either number could be the typed one. A piece that declares no `walk_y` at all meets `DW0886` before this. Measured on the shipped library at the pinned revision, where the three pieces that declare both stand one course clear: `cave-shore` (`walk_y: 2`, `waterline_y: 1`), `island-beach-camp` and `island-galley` (both `walk_y: 3`, `waterline_y: 2`). **`compiler::plan::check_ocean_waterline` keeps the binding count and no longer refuses**, and that is a loosening of one function paired with a tightening of the rule: the document arm's coverage is a superset for every piece seated at its area origin, and the one case it does not cover — a stair child, mated through a socket at a different local y — was an over-refusal by construction, of the same shape §1.2 diagnosed in the other arm, since a piece four courses up a stair cannot stand its water at sea level and nothing is wrong with it unless a body walks there. *The walk-plane arm* (`compiler::plan::check_ocean_walk_plane`, build tier, called from `emit` where the walk region and the placed boxes are both in hand): **is any walk cell of a placed piece at or below the sea plane?** A walk cell is where a body's feet go, and one under the sea is a party wading its critical path. **What this arm no longer asks, and why.** It used to ask whether the placement BOX reached the sea plane, and refuse a piece with no declaration when it did. The box's minimum y IS the area origin, so under one global ocean datum that was true of every piece in every ocean world whatever its bytes held, and the refusal's own second remedy — *raise the piece clear of the sea* — could not be performed by any amount of authoring. That is an over-refusal by construction. The quantifier moved onto the cells the rule was always about, and the RAISE move is now observable: the count in the refusal is what the author's own edit moves (`crates/delvec/tests/remedy_reachability.rs`). The static half of the same question, which needs nothing placed, is `DW0886`; this arm is the backstop for what the documents could not know — a piece the campaign edits after placement above all. It carries **`DW0344` and not `DW0886`** deliberately: it is one fact, and giving its earlier statement a different number would leave a creator with two codes for one answer. **Binding**: `waterline binding: horizon base …` states pieces placed and pieces held to sea level on every build, ocean or not; `sea walk-plane binding: horizon base …` states walk cells judged out of walk cells examined, and how many stand at or below the sea. |
| `DW0345` | The assembled world resolves **no entry anchor** — no area places a piece whose prefab metadata declares an anchor with `"role": "entry"`; an anchor's NAME is never consulted, so no spelling supplies one. See §4 "Entry point". The compiler then has no cell to call the campaign's start: no `setworldspawn`, no class-apply teleport, no first-join placement, no `dw:cp` seed. Build-tier (exit 3), `compiler::emit`, and raised **before any model is built** — a world with no start does not have a walking problem, and letting it through meant the crossing fell to the critical-path walk and was reported as `DW0311` over a leg nothing was meant to walk, which sends a reader looking for a wedged doorway. Silent before either — the delve compiled clean and fell back to the vanilla spawn search, which a **dedicated** server resolves to the surface (so every rung of the validation ladder stayed green) and the **integrated singleplayer** server resolves to the build floor, i.e. inside solid stone. Prescription: give the piece the party arrives in an anchor at that cell and put `"role": "entry"` on it (in a `prefab_pool`, that is the prefab the layout is seeded from), or bind the area to a prefab that already has one. Every producer can write it — `delvec prefab anchor <nbt> --name <anchor> --pos <x,y,z> --role entry` for a hand-built or ingested piece, `"role": "entry"` on the `mark` for a grammar program. |
| `DW0872` | **A crossing into an area with nowhere to arrive.** A leg of the party's forced route changes area, and the destination declares no entry point — no anchor in its prefab carries `"role": "entry"`, or the one that does resolves to a region rather than to a cell. Areas sit `AREA_SPACING` (256) blocks apart across void, so a leg that changes area is never a walk: the party has to be put down inside the destination, and nothing says where. Build tier (exit 3), `compiler::plan`, raised while the critical path is built — so it precedes the walk proof, which is the whole point. Silent before: the crossing was simply not emitted, the leg fell through to `DW0311`, and the author was told *the player cannot walk … a wedged doorway seam, a void gap, an unbroken fence ring* — or, on a campaign with a gate anywhere, to reopen a `close-gate` it does not have. Every word of that is true about a walk and this leg was never one. `DW0345` is the same rule over the **whole world** (no area at all declares one) and `DW0872` over **the one area a body must be put down in**; different quantifiers, different remedies, two codes. Prescription, in spec-0046's vocabulary: give the piece the party arrives in an anchor at that cell and put `"role": "entry"` on it (in a `prefab_pool`, that is the prefab the layout is seeded from), or bind the area to a prefab that already has one. |
...
### DW0889 — an anchor outside what the area guarantees (`compiler::guarantee`; advisory)

| Code | Meaning |
|------|---------|
| `DW0889` | **This campaign names an anchor, and the area that would provide it does not guarantee it.** An area binding a `prefab_pool` seats a **subset** of that pool's members, and the subset is exactly three things: the `entry`-role member, placed at the area origin with rotation `none` before any growth; one carrier for each anchor the campaign makes the solver guarantee (`plan::required_anchors_for_area` then `solver::required_carriers`); and `filler_count` `connector`-role pieces drawn **with replacement** by weight from the area's PRNG stream. Membership in the pool buys a piece nothing: a `room` or `terminal` member nothing requires is never seated, and an anchor it declares does not exist in the built world. **The guaranteed set is therefore the entry member's anchors plus everything the forced carriers declare**, and this code names every anchor of the pool's vocabulary that this campaign's documents carry and that set does not contain. `compiler::guarantee::check`, bound in the one validation funnel every subcommand goes through (`main::validate_loaded`), **validation tier, exit 0**. **Why it is answerable here**: the whole verdict is a fact about declarations — the pool's members, their roles, their `anchors` maps — so no `.nbt` is opened, no piece is placed and no PRNG is drawn; on `compiler::seating`'s precedent it is reported at the step the documents already decided it, rather than at the build, which is five authoring steps after the anchors are chosen. **Severity: advisory (warning, exit 0), deliberately.** The guaranteed set is a *sufficient* condition and not a necessary one: an anchor carried only by `connector` members is seated whenever the filler draw picks that connector, and shipping content rests on exactly that, so an error here would refuse campaigns that build green — the over-refusal shape. The sharp end is unchanged and still the build's: `DW0302` when the pool has no member that could supply a required anchor, `DW0303` when the `pieces` budget cannot fit the forced carriers, `DW0360` when an effect's anchor resolves to no position in the assembled world, `DW0142`/`DW0431`/`DW0447` at their own sites. What was missing was never a refusal — it was the answer arriving in time. **Its quantifier**, which is part of what it says: names this campaign carries that *some* member declares and *no* guaranteed member declares. It says nothing about a name **no** member declares (`DW0302`'s, at the use site) and nothing about a name **two placed pieces** answer to (`DW0498`'s and `DW0305`'s, and unknowable without the settled draw). **How the names are collected**: by serialising the stage documents and taking every string in them, not by walking the verbs that may carry an anchor. A typed walk here would be the twelfth hand-rolled enumeration of *where an anchor can be named* — the shape `CLAUDE.md` names, right the day it is written and silently narrower than the DSL every day after — and it would be wrong first for the verbs `required_anchors_for_area` already does not reach: a `loot` fill, a trap payload, a shop, a lethal volume, a `shortcut`, and an environment trigger in a campaign with more than one area. Serialising cannot forget a field. It over-reports rather than under-reports (a name is collected wherever it appears, so on a multi-area campaign one area's name is offered to every pool whose vocabulary holds it), the surplus is bounded by intersecting with the pool's own vocabulary, and the message says *this campaign names it*, never *this area resolves it*. **Prescription, and every branch of it is reachable from the campaign**: bind a name the area guarantees; or require this one where the solver must seat its carrier (an objective, an NPC stand, a wave spawn, a lane waypoint, or an anchor-bearing effect in this area) — which is the same mechanism, used deliberately; or bind the area to that piece with `prefab` instead of `prefab_pool`, which guarantees everything it declares. Rebuilding the library is **not** among them. **Measured on the shipped content library** (`delvec prefab anchors --json`, this revision): `pool/cave-shore`, `pool/stone-keep` and `pool/vertical-keep` each guarantee 2 anchor names — `spawn` and `anchor/exit`, both on the entry piece — of the 10 their members declare between them; `pool/island` guarantees 11 of 53; 17 of 83 across 42 members in 4 pools. **Binding**: `anchor-guarantee binding:` states pool areas of areas, members examined, the vocabulary they declare, how many of it is guaranteed, how many names the campaign carries and how many were reported — on every `validate`/`analyze`/`build`, including the run that reports nothing. |

**`DW0498` and this code are a pair, and they say different things.** `DW0498`
is about **ambiguity** and needs the settled draw: the pool seated one
anchor-bearing piece twice, so a name has two carriers. `DW0889` is about
**absence** and needs no draw at all: the layout is not required to seat the
piece that carries the name. One area can legitimately raise both about one
anchor — the gallery's `area/annex` does — and neither restates the other. The
earlier code is not retired and does not move: nothing `DW0498` asserts about a
settled draw is knowable at validation, and nothing `DW0889` asserts about a
guarantee is improved by waiting for one.
...
`DW0467`, which warns (exit 0). `DW0846` and `DW0858` were numbered later and belong to
this group.
...
| `DW0461` | **Placement consistency (proof 2), in two arms.** A declared `at` contradicts where the replayed effect history actually leaves the NPC when the quest opens. Declaring a position does not teleport anybody: prescription is to stage the `move-npc`, or declare where the NPC actually stands. Skipped for a branch-divergent NPC, which `DW0462` handles instead. **The document arm** (`compiler::cast::check_cast`, validation pass) compares what the two documents say — an anchor mismatch, a declared anchor for somebody not in the world, or a declared `"dead"`/`"offstage"` for a body still standing — and cites both anchors. **The place arm** (`compiler::plan::build_critical_path`) compares where the two answers *are*: `(area, cell)`, resolved through `plan::body_station`, the one authority the world-init summon and the cast station both ask. An anchor name is an identity within an AREA and nowhere wider, so where two areas declare one name the two sides are the same string and two buildings: measured on a two-area perturbation of `talkto-cast-pos`, the datapack summoned the body and its interaction hitbox at `5.5 65.0 4.5` in `area/keep` while the critical path sent the party to `[261, 65, 4]` in `area/annex`, and the campaign compiled clean. Bound at the single site that consumes a ledger row's POSITION — the `talk-to` step — because a row no beat stations a body for states nothing the world can contradict. The message names both cells, both areas, and the binding in `world.areas[]` that is the only remedy the campaign owns. |
...
| `DW0884` | **A cast row whose anchor name two of this campaign's buildings answer to.** The area a beat plays in and the area an NPC is declared in BOTH provide the row's `at` anchor, so the name alone does not say which building the body is standing in. The compiler resolves a cast row in the beat's area first, so the row means the beat's building; the NPC is declared in the other, and unless something walks them across that is where the datapack summons the body — two places, and a delve that ships both sends the party to an empty cell. Refused rather than resolved for `DW0461`'s reason: the beat's area may win the NAME, it may not win a BODY. `compiler::cast::check_shared_cast_anchor`, run in the validation pass beside `check_cast` (build tier, exit 3, as `DW0857`/`DW0859` are) — the same finding `DW0461`'s place arm refuses at the build tier from the seated pieces, refused here where the row is entered, before a cell is computed. **Scope: areas that CERTAINLY answer to the name** (`gates::certain_anchor_providers`) — a bare `prefab` declaring it, or a `prefab_pool` every member of which does. A pool the solver may seat either way is left to `DW0461`, deliberately: this gate must not make a campaign's validity depend on a seed. **Prescription, and it is one the author owns:** to station the body in the beat's area, walk it there (`move-npc` before this quest) — declaring an anchor does not teleport anybody; to leave it at its post, bind a piece to one of the two areas that does not declare the name, in `world.areas[]`. Renaming the anchor is a prefab-library change these documents cannot make, and the message says so rather than prescribing it. **Boundary:** exactly the two areas the row's own scopes name. A third area answering to the name settles nothing here and is `DW0859`'s finding. No-op where a beat and a body share an area, which is every single-area campaign. |
| `DW0886` | **This pool cannot stand on this horizon.** `compiler::seating::check`, bound in the one validation funnel every subcommand goes through (`main::validate_loaded`), **validation tier, exit 1** — on `DW0855`'s own precedent, and for its reason: the whole verdict is a fact about the documents and the library (the declared base, the pools the world names, the members those pools hold, each member's metadata and its bytes), so nothing has to be placed to know it and a creator should not spend a build to learn that the pieces they chose cannot stand where they put them. The `DwCode` declares build tier, which is what happens if the same rule refuses with a build under way: `plan::area_base_y` derives an ocean area's origin from the same number and cannot proceed without it. **One question, several answers, so one code.** *No walk plane*: the piece declares no `walk_y`, on any base, and there is no default to fall back on. *Wading under the sea*: the base is `ocean` and the piece's own bytes stand a body below its declared walk plane, so those cells land at or below y=62. *An unstated shore*: the base is `ocean` and the piece authors water in the sea's own plane while declaring no `waterline_y`, so nothing states where it meets the sea — water ABOVE that plane is a cistern rather than a shore and is not this finding. *Fluid off the world*: the base is `void` and the piece's fluid reaches its own outer face, which is the static form of `DW0318`'s finding. *Two walk planes in one set*: the base derives an origin from a walk plane and the members of the area's piece set do not agree about theirs — a property of the SET, which no member can answer, because every one of them may be individually perfect and the solver's draw decides which stands at the derived origin. That last shape is the defect this whole check was measured failing: it lived in `plan::area_base_y` at build tier and nowhere else, so `delvec prefab seating` read the same documents, asked every question about each member, asked none about the set, and printed `SEATABLE` for `pool/island` (`walk_y` values `[2, 3]`) and `pool/cave-shore` (`[1, 2]`) minutes before the build refused both. `compiler::seating::set_walk_plane` is the one implementation now, and its three callers are the command, this validation check and the origin derivation. **It opens the `.nbt`**: a verdict computed from declarations alone reports a library of waterline fictions as seatable, which is precisely the green that existed before this code. **`DW0855` is not restated here** — one rule, one code; this message names its case so that a creator reading either one reads a single answer about which base seats what. **The walk plane binds on every base.** The missing-`walk_y` shape is computed on every base and refused on every base, from the one rule in `seating::seating_reasons` that `delvec prefab seating` reads — so the command's verdict about a library and the campaign's verdict about the pieces it seats are one verdict, not two. The ocean is where the number is also CONSUMED, because `plan::area_base_y` derives the area origin from it; `void` and `valley` state their own datum for the origin and still owe the declaration, because a piece with no walk plane is a piece nothing can say a body stands on (spec-0060 §4 rule 1: a piece placed on ANY base without it is this code). Measured over the pinned content library: 36 of 36 piece documents declare `walk_y`, and the command reports this shape on no member of any of the four pools' 42 memberships on `void`, `ocean` or `valley`; over the gallery library, on none of the six. The refusal is proven on two bases in the gallery — `a-piece-with-no-walk-plane` (`ocean`) and `a-walk-plane-the-void-still-owes` (`void`) — and the move each one names is reachable on both (`crates/delvec/tests/remedy_reachability.rs`). **It reaches BOTH placement models.** A campaign places its pieces with `areas[]` or with a site plan and never both (`DW0839`), so a rule that read one model had nothing to say about half the campaigns this engine accepts — and about EVERY campaign on the base that builds terrain, since `valley` rings a declared extent and a plan is the only statement of one (`DW0855`). Measured before the arm existed: the gallery's site-plan overlay with nothing changed but `horizon: ocean` analyzed green at exit 0 while `node/undercroft` stood two courses under the sea plane, and the build then refused it at `DW0344` after paying for the assembly, with moves naming a `blockout/node/undercroft.json` no author can edit. `check` now reads the plan's own resolver (`siteplan::placed_boxes`, never a second reading of the document, because a box's corner is derived from the packing) and asks each placed box the same question, through the same predicate `seating::feet_under_the_sea(base, world_y)` the piece arm goes through: **at the world y this thing puts a party's feet, does the base put sea there?** A prefab reaches that number by derivation; a box states it, because `PlacedBox::floor` IS a walk plane in world coordinates. **Exactly one of the five shapes crosses**, and the module states per shape why the others cannot: *no walk plane* is answered by the schema (`floor` is required, and a box naming an undeclared datum is `DW0112` before this); *an unstated shore* and *fluid off the world* need authored bytes, and `blockout::palette` is a fixed set of opaque cubes with no water in it; *two walk planes in one set* is a property of a POOL the solver draws from, and a plan states every box's plane individually. The moves are the plan's own — RAISE the place's `floor`, which a named `datum` does for every place standing on it at once, or CHOOSE a base with no sea — and there is deliberately no move naming a `walk_y`, because a site plan's mass is derived and authored by no one, so there is no prefab document to put one in. Both are taken green in `crates/delvec/tests/remedy_reachability.rs`; the refusal is proven in the gallery by `a-place-cut-below-the-sea`. **Binding**: `seating binding: horizon base …` states each placement model with its own numerator and denominator — `areas[]:` areas examined of areas, members, `.nbt` opened, and waterline declarations examined against declarations borne out; `site plan:` boxes judged of boxes the plan states — on every run, including the one that finds nothing, so the model a campaign did not use reads as a zero rather than as a silence. |
| `DW0887` | **A declared waterline that is not in the piece's bytes.** `compiler::seating::waterline_reason`, validation tier (exit 1) when a campaign names the piece, and swept over a whole library by `delvec prefab audit <library dir>` from the same implementation. A `waterline_y` is a claim about the bytes — the local y of the piece's top authored `minecraft:water` block — and until this code existed nothing read the bytes: the check that consumed the field asked only whether `pos.y + waterline_y` equalled sea level, never whether there was water at that plane. **Two shapes**: declared with no water anywhere in the piece, and declared over water standing at another plane. **Base-independent by construction**: the fiction is in the library, not in the campaign, so the same document is refused whatever base the world declares. Measured on the shipped content library at the pinned revision: 3 declarations, 3 borne out, 0 refused. That is a green over a library whose fictions were repaired, which is not a green over nothing but is not a proof either, so the property is bound by a PLANTED one — `crates/delvec/tests/seating_cli.rs` writes a `waterline_y` onto a piece that authors no water and checks the sweep refuses it, which holds *a declaration-only implementation reports N of N* against any library at all. **Three doors, one rule.** Validation when a campaign names the piece; `delvec prefab audit <library dir>` over a whole library; and `delvec prefab audit <asset>` at the **admission event**, over one template or one tile-set manifest measured as one assembled building. That last was the door this code was written for and did not have: nothing in any gate hands this command a directory — the admission procedure and the content repository's palette job both walk the pieces one file at a time — so a planted fiction passed 39 per-file audits of 39 with the code appearing zero times in their output. Every per-file report now carries a `waterline` binding whose `state` is `checked`, `undeclared`, `no-document` (an ingested piece audited before its metadata exists) or `unreadable` (a document nobody can parse is not a document with no waterline), which are the four facts one silence used to conflate. **It replaces a generator-private copy of itself**: `prefabs/gallery-generator`'s `assert_the_shore_is_standable` asserted exactly this rule inside one generator, reachable by one library and by no piece any creator would ever write; that half is deleted, and the gallery's shores are now proven the way every creator's are. **Prescription**: delete the declaration (a piece that authors no shore has no waterline to state, and deleting it does not make the piece unseatable on an ocean, which seats it by `walk_y`), correct the number to the local y the bytes hold, or author the shore the piece claims. Never answer it by changing the horizon. **Binding**: `waterline audit binding:` states documents read, `.nbt` opened, declarations examined, borne out and refused. |
| `DW0888` | **A prefab document's declaration that its own bytes deny.** `compiler::claims::check_piece`, validation tier (exit 1) when a campaign seats the piece, and raised by `delvec prefab audit` over one template, over a tile-set manifest and over a whole library directory, and by `delvec prefab seating`, all from the one implementation. `DW0887` held ONE key of a class nothing else in the document was held to: a connector pointed at a cell holding solid stone passed the audit through both its file arm and its directory arm at exit 0, and `prefab seating --horizon ocean` still answered four pools of four seatable. **The class, derived from the document type rather than from the key that was noticed** — every key that asserts something a reader can falsify by opening the `.nbt`: `structure.data_version` (the game version the templates carry); `walk_y` (a body stands at that local plane, by the engine's own standable rule); an anchor's `pos` and `region` (cells of this piece); a trap anchor's `dispenser` (that cell holds the dispenser a payload is loaded into) and `trigger_block` (that block, blockstate and all, sits on the anchor — a flag gate removes it and puts it back verbatim, so over an empty cell opening the gate creates a block the piece never had); a connector's `local_pos` and `opening` (the piece is open there and a body fits, over `admit::socket::opening_cells`, the carver's own geometry); a connector's `facing`, `name`, `target` and `joint` where its cell holds a jigsaw marker (the block's own `orientation` and block-entity fields); and the converse that closes them — **every jigsaw block the piece authors is declared by some connector**, without which a connector moved off its marker onto open air satisfies every forward check and leaves a socket in the bytes nothing points at. **A socket is a way through, or a way this piece's own document says is SHUT**: an opening standing in the cells of one of the piece's own gate-anchor regions is the barred door a campaign opens later, which the shipped `cave-mouth` is; the exemption is the object's own declaration and a connector drifted onto random rock cannot supply it. **Out of the class, per key**: names (`prefab_id`, `structure.id`, a tile set's `base`) and file references, which every reader that opens them bears out; provenance (`structure.generator`, `license`, whose `generated_by` is a claim about the bytes whose instrument is a re-expansion rather than a read); an anchor's `facing`, `role`, `note` and `resolves_to`; a gate anchor's fill `block`, which is what the compiler WRITES when the gate shuts, so the cells hold something else by construction — measured across the pinned library and the gallery, 5 declarations of 5; `shown_faces`, which the document's own definition rules out because reading it off the material would be inferring intent from it (`DW0885` judges it against the world); and `lighting`, a probe of a placed piece under a sky. **Already held, and not restated — one rule, one code**: `structure.size` by `DW0803`, `waterline_y` by `DW0887`, the spatial contract by the admission door, `footprint_class` by `DW0848`. **Binding**: `byte-claim binding:` states documents read, `.nbt` opened, declarations of the class examined and denied, and examined/denied per key of `claims::ClaimKey`, zeroes included. Measured over the pinned content library: 36 documents, 335 declarations, 0 denied. |
| `DW0885` | **A placed piece's outward solid boundary standing in open air, with nothing answering for it.** Build tier (exit 3), `compiler::burial::check`, bound in `emit::build_with_warnings` — the one function that turns a plan into a datapack, and the first point at which the three things the question needs are all in hand: the assembled blocks, the placed boxes, and the walk region the party is proved to stand in. `Plan::build` cannot ask it, because it runs before a single `.nbt` byte is read and so knows where the boxes are and nothing about which of their boundary cells are solid. **The gap it closes.** A prefab is authored for a context and nothing in the pair of files it ships as said which: `cave-cavern` is 69 % solid box boundary and every one of those cells is the cut edge of a hill it expects to stand inside, `hero-galleon-oak` is 6 % and every one of those is hull a player walks up to. So an `ocean` world laid a cave piece on a shore, buried none of it, and shipped a 52-cell stone-and-grass slab standing in open air over the beach with the party walking underneath — while `DW0780` found the sockets mated, `DW0782` found the envelope closed and `DW0344` found the waterline on the sea. Each of those judges a piece against itself or against its neighbour; none of them asks whether the world the piece was placed in is the world it was built for. **The rule**: every outward-facing solid boundary cell of a placed piece is buried — by another piece's box, by a block, or by what the horizon puts there — or the piece declares that side in `shown_faces`. **Burial is read off the world the build writes**, never off what a horizon is supposed to provide: a `valley`'s ground is real blocks in real templates and arrives in the assembled block map like any other piece's, an `ocean`'s sea is analytic and arrives as `nav::Ambient`, a neighbour is a box, and all three answer one question about one cell — so a horizon that grows a surround, or a layout that moves a piece, moves this verdict with no change here. **Where it binds.** Over the air the party can be in, flooded from the reachable walk region through the same blocks and the same ambient every other proof reads, bounded two cells outside the content. A sealed one-room delve is solid and exposed on all six sides and is NOT judged, and that is the rule rather than a softening: there is nowhere out there for a body to be, because `DW0322` refuses a reachable walkable cell over a bottomless column and the sea is not standing room. A piece the party can get outside of answers for its **whole** outside — air wraps, and no sightline is computed anywhere here, deliberately: a sightline would make a building's obligation a fact about one walk through the delve, so moving a waypoint would move what the piece has to be. **Two arms.** *The placement arm* names the side, how many of its cells stand in that air, the first of them, what the document declares, and the three moves. *The declaration arm* refuses a `shown_faces` naming a face of pure air — a side is finished exterior surface or it is nothing — read off the piece's OWN template bytes rather than off the assembled world, so a campaign that carves a doorway through a wall cannot invalidate a claim about the asset. A `shown_faces` entry that is not one of the six sides (`east` `west` `up` `down` `north` `south`, the face contract's own vocabulary) is refused where it is written, because nothing can be judged against a side that does not exist. **The three moves, and which ones a horizon leaves open**: BURY it with terrain — only `valley` builds any (spec-0026), so under `ocean` the sea buries what stands below y=62 and nothing above it, and under `void`, the horizon that declares there is nothing outside the placed geometry, the move does not exist; PLACE something against it, available everywhere, and the pair then answers to `DW0780`; DECLARE the side shown, available everywhere and the only move for a piece meant to be looked at — a claim about the PIECE, made in the prefab document, so it changes what that piece is in every world, which is the cost that keeps it from being the cheap answer. Declaring a side the piece was built to have covered is refused in the message's own words as the fiction it is. **Every offender, not the first.** The check walks every placed piece and collects; `emit::build_with_warnings` raises the first as the refusal and prints the rest, because one placement defect is routinely several pieces and a report naming one sends a creator round the loop as many times as the world has walls. **Binding**: printed on every build as `piece-exposure binding:` — before the refusal, so the run that finds something owes its reader the same counts as the run that finds nothing — and shipped as `validation/piece-exposure.json`; the residual, exposure the two-cell skin left unasked, is `exposed_cells` beside `judged_cells` rather than a sentence.
...
| Code | Meaning |
|------|---------|
| `DW0890` | **The approved design and the built world do not agree about the sky.** `compiler::design::check`, bound in the one validation funnel every subcommand goes through (`main::validate_loaded`), **validation tier, exit 1** — on `DW0855`'s and `DW0886`'s precedent and for their reason: the verdict is a fact about the documents and a directory listing, so nothing has to be placed to know it. The `DwCode` declares build tier, which is what happens if the same rule refuses with a build under way. **The defect it generalises**: a delve whose four approved reference images were all night scenes declared no `time`, absent meant `noon`, and the build, the machine ladder and 36 emitted Chunky scenes all ran under a blue noon sky — first seen by eye on the first point-of-view frame, after a build and 82 minutes of a 109-minute render. **One code, three shapes, because they are one fact seen from three sides — the record and the world are not the same delve** and the repair is always the same pair of moves. *The world reaches a sky no approved picture shows*, or the record states a sky the world never reaches: the two sets are compared for **equality**, as the two independent sets the engine already keeps (times and weathers, not pairs — that is what `reachable_time_weather` returns and what `DW0210` and `DW0496` read; a beat timeline that would make pairs meaningful is not modelled). *A row names an image that is not there*, including a stem two files answer to — a resolve by name over a scope where the names are not unique yields a candidate, not a match, and the refusal lists them. *An approved image nobody recorded*, and a campaign with images and no `design.json` at all is that shape over every file, its remedy naming the document and `delvec schema --stage design`. **It reads the campaign's clock from one place**: `compiler::light::reachable_time_weather`, the same scan behind `DW0210` and `DW0496`, which walks the declared initial state plus every `set-time` / `set-weather` target through every effect root at every depth, quest and dialogue both. A private scan here would be a third reading of one fact and a third chance for the three to disagree. **It never opens an image.** A model-based check of the hour would be a measurement with an unstated error rate standing where a refusal belongs — the shape spec-0028 §3 already forbids for `refscore`. Whether a row's tokens are true of the picture is a human reading, made once, at approval; what a machine holds is the token. **The document's own refusals are not shapes of this code**: an empty `references` is `DW0100`, a name that is not a path under `design/` is `DW0110`, two rows for one picture is `DW0111`. **Every move it names is reachable**, with a row in `crates/delvec/tests/remedy_reachability.rs` taking it and reaching a different verdict; `tools/check-dw-codes.py` refuses a message naming a move with no row. **Binding**: `design record: N reference(s) recorded over M image file(s) under `design/` …` on every run, zeroes included — a campaign with no `design/` and no `design.json` prints `0 reference(s) recorded over 0 image file(s)` and does **not** refuse, because that is a measured zero of an optional surface and `tools/staging-gate.py` is where a zero of this class is a red. **Artifact**: `validation/design-record.json`, written by **every** build including one with no record, carrying `references`, `image_files`, `by_directory`, `skies_stated`, `world`, `unrecorded_files` and `unresolved_rows` — `references: 0` is a number the staging gate reads, and an absent file is *I could not look*, which is a different fact. |
| `DW0891` | **A killing volume the player cannot see** (spec-0062). One code, three shapes, one rule — *a killing volume and what shows it agree*. The ruling it transcribes is the owner's: **avoiding a lethal volume's collateral damage is never done by marking ground that looks walkable as unwalkable, because the player does not know.** The constraint belongs on the volume: it sits at the bottom of a pit, or far enough from the mouth that a body standing on solid ground cannot be caught. **The world arm** is `compiler::lethal::check_danger_is_visible`, build tier (exit 3), raised once per volume over the final assembled world — whether a cell is floor is a fact about the settled bytes and where the volume is at all is a fact about the solved layout, so nothing before assembly can answer it. Its two shapes: *caught floor that shows nothing*, naming the volume, its keep-out box, the caught cells by floor as `DW0881` prints them, and the declared signals if any; and *a declared signal the bytes do not hold* — a `shown_by` block under or in no caught cell, naming the block and the count of caught cells it was checked against, where a count of zero says the volume catches nothing and the declaration is what is wrong (`DW0887`'s shape, on a volume instead of a waterline). **The document arm** is `dsl::validate`, validation tier (exit 1): a `shown_by` id that is a known block and not one vanilla hurts a body with — nothing has to be placed to know it, and the message prints the set to choose from. **The terms, and each is a set the engine already computes**: the keep-out is `metrics::keep_out_box(Body::PLAYER, V)`; the population is `World::reachable_walkable` over `World::without_lethal`, rooted at every cell the campaign PUTS the party (the entry spawn, every `set-checkpoint` and `bonfire` seat, every transit-teleport destination) rather than at the entry alone as `DW0881`'s is, because a party teleported into an area stands on that area's floor; the caught cells are the intersection; and a caught cell **shows** when the block **under** it or **in** it is one of `shown_by`. Under or in, never beside — a shore cell next to lava stands on stone and holds air, and the lava beside it is not what the body is on. **The population is the lethality-free one, and that is the whole binding.** On the world the router walks, the keep-out is already impassable, so a population taken from it can never contain a caught cell and this check would be green over every volume ever written while matching nothing. `the_population_is_the_lethality_free_one` computes it both ways on the `lethal-volume` fixture and asserts **zero** over the lethal-applied world and **nine** over the counterfactual, and the check is bound to the second. **What separates a lava lake from a kill zone over stone** is never the volume: the occupancy model classes water and lava alike as flooded — impassable, never floor — so no cell of a lake's surface is ever standable, ever in the population, ever caught. A volume drawn to the lake's *shore* catches the shore, which stands on stone and shows nothing, and is refused; the remedy is to draw it one cell in or one course under. **One code for three shapes** because they are one disagreement and every remedy each names is admitted by the others: signalling the floor satisfies the first and creates no fiction, deleting a fiction never uncovers floor, naming a hurting block never makes a floor read as safe. **Where it stands in the order**: `DW0511`, then this, then `DW0510`, then `DW0850` and `DW0881` — a volume that catches walked floor usually closes a route as well and often sits under a reach, so asked first the refusal names the cause and every later rule then judges a volume the player can see. It reads the plan's volumes, the population and the block map, never a route, so nothing is lost by asking it early. **Bound at both entry points**: `emit::build` and the world-edits batch replay (`compiler::edit::check_batch_invariants`), before the route proof at each — a gate bound at one of two doors is bound at neither. **It changes nothing about the walk graph**: the router still refuses every cell of the keep-out, the recovery stake still chooses its lip outside it, `death-plan.json` still carries it to the bot. What this guarantees is that the cells the walk graph loses are cells a player could see were dangerous. **Prescription, always the geometry's**: lower the volume so its keep-out's top course lies under the floor (a pit's volume sits on an anchor at the pit's bottom); draw its `extent` in so the keep-out stops one cell short of the floor; or author one of the blocks vanilla hurts a body with under those cells and declare it in `shown_by`. Never mark walkable-looking ground unwalkable. **Binding**: `danger-visibility binding: N volume(s) examined against a walked population of M cell(s); …` on every build that declares a volume, refusals included, and the same numbers in `validation/lethal-gate.json`'s `danger_visibility`. |
...
`DW0862` judges what the document says — a `hint` that asks to be shown and an
absent `title` that guarantees it will not be — a contradiction between two
authored fields. The other three require the campaign to have something: they
are the forcing function `DW0481` applies to the story, turned to face the
player, so an objective owes an account of itself in both directions.
...
| `DW0849` | **An item gate a class cannot bring.** An `interact` completes only for a player HOLDING a named item, and the item has no supply this campaign gives to a player of every class: its only source is another class's `kit`, or it has no source at all. Validation tier (exit 1), `dsl::validate::item_gate_class_checks`. A delve is played by one to four players who each pick one class, so a **solo player of any class is a supported party** — an item only one class carries makes some real party assembled unable to finish, and it learns that standing at the thing it cannot press. Quantified over EVERY class for the same reason `DW0476` is: one class that cannot bring it is as broken as none. **The class-blind supplies are an enumeration, not a list of the verbs one author remembered** (`dsl::validate::class_blind_item_sources`): a `give-item` at any effect root (walked through `dsl::stages::for_each_campaign_effect`, which is `dsl::effects::for_each_effect_root` underneath — the same closed enumeration emission lowers from), a `collect` objective's item, a `loot` container's stack, and a wave mob's `drops` — the quest-token form and the worn-piece form, whose item comes from that mob's own `equipment`. A trap's `dispense` payload is deliberately **not** a supply: a dispenser fires its stack at the party as a hazard, and being shot with a thing is not being handed it. **The approximation runs one way on purpose.** A flag-gated `give-item` late in the DAG counts, because the reachability model does not model items at all and a stricter rule without one would refuse correct campaigns. So this fires only where *nothing* class-blind supplies the item — which is the shape the finding had, and the shape a mistyped-but-real item id has. **Binding: `interact` objectives declaring `requires_item`.** Prescription: put the item in a `collect` or a `loot` container on the way to the gate, hand it out with a `give-item` (whose default `carrier` is `all`), or add it to every class kit. Never drop `requires_item` to silence it — presenting the item is the beat. |
| `DW0850` | **A `reach` the party can arrive at without completing.** Build tier (exit 3), `compiler::reach::check_reach_completion`, run at the same site and over the same final assembled world as `DW0314`. Two assertions per reach objective, one code, because the remedy is the same. **Occupiable:** some cell of the completion volume is standable — a volume no body can be in is an objective nothing completes, which is the reported instance (a point sphere too tight for a human standing on the altar cell) stated as a property instead of as one altar. **Delivered into:** where the critical path walks to the anchor, the cell the leg actually ends on is inside the volume. That half is the one no existing proof could see, and it is live arithmetic rather than a hypothetical: `nav::SNAP_RADIUS` is **3** and the v0.3+ completion cube's half-extent is `max(1, radius)`, so a reach whose only footing lies further out than its own volume reaches is routable (`DW0311`), standable-on-export (`DW0314`) and walkable — and the party, standing exactly where the campaign routed them, is outside the volume that fires the objective. **The volume has one authority** (`compiler::reach::reach_completion`): the emitter formats its `tick` selector from the value this proof judges, and `critical-path.json` carries that same value to the bot as `completion`, so the string and the check cannot come to disagree about a rule that is invisible in the DSL and shows up only as a beat that never happens. Membership is read conservatively — vanilla tests hitbox intersection, so a body one cell out may complete on a face-touching tie, and demanding the certain case is what makes a green here mean *the party completes this* rather than *might*. **Binding: `reach` objectives whose anchor the plan resolves**, quantified over the quests rather than over the exported path, because a reach on an optional quest is a reach a player can be standing at. **Prescription names a VERIFIED radius** (spec-0062 §7.2): `r_min`, the smallest radius from 1 to `max(authored, SNAP_RADIUS)` whose judgement — taken again there, over the same world — is green under both this rule and `DW0881`. The message says `set radius: r_min`, or, when no radius in that range answers, that none does and the remedy is the anchor's placement. The old *never widen the volume* is gone from the message and its reason is why: widening it BLIND is what left the other number live, and a number this rule has re-judged is not blind. Move the anchor onto the footing, or give the anchor cell floor, remain the other two moves; never nudge the waypoint. Note the arithmetic the authored radius sets: an endpoint snaps inside a box of half-extent `SNAP_RADIUS`, so at `radius >= 3` every arrival the route can deliver is inside the volume and the **delivered into** half has nothing left to catch, while **occupiable** binds at every radius. That is the defect being absent rather than the check being blind, and `tests/reach_completion.rs` pins it so a later narrowing of the volume cannot re-open the gap silently. |
| `DW0881` | **A raised `reach` anchor that completes from the floor below it.** Build tier (exit 3), `compiler::reach::check_reach_footprint`, run at the same site and over the same final assembled world as `DW0850`, immediately after it. The completion volume is centred on the anchor cell in all three axes and vanilla adjudicates the selector against a body's whole AABB rather than against the cell its feet are in — a standing body rises `metrics::PLAYER_HEIGHT`, so a cell one course under the volume's bottom layer already reaches into it. Put the two together and any raised anchor whose radius reaches a lower floor completes from that floor: the party never climbs, the beat fires during whatever they were doing down there, and every other proof stays green (`DW0311` routes to the anchor, `DW0314` finds the waypoints standable, `DW0850` finds footing in the volume because the footing is the anchor's own). **The rule is over the FOOTPRINT, never over the radius**, because a number is the wrong thing to bound: a radius generous over a flat plaza and one reaching through a mezzanine floor are the same number. The footprint is every standable cell whose body box meets the volume (`ReachCompletion::possibly_completes_from` — the GENEROUS reading, deliberately the opposite of the conservative one `DW0850` takes, because a rule that refuses a completion must ask what could complete rather than what certainly does), and the demand is that each cell can walk to the anchor's own footing **without leaving the footprint**, under `World::neighbors`, the engine's one step rule. **The footing is a SET** (spec-0062 §7.1): the anchor's own cell when it is standable — one cell, and nothing changes — otherwise every footprint cell at the minimum distance² from the anchor, and the demand is a walk to *some* cell of it. That is a **loosening**, declared in those words, and it is bounded exactly: floor farther from the anchor than its footing is refused as before, because the set is taken at the minimum. What it stops doing is resting the verdict on a tie-break. `World::snap` is no longer consulted for the reference: for an anchor no body can stand on — a one-cell killing volume, a lever in a wall, a solid altar — the lips around it are equidistant and mutually unreachable inside the volume, so which one `(distance², cell)` picked decided the verdict, and a resolve-by-name over a scope where names are not unique yields a candidate, not a match. The snap's own defect — preferring a lip the party cannot reach over a doorway it can — is recorded against the snap and not repaired here. A ramp or stair inside the volume satisfies it and is meant to: a body on it is arriving. **Directional**: the walk runs backwards from the footing, so that a body able to fall off a loft into the hall does not excuse the hall floor — the question is whether a body standing there can get to the anchor, not whether the anchor can get to it. **An offender has to be somewhere the party can already stand, and which cell that walk is rooted at is the whole difference between a rule and a false alarm.** The population is the walk from the CAMPAIGN'S OWN START over the same assembled world, so a roof, a stone canopy or the top of a stamped shard that a generous radius happens to cover is not reported, and neither is floor beyond a doorway whose planks a beat lays at runtime — before that beat nobody is over there, and the beat that puts them there is the same one that joins the two inside the volume. Rooting it at the ANCHOR's footing instead looks equally reasonable and is wrong, measured rather than argued: the gallery's mezzanine ships with its flight broken and laid by an `open-way`, so a population walked from the loft drops the hall floor the party actually completes from — zero offenders where there are nine. **Residual, named rather than implied:** a cell the party reaches only after a runtime write is outside the population and so outside this rule; that is the conservative direction, and it is bounded by the same write, since whatever joins the party to such a cell also changes the geometry the volume is measured against and no static world holds two versions of it at once. **Binding: reach objectives examined, the cells their volumes cover, the cells the party can stand on, the footprint drawn from those two, and how many of the footprint stand off the anchor's floor** — printed on every build whether anything was found or not. A code of its own rather than a `DW0850` variant for the reason `DW0510` is one: `DW0850` says the objective cannot be completed and sends the author looking for missing footing; this says the objective completes too easily, from a place that is not the place, and more floor is the last thing that would help. **Prescription names a VERIFIED radius**, and never one above the authored value — this rule's whole finding is that the volume reaches too far, so a wider one cannot be its remedy. The message says `set radius: r_min`, the smallest radius from 1 to the authored one whose judgement is green when taken again there (spec-0062 §7.2); where none answers it says so and sends the author to the anchor's placement, which is the move `DW0850` also names and which `remedy_reachability.rs` takes under that code. The other two moves stand: move the anchor so its own footing is the only floor inside the volume, or build a way UP inside it. **One judgement, two callers**: `compiler::reach::ReachJudgement` derives the standable set, the footprint, the footing and the walk once, and `check_reach_completion` and `check_reach_footprint` read it — neither derives geometry of its own, which is what makes the pair unable to prescribe each other's refusal. **Provenance, so a later reader knows which half to distrust:** the PROBLEM is cited — an oversized trigger volume activating from the wrong side of geometry is a known level-design failure, and "floor plane" is the established unit a place is chunked into (*The Level Design Book*, `book.leveldesignbook.com/process/layout/flow/verticality`: verticality is chunked into floor planes and most areas hold at most three). The RULE — walk-connectivity to the anchor's footing inside the volume, over a population rooted at the campaign start — is AUTHORED: no published source was found stating a connectivity criterion for trigger-volume extent, and searches for one turned up only engine documentation on how trigger volumes are configured. That is the weak spot, recorded rather than smoothed over, and it is a gap against the level-design research line rather than a permanent hole. |
...
### DW0510–DW0512, DW0891 — lethal volumes (`compiler::nav` / `compiler::lethal` / `dsl::validate`; spec-0031, DSL v0.10; spec-0062)
...
A *wave seat* is the third: the cell the seating pass stands one of a wave's mobs
...
* a **walker** does not, because its cell fixes it only to `[c, c+1)`, so
  ...
  is the set the navigation model makes impassable, the set a recovery stake's
  near lip is chosen outside of, and the set `death-plan.json` carries to the bot
  tier as each volume's `keep_out`.
...
critical-path legs routed, PackTest templates generated (one per volume), and
`danger_visibility`: the walked population, the caught and shown cells per volume
and over the campaign, and how many declared signals the bytes bear out. A
...
`crates/delvec/tests/fixtures/die-retry` is the smallest campaign that lets it
bind, `crates/delvec/tests/die_retry_fixture.rs` holds it to that shape, and the
...
verb reads a `detail-plan`. `DW0848` runs at `delvec prefab audit` — the admission
...
`delvec render` exit codes: `0` ok · `2` input · `3` output · `4` fidelity-gate
failure · `5` renderer/GPU · `10` internal.

#### `delvec render` dark-shot REVIEW POLICY (night-vision emulation)
...
| Code | Meaning |
|------|---------|
| `DW0840` | **The gym leaves a building metric unwalked.** One warning per `delvec metrics --gym` run (exit 0), naming every building entry the generated gym is built from nothing of. The gym's whole argument is that walking it settles the standard, so an entry no bay instantiates is a number the walk cannot rule on however carefully it is walked — a finding about the authoring vocabulary rather than about the run. **Bound to the READ, and stated against the whole table.** The numerator is `Reads` — the same ledger `DW0813` uses — so an entry counts as instantiated only when the generator actually consumed its value to decide a footprint, an opening, a pitch, a datum or the walk's length; the denominator is every entry in the table. A hand-maintained list of "what the gym covers" would be exactly the drift this measures, so there is none, and a table entry added tomorrow and reached by nothing is named the first time anyone regenerates. There is no exemption and no acknowledgement: an entry is reached or it is reported. **Binding: entries instantiated, of entries defined.** Zero unreached entries means the line does not print, which is the calibrated end state and not a vacuity, because the count is taken against the whole table either way. |
...
| `DW0828` | **A seam is not on a shared face.** The declared face is not shared by the edge's two boxes — their spans miss each other on an in-plane axis, a horizontal seam names a sky-open place with no ceiling or floor plane to sit in, or, through a floor or ceiling, the two are not one cell apart (the message states the gap it measured, and an overlap is stated as one) — or the crossing's corner lies off the shared area. Three more shapes since placement became relational (spec-0059): an `at` or `meets` that lies off its own box's face; an offset of the wrong shape for the face (one number through a floor, two along a wall); and, on a seam whose two boxes both already stand, a crossing that is not the same cells seen from `a` and from `b` — **the loop that does not close**, refused at the seam that closes it with both corners and how each was obtained. **Seams are allocated on faces both boxes already have**: the two-places-cannot-mate failure class is resolved here, where both boxes are still free to move, and never later between two finished buildings. The refusal prints both boxes' cell ranges and floors. |
| `DW0829` | **A seam's opening does not fit, or its sill cannot be reached.** Two halves of one claim that the opening is usable. Geometric: the named standard's `width × height` anchored where `at`/`meets` put it runs past the shared face, named with both rectangles — the standard set is the vocabulary, so an opening is never quietly cropped to fit. Physical: on a vertical face, a `walk` or `barred` seam's sill is more blocks over the floor of a side the connection is entered from than a body reaches by jumping, so the connection the graph declares is not one; the sill is the higher of the two floors (spec-0059), so the prescription is to bring the floors within a step of each other or declare a `stair` and let the treads carry the climb. (An opening name the table does not define is `DW0812` — that is the table being the single authority for the vocabulary, and this code is the geometric half.) |
...
| `DW0883` | **A box is not placed exactly once** (spec-0059). Two shapes of one claim. A connected component of the seam graph in which no box pins its `min`: nothing places it — a box stands only where a pin puts it or where a seam hangs it off a box that already stands — named with every box in the component and the prescription to pin one of them (the entry's, when it is there). Or a pinned box a seam also reaches, at a different corner: the pin and the seam disagree, named with both corners and how each was obtained, addressed at the seam. A pin is a claim the packing verifies, never a second authority; a box the packing does not reach has no cells, so no geometric rule fires over it and the one refusal is the whole story. Validation tier (exit 1). Binding: the pinned / derived / component counts in the site-plan binding line. |
...
| `DW0851` | **The sea is in the walk region.** A cell a body was proved to stand on, in a `horizon: ocean` world, whose own cell holds water once the world loads. `compiler::nav` (`measure_sea_seepage` / `SeaSeepage::finding`), build-tier (exit 3), at the same **two** call sites as `DW0318`/`DW0322`: once per world-edits batch in the stage-8 replay, naming the batch, and once over the finished assembled world at stage 10. **The gap it closes.** The world model holds water in two disjoint places and only one of them reached walkability: `assembled::Occupancy::flooded` is seeded from the assembled BLOCK MAP — prefab-authored sources and waterlogged blocks — while the ambient sea is not in that map at all. So the sea never reached `flooded`, never reached `is_occupied`, and never reached `is_standable`. **The model.** The question is asked of THE WALK and of nothing else: the denominator is the reachable standable set the build already computed, with the critical path's own cells counted inside it. The sea gets in two ways, and both are seeds of one flow. *An open face*: a non-blocking cell inside the built volume, in the sea's own band (`floor_top < y ≤ level`), 6-adjacent to an ambient sea cell. *A waterloggable block the placement hands it*: `/place template` carries the fluid already in a cell onto the block it writes there, so a stair, a fence, a pane, a chest or a set of iron bars placed below the sea plane comes out `waterlogged=true` whatever the template declared — and a waterlogged cell is a genuine water source. *Flow*: `assembled::flood`, the same function the block map's water runs through (infinite-water source formation, then 7-level decay with infinite downward fall) — never a second physics. *Confinement*: the one-cell skin of non-built cells around the built volume becomes barrier, so the flow stays inside the content; what leaves it is `DW0318`'s question. *Verdict*: a walk cell whose OWN cell the flow reaches, because that is where the feet go. **Why the foot cell.** The line was the head cell once, on the argument that feet wet with a dry head is wading and vanilla lets a body walk it. The field case settled it the other way: a two-scene delve whose two objectives stood at `[260,61,4]` and `[260,61,8]`, both `minecraft:water` in the delivered world, both dry to the head, under `walk_cells_wading: 0` and a passing verdict. The mechanism was the second seed — its staircase came out waterlogged four cells under the surface and ran down its own treads, through a hull whose open contact face was correctly zero. A walk cell that is dry underfoot and merely TOUCHES water, at head height or beside it, is the shoreline: counted and named, not judged. **Runs after `DW0318` and before `DW0322`**, inside `verify_boundary_safety` so no caller has an order to get wrong: a walk region the sea is about to fill is a false premise of the stranding proof. The walk region is computed once and handed to both. **Direction of error**: the seeds enter the flow as sources where vanilla would start them one level down, so a wide contact face fills further than the game would — over-marking, which can only turn a proof red. **Not the shoreline outside the content**: a shore piece that authors its own water to the waterline (`DW0344`, spec-0048) has it in the block map already, so those cells are `flooded`, not standable, and never in the walk region. **Binding**: the walk cells examined and how many of them the critical path names, the submerged and wading cells BY COORDINATE, the objectives standing in them, the open contact face, the blocks the placement waterlogs, the cells the sea reaches and the pieces examined — in the message, in the printed `sea-seepage binding:` line and in `validation/sea-seepage.json`. Prescription: raise the floor clear of sea level, close the face the sea comes through, or author the water so the room IS flooded and every proof downstream knows it — never move the path around it. |
