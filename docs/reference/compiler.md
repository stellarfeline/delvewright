...
| `DW0366` | A horizon param is out of range for its declared base (v0.9, spec-0026): `valley.ratio` outside `2..=3`; `summit.min_drop` < 100; `summit.plateau_y` overflowing the −64..320 build range once its `min_drop` gorge is carved under it; `sky.float_y` outside the build range; `summit.vista_radius` < 192; `flatland.blend_width` outside `1..=16`. **Also** a param declared on a base it does not belong to (`blend_width` on `ocean`, `ratio` on `flatland`, …) — the object form is one flat schema, so param-to-base ownership is a validation fact, not a parse fact. Validation-tier (exit 1), `dsl::validate::horizon_rules`. The last two arrived with the spec-0026 amendment of 2026-08-04, which this slice had reported as unresolvable ambiguities rather than invent: `vista_radius` is now measured outward from the scene bounding box with floor 192 (= the shipped summit `view-distance` 12 × 16) and **default 208** — the pre-amendment default of 176 sat below its own floor, so the check could not be written; `blend_width` gains `1..=16`, since 0 is the hard material edge the §3 interpenetration ruling forbids. A pinned default that fails its own range is itself a test (`pinned_defaults_satisfy_their_own_ranges`, all 6 bases). Prescription: bring the param into its documented range, or move it to the base it was meant for. |
