...
- The compiler is the module `delvec::compiler` of the one package
  ...
  [`tools.md`](tools.md).
- Versions (as of this doc): `delvec 1.2.0`, `dsl 0.21.0`, `mc 1.21.11`.
  `dsl 0.21.0` is the **one** `dsl_version` this engine accepts (ADR-0024): every
...
| `DW0102` | The document's `dsl_version` is not the one this engine accepts, `0.21.0`; the message names it (ADR-0024). Raised per stage document by `dsl::validate::envelope`, and for an l10n sidecar under `DW0180`. |
