...
- task #179 has **landed** at `dsl_version 0.9.0` (owner ruling 2026-08-04):
  declared `drops[]` on an elite/boss wave mob and on an actor — either a worn
  `{slot}` or a `{item, name?}` quest token — plus the `collect` objective's
...
| `close-gate` | The same **region write** with the anchor's declared fill block and no `replace` clause — `/fill <region> <block>` (the dual of `open-gate`), **plus `execute unless entity @e[tag=dw_seal_<anchor>] run function <ns>:seal_arm_<anchor>`** (v0.8, task #142 — the owner's island finding #34: a sealed boulder answered a right-click with silence). See [The seal answers](#the-seal-answers) below. |
...
- `<out>/validation/traversal-gate.json`: the `DW0452`/`DW0453` proof's **binding
  ...
  nothing else). A second block, `declared` (DSL v0.11, spec-0033), states the
...
- `<out>/validation/respawn-safety.json`: the `DW0478` proof's **binding ledger**
  ...
  `reign_end`), every hostile force, `examined`, and `pairs`: the comparisons
  actually made. Per respawn point it also lists `compared_against` and
  `not_compared`, each skip carrying the reason it could not meet the party
  there. `grandfathered[]` is the respawn points the **obligation fence**
  withheld — a plain `set-checkpoint` whose `stage` document declares below
  `0.11.0` — each with its `anchor`, `kind`, `pos`, `stage`,
  `binds_at_dsl_version` and reason; it is that campaign's adoption-round
  worklist for this rule. `unbound` is `pairs == 0`, and a `reason` is present
  exactly then, naming which of the four zeros it is: **every respawn point
  fenced out** (asked first, so a fenced campaign can never wear the empty
  campaign's sentence), no respawn point, no hostile force, or both present and
  never contemporaneous. This exists because `DW0478` returned
...
### Press answers — what a pressable thing says back (v0.8 task #142; generalized v0.11)
...
Since task #169 the roots themselves are enumerated exactly once, in
...
Four verbs produce those writes and none of them owns the rule — `close-gate` /
`open-gate` (a box and a block off a prefab gate anchor), `fill-region` /
`clear-region` (an authored box), plus a `shortcut`'s world-load seal. What each
...
- **Boundary safety and the world-generator ambient (`DW0322`).** The check's
  ...
  - **Anchor seating (`nav::AnchorRoot`).** The walk region this proof examines
    ...
    that `World::confined_standable_cells` closed for wave seating (task #41).
...
| `DW0478` | **The respawn-point safe zone** (spec-0016 §1, owner ruling 2026-08-04). A cell the party comes back to life on sits inside some hostile force's aggro range. Build-tier (exit 3), `compiler::nav::check_respawn_safe_zone`, run after wave seating and lane resolution because it needs both. **The object class is every respawn point** — a `bonfire` and a plain `set-checkpoint` alike (they are siblings of one sum type, resolve to one `CheckpointPlan` distinguished only by `rest`, and vanilla returns a dead player to either by the identical `spawnpoint`). Keying it to `rest == true` made it a hook on one variant and not its sibling: `nobodys-cave-island` shipped three `set-checkpoint`s and six hostile forces while this proof examined ZERO objects and reported green. **The code binds at every version; the WIDENING is fenced at 0.11.** `DW0478` is `DwCode::every_version` and stays so — it has bound every `bonfire` since it was written, and a `Binds::Since` on the code would grandfather that half too, which is a weakening and not a fence. What widened is the BINDING, so the binding carries the version, at the granularity it has: **per respawn point**, keyed to the stage document that authored the verb (`CheckpointPlan::stage`, read through `delvewright_dsl::stage_version_of`). A `bonfire` is examined at every declared version; a plain `set-checkpoint` from `0.11.0` (`nav::PLAIN_CHECKPOINT_BINDS_AT` — the version current when the widening landed). This is the second granularity `dsl::fence`'s docs name, the same shape as `l10n::required_inventory`, and it exists because the widening first shipped unfenced and stopped `nobodys-cave-island` (quests 0.8.0) compiling on an engine change alone — the one outcome the owner's 2026-08-10 ruling forbids. Nothing is weaker at or above 0.11: every respawn point is examined against every contemporaneous force on exactly the geometry it was before, and a grandfathered soft-lock is deferred to the campaign's own adoption round, not forgiven — `validation/respawn-safety.json` publishes `grandfathered[]` as that round's worklist. A plain checkpoint is **monotonic**, so it is measured only against forces that can be in the world while it still governs — its reign ends when a later `set-checkpoint` replaces it (`Plan::respawn_reign_ends`), and a force's onset is the earliest `spawn-wave`/`spawn-actor` beat that stages it (`Plan::hostile_onsets`; a staging from a root with no beat of its own — a trigger, a trap payload — is conservatively step 0, and `unleash-actor` is not an onset because it replaces a puppet rather than summoning one). A bonfire never stops reigning, so it is measured against everything, exactly as before. The window narrows WHAT IS COMPARED, never what is demanded of a compared pair. The binding count is published as `validation/respawn-safety.json` (`examined`, `pairs`, `grandfathered[]`, `unbound`, `reason`, plus per respawn point the forces it was and was not compared against, each skip carrying its reason) — a proof that examined nothing must not read as a pass. The rule: for every wave and every fighting actor, the distance from the respawn cell to that force's occupied cells must **exceed** its `follow_range` — and for a **lane path cell**, `follow_range` **plus the measured marching drift** of 7.9 blocks (`nav::LANE_MARCH_DRIFT`; owner ruling 2026-08-04): the td-routing-spike dossier measured a marching squad as a corridor around its polyline (followers mean ≤3.2, max 7.9 blocks off-lane), so a centre-line distance understates the squad's real aggro reach — a fire can clear the polyline by 2 blocks and still be perceived, which is exactly how run nine died at 17.7 blocks from a 16-`follow_range` lane. Stationary cells (seated spawns, staging anchors) keep the plain `follow_range` term. Occupied cells are the DW0312-proven **seated spawn cells** (where the datapack really summons it, not where its anchor is), plus — for a `lane` wave — every cell of the DW0386-proven **march polyline**, because a lane wave's whole design is that it walks that corridor while the party is elsewhere. Radius: a lane's `aggro_radius` (emitted verbatim as each lane mob's `follow_range`), else the largest declared `follow_range` among the wave's mobs, else the documented default 16 — one number, never a per-species table the compiler would have to invent (`DW0475`'s rule). An **actor** counts when the campaign declares it as a fighter — `unleash-actor`ed somewhere, or staged `vulnerable` — the same declaration-based test `DW0469` uses; species is never consulted, because the pinned entity registry is a membership set with no mob-category data. Why error tier and not a §7 pacing lint: a respawn point is where the party comes back after a death — and, for a bonfire, where every `respawns_on_rest` wave is put back on its feet — so a cell inside a perception radius delivers the party into contact on the tick they arrive — the retry loop the fire exists to make cheap becomes a soft-lock, and there is no reading of that geometry that is the authored point. The message names both sides, the closest offending cell, what kind of cell it is, and the measured distance. Prescription: move the respawn point out of the danger (a side room, past the threshold, beyond the end of the lane) or move the force's anchor / lane — **never** shrink `follow_range` to buy the clearance, which retunes a fight to hide a placement bug, **and never delete or retire the checkpoint** to buy it either: an armed respawn point ahead of a mandatory encounter is exactly what the bot ladder's die-retry stage binds on, so removing it trades the red for a proof that scripts no death and examines nothing. Both cheap ways out of the message are a vacuous green; geometry is the only honest lever. **The joint constraint with the die-retry precondition, and what it excludes.** For the checkpoint that *governs* a mandatory encounter the two demands are live at once, and the contemporaneity relief above can never apply to it — a checkpoint retired before the wave is staged is by definition not that encounter's governing checkpoint. So the pair is satisfiable at any world scale above **one perception radius of walkable separation** (the loop is respawn → walk back → re-engage, so a second area `AREA_SPACING` across the void does not count), and unsatisfiable below it. Excluded shapes, exactly: a single-room delve no wider than the radius, an arena campaign that rests inside the arena, and a lane wave whose polyline passes within `aggro_radius + 7.9` of the only rest point. Those campaigns still ship — they simply cannot prove the retry loop, and the run report states that as a zero binding rather than as a pass. `crates/compiler/tests/fixtures/die-retry` is the worked example: over one 11×11 `prefab/hello-room` it satisfied neither demand (the marked stone stood 3.0 blocks from the guard's seated cell), and it is now the smallest `pool/stone-keep` assembly — five pieces — that satisfies both, at 23.0 blocks. Its `quests.json` declares `0.11.0` for the same reason: its checkpoint is a plain one, so below the fence it would be grandfathered and the keep would go on being five pieces while proving nothing. Four pieces is 15.0 and still red: the layout solver spends filler on the gaps *before* each required room, so the fifth piece is the first that buys a corridor between the gate room and the boss hall. |
...
Owner ruling, 2026-08-08: *the stake anchor is the point, on the walkable path from
the respawn point in force at the moment of death to the death point under the
quest state in force at that moment, that minimises distance to the death point.*
...
| `DW0727` | `delve-render` | **A shot's cutaway removes the whole model** (exit 2). A cutaway is a set of axis-aligned half-space clips over the model's box, so the kept set is a box and this is arithmetic (`cutaway::Cutaway::kept_box`), not a sampled guess — it catches both the single over-deep clip (`y-max:100%`) and the pincer whose halves only meet each other (`x-min:50%+x-max:50%`). Refused before a pixel is rendered, and before the resource pack is even loaded, because an empty frame is indistinguishable from an empty body: the reviewer reads "there is nothing here" where the truth is "you asked for nothing". Cut less, or cut from another face. |
| `DW0728` | `delve-render` | **The supplied pack has no blockstate for a block the piece contains** (exit 2), naming each block and how many cells it covers. The mesher's own answer to an unresolvable block is to leave it out and print one warning per occurrence, so the frame shows a building the `.nbt` does not describe and the only trace is chatter nobody counts — a renderer that silently omits blocks lies to the reviewer it is the sole evidence for. Decided over the palette before any GPU work (`render::unresolved_blocks`). Two causes, one verdict: the block was renamed or removed in the pinned version (measured: 1.21.11 calls the plain chain `minecraft:iron_chain`, and `minecraft:chain` cost 127 cells across three of eight zone sheets), or the pack is not a complete client jar. Fix the block or the pack; never render past it. |
| `DW0729` | `delve-render` | **A plan key annotated nothing**: the piece's metadata declares no anchor the key could place (warning; the picture is still honest about the shape). Stated because a key that annotated nothing is indistinguishable from a key of a piece with nothing to annotate, which is exactly how anchors reached no page for as long as they did (CLAUDE.md: a gate that binds to nothing is vacuous). Warning rather than error because the repair is upstream — an anchor is DECLARED by a rule (`mark`), and no amount of rendering recovers one that was never declared. `delve-render batch` repeats it once for the whole run, naming every piece. |
