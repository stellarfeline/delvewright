...
| `DW0736` | `delve-grammar` | An **orientation-sensitive block state filled into a reoriented scope with no `orientation` guard**. A grammar reorientation permutes geometry and never rewrites block-state properties (`crates/grammar/src/orient.rs`), so a literal `facing`/`axis`/connection/`rotation` state lands however the scope was turned; the intended mechanism is `Cond::Orientation` — one alternative per orientation, each carrying the matching state — and this fires when a fill skips it. Checked during expansion (where scope orientations exist), sensitivity derived from the registry's own value vocabulary (`BlockRegistry::oriented_mismatch`; `rotation`/`hinge`/non-`straight` `shape` are the documented residue). Surfaces as a red `oriented-fills` gate (with fills-examined binding counts) and an export refusal. A passed guard licenses a fill only while the orientation it asserted still holds — a reorientation below the guard voids it. |
