...
- **`delvec prefab seating --horizon <base>`** answers the pairing over a whole
  ...
  denominator at pool, member and declaration level; `delvec prefab audit
  <library dir>` sweeps `DW0887` over the same library. Both run
  `compiler::seating`, the implementation the compiler's own validation runs, so
  a library can never be seatable according to the tool and refused by the
  build.
