...
- The compiler library is `crates/compiler` (Rust-native, ADR-0011); the binary
  is `crates/delvec`. The other surfaces of the same binary — `delvec schem`,
...
- Versions (as of this doc): `delvec 1.2.0`, `dsl 0.21.0`, `mc 1.21.11`.
  ...
  not prose: it is bound by equality to `crates/compiler/Cargo.toml`,
  `crates/dsl/src/envelope.rs` (`DSL_VERSION`) and `versions.toml` by
