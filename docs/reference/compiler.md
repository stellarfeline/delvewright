...
| `DW0736` | `delve-grammar` | An **orientation-sensitive block state filled into a reoriented scope with no `orientation` guard**. A grammar reorientation permutes geometry and never rewrites block-state properties (`crates/grammar/src/orient.rs`), so a **world-frame** literal `facing`/`axis`/connection/`rotation` state lands however the scope was turned. Two mechanisms answer it: write the state in the scope's own axis frame (`Paint::Local`, resolved at fill time — one binding, every orientation) or pin the orientation with `Cond::Orientation` and write one alternative per orientation. This fires when a fill uses neither. Checked during expansion (where scope orientations exist), sensitivity derived from the registry's own value vocabulary (`BlockRegistry::oriented_mismatch`). Surfaces as a red `oriented-fills` gate — whose detail states fills examined, fills carrying properties, and how many of those were resolved out of the local frame — plus an export refusal. A passed guard licenses a fill only while the orientation it asserted still holds: a reorientation below the guard voids it. |
...
| `DW0738` | `delve-grammar` | A block state written in the **scope's own axis frame** (`Paint::Local`) carries a property the pinned vocabulary cannot map onto the world axes the scope was given. A permutation of three axes that keeps the local `Y` on the world `Y` is either the identity or the horizontal transposition `x↔z`, and the transposition is a *reflection*: a direction moves by key or by value, an `axis` moves, a 16-step `rotation` becomes `(12 − r) mod 16`, a handedness (`hinge`, a stair's `shape`, a double chest's `type`) swaps left for right. A permutation that moves the **vertical** has no image for any of those — `half=top` cannot mean the top half of a horizontal axis — and neither has a horizontal connection flag turned onto a block with no `up` key, nor a rail's direction-composed `shape`. Refused at expansion, naming the state, the property and the orientation; the fill never writes a substitute. Rule lives in `delvewright_schem::blocks::BlockRegistry::permuted_properties` — the same classifier `DW0736` judges with, so a state one of them calls wrong is never one the other quietly rewrites. Fix: keep the scope's vertical on the world's, or write the state in the world frame under an `orientation` guard. |
