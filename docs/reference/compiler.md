...
- **Own the gate you assert on (`DW0807`, `compiler::batchstate`).** The rule
  ...
  by the ones it only *reads*. A template that runs the campaign's real `tick` and
  asserts on a gated outcome must therefore WRITE every `#party` term that gate
  reads — `requires_flags`, `forbids_flags` and `requires_state` alike, which is
  why the terms are driven from the whole `Gate` through `packtest_gate_drive`
  rather than listed beside each template. The campaign-playthrough template is
  the reason it cannot be left to convention: it drives the real campaign,
  `schedule`s its next phase twenty ticks out and `await`s, so the party ledger it
  leaves behind is live for the rest of the run, and a template that read a term
  it did not own simply inherited it. That is decided at build time over the
  shipped bytes, not by a convention a future template can be written without.
...
| `DW0806` | `delve-grammar audit` | **A composed part's own manifest row names a different box from the one the composition allocates it.** A campaign's zones are designed apart and assembled afterwards, so "how big is this part" is stated twice: once by the part's `zones.json` row, which is the extent it is developed, expanded and reviewed at standalone, and once by the map program's own site plan, whose `split` hands the composed prefix a box. Both documents are internally consistent, both build, and every other gate is green — while the box the part was reviewed in is not the box it is built in, so the review certified a different object. Measured cost: a composed citadel at 1 : 5.5 against a brief asking for a compact stepped mass, its crown subtending 6.7° from the arrival point against a derived 27°, because the map's region had been set to the arithmetic sum of the parts' pre-existing depths — extent flowing UP, from parts to whole. **Extents, never origins**: a part developed standalone sits wherever it likes and where it was parked says nothing about how big it is. **In the part's own frame**, so a composition that turns a part a quarter-turn compares the turned extents and stays green; comparing world extents would red a correct composition for having rotated something. Every distinct box a prefix is entered at is compared, and a prefix that is composed and never entered is the same red with the strongest detail — the row was compared against nothing. Binding: composed parts that also have a manifest row, stated in the audit summary as `part-allocation bound N` whether or not it is zero, with a zero named as the corpus fact it is. Prescription: revise the part to its allocation, or revise the site plan deliberately so the whole's own identities re-run over it — never grow the container to what the parts sum to, and there is no per-part exemption. |
| `DW0807` | `compiler::emit` | **A generated PackTest template asserts an outcome whose gate reads `#party` state the template never writes.** The suite runs as ONE batch on ONE shared server (`compiler::batchstate`), and `#party` is batch-global progression state (spec-0018), so the contract is *own dummy, own scores, own init*. A term a template only READS is decided by whichever sibling ran last — and one template is not atomic: the campaign-playthrough template drives the real campaign, `schedule`s its next phase twenty ticks out and `await`s, so for those ticks the party ledger holds whatever the campaign's own completion functions wrote, and the last phase's leavings persist to the end of the run. The worked instance: the gallery's `trigger/skip-the-label` forbids `flag/hall-sealed`, `complete_q_far_hall` SETS that flag and nothing clears it, so `v04_strike_npc` passed or failed on nothing but whether it ran before or after — the SAME BYTES producing both verdicts, which is why byte-identity against the baseline could not exonerate the emission. Such a template does not fail; it lands green most of the time, which is the shape that gets re-run instead of read. Judged from the shipped bytes and feature-blind, so it guards templates not yet written. Build tier (exit 3). **Scope**: `#party` only (other global sentinels are reached through the machinery that owns them, so demanding a literal write would red correct templates); only templates that invoke the campaign's real `tick`; and only the gates that actually reach an assertion. **Binding**: `BatchStateBinding { templates, ticking, judged }`; a suite whose templates tick but whose assertions resolve to no gated dispatch reports the zero binding as a `DW0807` warning rather than a clean pass over an empty comparison. Prescription: drive the whole gate through `packtest_gate_drive`, which takes a `Gate` and so covers `requires_flags`, `forbids_flags` and `requires_state` together. Never hand-roll the terms beside the template — three sites did, each dropping a different axis, which is how this arrived — and never widen the template to a full progression baseline, which would reset the scores the campaign template is awaiting and trade one order-dependent verdict for another. |
| `DW0809` | `delve-grammar audit` | **A binding count was taken over fewer objects than were offered to it, and its zero was read as a fact about the corpus.** The vacuity modes `CLAUDE.md` enumerates are about a gate that binds nothing; this is the mode one step further in — the *reading* a binding line gives its own zero. A line that says `bound 0 — nothing here to examine` is not reporting a number, it is claiming the corpus held nothing, and that claim is true only when nothing was offered. Let something upstream refuse and the identical zero arrives carrying the identical sentence, now describing what stopped as though it were what exists — and it fails in the reassuring direction, the one direction a reader does not re-check. Measured cost: the `part-allocation` line of `DW0806` printed `bound 0 — nothing in this corpus composes a part that also has its own row` over the first corpus to adopt spec-0040 §3c, one line under an expansion error naming the program that composes eight such parts; every one of the eight rows was offered and none compared, because a part in debt to its allocation refuses for itself, so nothing expands, so nothing is compared, so the count is zero. The guard for it existed that day and was ordered BEHIND the zero test, which made it unreachable at the single value it was written for. **The repair is never to redefine what counts** — the objects genuinely were not examined — nor to compare against a box produced by the expansion that failed, which would be a measurement whose calibration is the thing that broke. It is to say so. Bound at both levels the audit reports: rows offered versus rows compared, and programs offered versus programs expanded, the second qualifying `local-frame` and every per-gate total below it, since all of them are summed over the programs that expanded and a surface only a refused program would have bound goes missing from the table rather than reading zero in it. Structural rather than ordered: the three readings of a count (shortfall, honest zero, ordinary total) are decided once inside `Binding::note`, with the shortfall tested first, so a call site hands over both numbers and cannot express the wrong order — a swapped pair of branches is correct today and silent the next time a binding line is added. |
