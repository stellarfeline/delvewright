...
- The compiler is the module `delvec::compiler` of the one package
  ...
  [`tools.md`](tools.md).
- Versions (as of this doc): `delvec 1.2.0`, `dsl 0.21.1`, `mc 1.21.11`.
...
- **Sea-level datum (ocean).** An ocean world places its areas at **y=60**
  ...
  simply follows the new Y. The world ocean is **not** backdrop: `/place template`
  carries the fluid already in a cell onto the block it writes there, so a
  waterloggable block placed below the sea plane comes out `waterlogged=true`
  whatever the prefab said, and a waterlogged cell is a water source. That is what
  `DW0851` measures; the walk plane one block above the waterline is what keeps a
  shore clear of it.
- Prefab metadata may declare **`waterline_y`** (optional, integer, local y of
  ...
  Consumed only by `DW0344`, and **a piece that stands in the sea without it is
  refused**: any placed piece of a `horizon: ocean` world whose box reaches at or
  below the sea plane (y=62) and declares no `waterline_y` is a build-tier
  refusal, because a check keyed off an optional field goes quiet when the field
  goes missing and the two states — deleted, and never needed — are
  indistinguishable in the declaration. The refusal names three moves: declare the
  waterline the piece really authors, raise the piece clear of the sea (an ocean
  area's origin is y=60, so lower courses solid to local y=2 put the walk plane at
  y=63, one block above the water — the island convention read forwards), or
  choose a horizon that is not `ocean`. Declaring a waterline for water the piece
  does not author is not a fourth move: the gate would then be reading a fiction.
  The binding count — pieces placed, pieces checked, pieces whose box reaches the
  sea plane — is printed on every build; a zero binding is now only reachable by a
  world whose pieces all stand clear of the water, which is geometry and not a
  declaration.
...
| `DW0344` | **A piece that stands in the sea answers for where it meets it.** Two arms, one code, `compiler::plan` after placement, build-tier (exit 3). *The placement arm*: in a `horizon: ocean` world a placed piece whose prefab metadata declares `waterline_y` must land that waterline at sea level (`piece.y + waterline_y = 62`) — otherwise the piece floats above the sea (its shore an unclimbable cliff, its authored water pocket hanging in the air) or is drowned under it. The message names the area, prefab, placed y and the signed offset. *The declaration arm*: a placed piece whose box reaches at or below the sea plane and declares NO `waterline_y` is refused, because nothing then states where that piece meets the sea while every downstream proof — nav, boundary, POV, PackTest — derives from the placement none of them checked. The refusal owes the author a move and names three: **declare** the waterline the piece really authors (the local y of its top water block; the island convention is 2, walk plane one above); **raise** the piece clear of the sea — an ocean area's origin is y=60, so lower courses solid to local y=2 stand the walk plane at y=63; or **choose** a horizon that is not `ocean`, which is byte-identical and has no sea to fall into. Declaring a waterline for water the piece does not author is refused as a fiction rather than offered as a fourth move. **Why it is a refusal.** The invariant is keyed off an optional field, so a declaration that was DELETED looks exactly like one that was never needed — and that is a real path: an admission step that read prefab metadata through a type not modelling the field deleted it on write from all five island prefabs that carry it. It was a zero-binding WARNING while the demand was held to be unauthorable, and a two-scene delve then shipped green with both of its objectives standing in shin-deep sea, this diagnostic printing the words 'is still standing in the water at y=60' beside the build. The lever was never the datum, it is the piece. **Binding**: the count — pieces placed, pieces checked, pieces whose box reaches the sea plane — is printed on every build (`waterline binding: …`), ocean or not, and a non-ocean world says so in as many words. A zero binding is now reachable only by a world whose pieces all stand clear of the water, which is geometry a missing declaration cannot fake. |
