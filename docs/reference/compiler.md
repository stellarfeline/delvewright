...
| `DW0780` | `delve-render` | A prefab blockstate has **no definition in the asset source** it was coloured from (`viewer` / `palette`): the id does not exist at the pinned version, or its model or textures are absent. A **warning**, with the cell count and the offending state — the reviewer still needs the rest of the building, and the block is drawn in the missing-texture magenta and listed on the page rather than silently coloured as if it had resolved. This is not only a rendering finding: an id the client jar does not have is an id the pinned server does not have either. `minecraft:chain` is the live instance — 1.21.11 renamed it `minecraft:iron_chain`, and the prefabs still naming the old id place a block that does not exist. |
