...
**Which bodies get an answer, and where the default is baked.** One rule, one
site: *a sealed body the campaign never answers is answered by the compiler.* The
bodies are `close-gate` seals and sealed `shortcut` doors, in that order; "the
campaign answers it" is any `use` trigger the author anchored there, and when
there is one the compiler supplies nothing — the engine does not talk over the
campaign. A third pressable object class gets an answer by joining that list, not
by growing a field. The wording is the anchor's `sealed_hint` where one is
authored (`close-gate` only), otherwise the compiler's
`delvewright.ui.gate.sealed` chrome; a `shortcut` has **no** wording field and
does not get one, because writing your own line is what the trigger is for.
...
**Wording.** `sealed_hint` is optional and is now **sugar, not machinery**: it is
the wording of the press answer the compiler synthesizes for that gate, and
nothing else. Unauthored, the line is the `delvewright.ui.gate.sealed` chrome
