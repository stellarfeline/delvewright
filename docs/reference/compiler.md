...
- **Entry point.** One cell per **area** — the cell a body arrives at when it
  ...
  a grammar program (`grammar.md` §2b), `delve-admit anchor --role entry` for a
  ...
  `delve-admit anchor --no-role` (or dropping `role` from the `mark`) on the one
