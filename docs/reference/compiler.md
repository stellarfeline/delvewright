...
| `DW0364` | In a horizon with a **flood level** (ocean: 62), a placed piece has a standable cell at or below it — the assembled world floods the walk surface on boot. The empirical restatement of the #149 class (spec-0026 §2): unlike `DW0344`, there is **no exemption** for pieces that declare no `waterline_y` — the check reads assembled geometry (`nav::check_flood_level`), never metadata, so an interior piece placed onto the wrong datum can no longer ship green and flood live. The message names the area, prefab, placed y and the datum-equation term (`walk_ref_y − walk_y`) that disagreed. Build-tier (exit 3), `compiler::emit` (proof in `compiler::nav`), run off the edited model when a stage-7 script exists. `DW0344` is retained for waterline-declaring pieces (shore mating at sea level). |
| `DW0366` | A horizon **parameter is out of its spec-0026 range, or foreign to the declared base**: `ratio` outside 2..=3, a non-positive `rim_height`, summit `min_drop` < 100 or `plateau_y` overflowing the −64..320 build range after the gorge drop, sky `float_y` outside the build range — or a param present on a base it does not belong to (e.g. `flora` on `flatland`). Validation-tier (exit 1), `delvewright_dsl::validate` (`codes::HORIZON_PARAM`); the build path re-states the range fence with the same code when generation itself would leave the build range (`compiler::plan::build_valley_surround`). One code, one rule: params belong to their base and to their pinned ranges. |
| `DW0367` | A piece placed under a **non-void horizon** has no `walk_y` in its prefab metadata (or a pool's entry members disagree about it), so the per-area datum `walk_ref_y − walk_y` (spec-0026 §2, superseding the global `OCEAN_BASE_Y`) has nothing to compute from — and guessing a tileset walk-plane convention is exactly how the #149 flooded-interior class happened. Build-tier (exit 3), `compiler::plan` (`DW_WALK_Y_MISSING`). Prescription: declare the local y a player's feet occupy in the piece's metadata (the generators emit it; island 3, keep 1, cave 2 — verified empirically from the `.nbt` geometry, see the content repo's walk_y backfill). |
