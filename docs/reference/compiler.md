...
| `DW0734` | `delve-admit` | Audit: a palette block state written **at** DataVersion 4671 leaves properties unwritten. The state is legal — vanilla fills the rest from the block's default state — but the file then means something only a running 1.21.11 server can work out, and every other reader has to guess. The one that guessed was the prefab viewer: given a `minecraft:cobblestone_wall` with no properties it could satisfy no `multipart` case, unioned every case (which is a full cube), drew a solid block where a wall post stands, and reported `0 unresolved`. **Bound to the pin only**: a pre-pin template's palette is upgraded by vanilla's DataFixerUpper on load and this registry describes 1.21.11 alone, so it cannot say what a 1.18.2 entry left out. Prescription: write the property at the value the message names — `BlockRegistry::complete` / `invariants::complete_state` produce it, and because completion never overrides a written value the BlockState is unchanged. |
