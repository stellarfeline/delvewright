...
| `schema --stage <n\|name\|all>` | export a stage's JSON Schema | `--stage` (required): `1`..`7` for the numbered campaign stages, `geometry-brief` or `layout-graph` for the map-pipeline documents (named, never numbered into that sequence — a number would assert an ordering between two pipelines that have none), `all` for every one of them |
