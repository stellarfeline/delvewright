...
| `tools/check-skill-version.py` | CI | `python3 tools/check-skill-version.py` — ADR-0016's **third version line**, made true. `.claude/skills/new-delve/SKILL.md`'s frontmatter declares the skill's own product version (`version:`) and the engine range it drives (`requires: delvec: ">=X.Y.Z <A.0.0"`); this gate is what stops that being a `requires:` nobody reads. Three bindings. (1) The range's floor must EQUAL `crates/compiler/Cargo.toml`'s `[package] version` — the single source `DELVEC_VERSION` derives from — in **both** directions: a floor above names a compiler that does not exist, a floor below claims the workflow still runs on releases that are not in this tree to be exercised, and the only engine any job here can drive the skill against is this one. The floor is therefore a *verified* floor, restamped in the engine's own release commit; that is the RANGE moving, never the product version (ADR-0016 keeps the two independent so engine fixes never bump the skill). (2) The ceiling is the floor's next major, and this repo's engine sits inside the window — the direction that catches `delvec 2.0.0` shipping beside a skill that still says `<2.0.0`. (3) Every `delvec` subcommand the skill's code spans name, and every long flag named with it, must exist in the clap CLI parsed out of `crates/compiler/src/main.rs` (nested `edit apply`/`preview` actions fold into their parent, so `delvec apply` is correctly not a subcommand). A version range is a claim about a command surface, so the command surface is what it binds to. States its binding count on every run — currently 9 distinct subcommands, i.e. all of them; **extracting zero subcommand references is a red**, as is parsing zero subcommands out of `main.rs`, because a green that binds to nothing is vacuous (CLAUDE.md). Runs as a step of the `docs (local link check)` job — a step, not a job, since every job name is a required status context |
