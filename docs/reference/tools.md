...
| `tools/gallery-baseline.py` | CI + agent | `python3 tools/gallery-baseline.py --prefabs <generated> [--write]` — the gallery's **emission baseline and expected-warnings ledger** (spec-0039 §5). Builds the whole domain (the primary in every declared language, plus each overlay in `en`) and compares each build's `manifest.json` to the copy committed under `gallery/baseline/`. Each point is materialised by `tools/gallery_domain.py`, the one authority on what a build point is, and compiled in a temp directory this tool deletes — it keeps a hash, not a world, and `tools/gallery-build.py` is what writes a point to disk when a tree is what is wanted. The comparison asserts the **header** first — delvec version, `dsl_version`, gallery source hash, generator-input hash — and refuses with its own message on a mismatch instead of diffing noise, because a baseline taken over different inputs is a measurement of two different things. A manifest mismatch is classed by what the diff touches: **emission change** when it touches `gallery/` or `crates/`, **determinism finding** (ADR-0006) when it touches neither, which makes the baseline a standing cross-machine determinism probe for free. The emitted warning set must equal the committed ledger exactly, so "still green" can never absorb "warns differently now". `--write` regenerates all of it plus `delta.json`, the review artifact listing every added/removed/changed path grouped by output class; rewriting with an empty delta is a red. States builds compared, paths compared and warning rows checked; **zero of any is a red** |
