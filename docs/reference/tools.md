...
## 2a. `delve-grammar` — grammar back end + sweep driver (`crates/grammar`, package `delvewright-grammar`) · agent

Expands box-split grammar programs (spec-0027) and drives the **sweep** that
feeds the owner's contact sheet — step 3 of the §3 authoring loop, and the step
that used to be assembled by hand. Semantics live in
[`grammar.md`](grammar.md). Needs no GPU and no client jar.

```
delve-grammar list                       # every program with a design box, and its box

delve-grammar sweep -o <dir>
    --program <id> --seeds 1,2,3         # the quick look: one candidate per seed
    --manifest <sweep.json>              # the real sweep: per-candidate seed/region/params
    [--region XxYxZ]                     # override the program's design box
...
block) and distinct **massings** (the solid/air bitmap alone — the same building
in different stone counts once). Computed from the models, never from the
pictures, because two renders can differ by a shadow and two identical renders
can hide a moved wall behind a roof. A sweep whose `distinct_massings` is 1 is a
**finding**: every cell of the page is one building, and the curation gate in
...
dimensions and only consults the RNG when two alternatives apply at once. A
refused candidate keeps its row in the report (a guard refusing is a fact about
the program); an override naming a parameter the program does not declare is an
error, because a silently-dropped override is a candidate that reads as varied
and renders as a duplicate.

**A refusal prints its whole reading, and the report carries it as data.** On
stderr a refused candidate states the box it was handed with the rule's own
axis mapping, then every clause of every alternative's guard with both sides
measured, and for the clause that decided it the distance to satisfaction from
both sides — so the next candidate is a deduction rather than a guess
([`grammar.md` §4](grammar.md)). The same thing sits on the row as `refusal`,
beside the prose `error`, so a driver can rank or group refusals without parsing
a sentence; a candidate that failed for a reason that is not a guard declining
has `error` alone.
...
| `validation/bot-run.sh` | agent (**the bot ladder entry**) | `EULA=TRUE validation/bot-run.sh --project dw-<id> [--output <tree>] [--run-out <dir>]` — the `validate` profile end to end: fresh-volumes the project, boot server + mineflayer bot, propagate the bot's exit code, tear the project down and prove it clean. `--project` is REQUIRED (task #185): the compose project is the only name the stack has now, so a missing id would land in compose's default project — a shared name by another route, and the collision that made ladders queue. Every `DELVEWRIGHT_*` run variable below is forwarded (task #68 closed the four `compose.yaml` used to drop), so all of them can be set on this command line. `--output` selects the build tree and now really does: before task #68 it set `DELVE_OUTPUT`, which only the `packtest` profile read, so the bot ladder silently booted `./delve-output` whatever it was asked for. The delve image tag follows the project (`delvewright/delve:<project>`) for the same reason the compose project does — an image tag is global to the daemon, so two ladders building different trees into one tag race and the loser boots the other's delve. The run report lands in `validation/run-out/<project>/run-report.json` (project-scoped so two ladders from one checkout cannot overwrite each other's) |
...
Four used to be dropped — `DELVEWRIGHT_ACTOR_FLOOR`, `DELVEWRIGHT_RETRY_ON_DEATH`,
`DELVEWRIGHT_CUTSCENE_GRACE_MS` and `DELVEWRIGHT_ENTITY_SETTLE_TIMEOUT_MS`: no
`validation/*.yaml` declared them, so they arrived unset inside the container and
setting them on a command line was SILENTLY DROPPED (the same container-boundary
defect task #102 fixed for `DELVEWRIGHT_RUN_TIMEOUT_MS`). Closed by task #68;
`compose.yaml` now names them in the `bot` service's pass-through form (`VAR:` with
