...
| `validation/check-versions.sh` | CI (tier 1) | fails if any Dockerfile/compose/workflow disagrees with `versions.toml`. It is also the registered `bound_by` of the `rust-toolchain` pin: it reads `[engine].rust_toolchain` and holds the channel, both published `rust-version` floors and the manifest equal, which is what covers the sites no shape scan can discover. `check-pins.py` verifies that binding, so dropping the key or a consumer path from this script reds the pin registry too |
