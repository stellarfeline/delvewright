...
| `schema --stage <n\|name\|all>` | export a stage's JSON Schema | `--stage` (required): `1`..`7` for the numbered campaign stages, `geometry-brief`, `layout-graph` or `site-plan` for the map-pipeline documents (named, never numbered into that sequence — a number would assert an ordering between two pipelines that have none), `all` for every one of them |
...
**`delvec allocation <place>` / `--all` is what the whole hands a part.** It
prints, as JSON on stdout, everything a creator needs to build the piece that
fills one of a site plan's places: the frame's extents (the box's play space plus
...
is the whole of what it holds, and the swept set is derived from `git ls-files`
rather than listed — `python3 tools/check-json-canonical.py`, one command on a
...
**The score RANKS the page; it NEVER gates it** (spec-0028 §3).
...
- `DW0790` — **a blockstate the pinned version does not have**: the id, its
  ...
  `minecraft:chain` was found (the pinned game holds `minecraft:iron_chain`): the
...
| `tools/extract-block-renames.py` | agent (rare) | `python3 tools/extract-block-renames.py crates/dsl/data/block-renames-1.21.11.json [--cache-dir DIR]` — derives the block-id **renames** the pinned game's DataFixerUpper applies on load, so a check that judges a NAME can judge the id a 1.21.11 server will actually hold rather than the one a pre-pin template spells. Which ids left the registry and when comes from each release's own `registries/data.min.json`; what each became comes from the crafting recipe whose ingredient side is unchanged across the version step. A removal the recipe graph cannot pair is reported and LEFT OUT, which fails closed — the audit still refuses the id. Reads the `misode/mcmeta` summaries directly (one branch per release, cached between runs) and refuses to run unless the newest release at or below DataVersion 4671 is the pin; see `crates/compiler/data/PROVENANCE.md`. Only ever run when ADR-0009's revisit triggers fire |
| `tools/extract-block-registry.py` | agent (rare) | `python3 tools/extract-block-registry.py <blocks/data.min.json> crates/dsl/data/blocks-1.21.11.json` — regenerate the pinned 1.21.11 block-state registry from a `misode/mcmeta` summary. Pins and checks the source SHA-256 and the block count; see `crates/compiler/data/PROVENANCE.md`. Only ever run when ADR-0009's revisit triggers fire |
...
| `tools/build-every-campaign.py` | CI + agent (run it before proposing any engine change that touches emission, layout or validation) | `python3 tools/build-every-campaign.py --delvec <binary> [--content <checkout>]` — builds **every campaign** the pinned content repo carries, in **every language its `world.json` declares**, and reds if one stops building. Closes the gap that let a change reach 10/10 green while stopping the flagship released campaign `nobodys-cave-island` from building at all (26 × `DW0364`): every other gate builds a FIXTURE, and a fixture exercises one verb, where a campaign is the only place the verbs meet a real prefab library, a real layout solve and a real translation sidecar. Campaigns are **discovered** (any dir under `<content>/campaigns/` with a `world.json`), never listed, so the next content re-pin gates a new campaign with nobody remembering. `--delvec` is required and never inferred — the gate's whole subject is *which engine* built the campaign. A campaign that cannot build today goes in `.github/campaign-build-exclusions.toml`, which **inverts** the assertion rather than removing it: still built, must still fail, and must fail with **exactly** the recorded `expect_codes` — an extra code is a new break that was hiding behind the exclusion, and a SUCCESS is an expired exclusion, both red — so an entry is deleted by the same change that moves the content pin onto the fix. States its binding count every run (discovered / built green / known-red, each named); discovering zero campaigns, building zero campaigns, or an exclusion naming a campaign that no longer exists are each a red. Runs in CI as `campaign builds (every campaign in the content repo)`, on every push |
| `tools/staging-gate.py` | agent (**mandatory** before any build is handed to the owner) | `python3 tools/staging-gate.py --campaign <dir> --build <delvec-out> [--ledger docs/playtest-findings.json] [--report R.md] [--json R.json] [--strict]` — **the coverage gate on the findings ledger**, and the only tool that asks a question the ladder cannot: for every defect the owner has EVER reported, on any campaign, does a general-form check exist and does it BIND — non-zero — on the build about to be staged. It re-runs nothing; it reads `docs/playtest-findings.json` and reports one row per finding (finding → general form → the check carrying it → that check's binding count here → verdict). Six reds, each a way a green has really lied on this project: `NO-GENERAL-FORM` (the instance was fixed, the class never built), `MISSING-CHECK` (the ledger names a check this engine no longer has — not in source, undocumented, or asserted by no test — or names a stage document the COMPILER read that this gate holds no parsed copy of), `UNBOUND` (matched zero objects), `INAPPLICABLE` (zero binding AND zero precondition — this campaign cannot exercise the class at all), `UNFENCED` (the campaign's `dsl_version` never reached the surface the check keys off), `NO-SOURCE` (the campaign has no stage JSON, so nothing can be measured — the bell remake's state today, and never a pass). Two non-reds. `DECLARED-UNCOVERABLE` is playtest-methodology.md rule 2's escape: a `disposition` AND a substantive `justification`, counted in the headline because rule 4 makes each one a risk item at that staging review. `OUT-OF-STAGE` is not an escape but a different subject — a **pre-detail blockout** (spec-0049: the campaign places by site plan, the build's compiler-written manifest was compiled from it, and no `detail-plan.json` exists in either; any disagreement fails closed) on which the row's class measured a DOUBLE zero: zero binding, zero precondition (`applies_when` where declared, or the probe's own shape where its predicate selects the class by identity — `eq`/`in`/`prefix` only; a `has`/`has_any`/`artifact`/`out` zero without a declared probe stays `UNBOUND`, blockout or not). A `dsl` probe over an **optional** stage document the campaign does not ship counts zero rather than answering "I could not look", and the fact that settles it is the compiler's `manifest.json` `inputs` — never the filesystem, since a directory or an unreadable file standing where a document belongs is what a broken campaign looks like on disk, and `load::optional` makes such a campaign fail to build so it has no manifest to present. A document IN `inputs` this gate cannot parse is `MISSING-CHECK`; a build that cannot say restores it too. The guard that keeps this honest is a load-time refusal of any `dsl` `binding`/`applies_when` naming a document outside the gate's stage-file vocabulary, since a mistyped name is in no manifest and would measure zero on every campaign forever. The walk cannot exercise a class this build contains zero objects of, and the build does not claim to be the build that could; every such row is named in the token, announced by the boot banner, and re-adjudicated as a red the moment the campaign gains a detail-plan document. Nothing an operator types reaches this verdict. On a pass it mints an **admission token** (`<build>/staging-admission.json`) binding the sha256 of that tree's `manifest.json`; a refusal DELETES any existing token. `--stage-anyway "<reason>" --acknowledge-red <N>` is the deliberate override — N must equal the current red count exactly, so it cannot be typed from memory. **Invoked by the staging surface, not by a doc line**: `tools/playtest-server.sh` runs it between build and container, `validation/owner-play.yaml` requires its token via the `staging-admission` service, and `release.yml` runs it before the GHCR push. It shipped called by nothing but its own tests — the UNRUN shape — and `tools/tests/test_staging_gate.py` now carries a tripwire against that returning. **Not a CI status check, deliberately** — it is red today by design (18 rows on `nobodys-cave-island`), and wiring an honest red list as a blocking context would force the one thing CLAUDE.md forbids: weakening it to get green. Its own falsification suite (every verdict driven red then green, plus the token round-trip through the real verifier) IS in CI. |
| `tools/check-json-canonical.py` | CI + agent | `python3 tools/check-json-canonical.py [--delvec <binary>]` — **every JSON document this repository holds is in `delvec fmt` canonical form, over a set that is DERIVED rather than listed.** The population is `git ls-files -z -- '*.json'`; the whole of it goes to `fmt --check`. This replaces two roots named by hand in `ci.yml`, which is the enumeration-somebody-remembered shape: they covered 240 of the 335 tracked JSON files, and 50 of the 95 nobody looked at were not canonical — every one of `gallery/`'s among them, while spec-0039 criterion 16 asserted in writing that gallery JSON was inside the sweep. Nothing was red because nothing looked. Taking the population from git is also what makes the exclusions **properties instead of names**: `target/`, `node_modules/` and every build product are absent because they are untracked, `campaigns/` contributes one symlink entry rather than a second repository, and `delvec build` output trees are absent for the same reason `BUILD_OUTPUT_MARKER` skips them — none of which is a rule that can go stale. There is exactly **one** exemption and it is a POINTER rather than a judgement: `crates/compiler/tests/golden/`, pinned byte-for-byte to emitter output by `golden_scene_matches` with its membership closed by `every_golden_is_emitter_output`. That is an opt-out the defect cannot supply (CLAUDE.md, the sixth vacuity mode) — the defect is *nobody canonicalised this*, and getting a document in there means making the emitter actually emit its bytes; a marker file or a name in a list would have been openable in one line by exactly the author who forgot. Generated JSON is otherwise not exempt but made canonical **at the writer** (`tools/gallery-baseline.py` writes `ensure_ascii=False`), and a foreign program's file — npm's `package.json`/`package-lock.json`, `.claude/settings.json` — is formatted anyway and re-formatted after that program rewrites it, the same relationship `cargo fmt` has with hand-written Rust. States files swept **against the tracked-JSON population** plus the exempt count every run; a population of zero, a swept set of zero, an exemption matching zero tracked files, or a deleted pin are all refusals rather than passes. Bound as a step of `rust (fmt, clippy, test)` — a step, not a job, since every job name is a required status context |
...
| `tools/check-gallery-coverage.py` | CI | `python3 tools/check-gallery-coverage.py --prefabs <generated> [--build-out <tree>] [--report x.json] [--index x.md]` — **every surface the DSL declares is written somewhere in the gallery** (spec-0039). The unit set comes solely from `delvec schema --stage all` of the delvec in the tree under test; the tool contains no parser of `stages.rs`, because a second enumeration is a second authority and the two disagree exactly when it matters. A **surface unit** is every named schema property and every enum/tagged-union variant, recursively, across every stage the export holds — the seven numbered campaign stages and the four named map-pipeline documents; registry values (block/item/sound ids) are data and are never units, which needs no filter because the DSL models every registry id as a transparent string newtype. Each overlay and probe is materialised by `tools/gallery_domain.py`, so the tree this gate validates is byte for byte the tree `gallery-baseline.py` compiles and `gallery-build.py` writes; it used to answer that question itself, slightly differently. The walk over the gallery's authored JSON is **schema-guided, never grep** — a string that merely spells a variant name binds nothing. Every unit is either **bound** in the primary+overlay domain, or **refusal-proven** by a committed probe under `gallery/probes/` that `delvec validate` rejects with the ledgered code; there is no third kind and no free-text exemption, which is what makes the escape hatch demand a machine-produced refusal the defect itself cannot supply. A probe may also declare **no unit**, and it is then a **refusal demonstration** rather than an exemption: some rules are about a COMBINATION of surfaces that are each perfectly writable — a campaign carrying both `areas[]` and a site plan is refused while every unit involved is bound — so there is nothing for such a probe to discharge, and demanding a unit would force it to claim one the domain already binds (which this tool refuses by name, correctly) or leave the rule with no committed artifact at all. It is held to the same two obligations as any other probe — refused, and refused with the code it names — and is counted on its own line, never folded into the refusal-proven total. Which kind a probe is follows from what it claims and is never a flag, and the choice buys an author nothing: the discharge is the ONLY thing an exemption adds, so emptying `units` can only move a unit back into the unaccounted list. Every probe names a `DW` code and a `why` — the reason `--index` renders beside it — and the tool refuses one that names neither, because a probe a creator cannot read has met half its obligation. The contract lives in `probe_kind` / `assert_refused` / `probe_discharges` and the tests DRIVE those rather than restating them; the restatement is what drifted the last time it existed twice. Each overlay declares a non-empty `binds` set and the tool reds both ways — a unit it claims and does not bind, and a unit the primary already binds (a redundant overlay). `--build-out` carries the build's own `validation/*.json` binding ledgers into the report, including the `effect-roots.json` added with it, and **reds naming any ledger whose stated binding is zero** — the gallery declares everything, so a proof that examined nothing there has stopped reaching what the document plainly writes. `--index` writes the reader-facing map from every surface to the place it is written, and **nowhere** for anything in neither state. States its binding count every run — units enumerated / bound / refusal-proven, plus documents walked; **enumerating zero units is a red**. Job `gallery (coverage + build + baseline)` |
| `tools/check-gallery-render.py` | CI + agent | `python3 tools/check-gallery-render.py --prefabs <generated> --build-out <tree> --frames <dir> [--write]` — **every declared view of the gallery is produced and shows something** (spec-0039 §7). CI rendered nothing before this. Four assertions, each one a fact a machine can check and a reviewer cannot, across thirty-odd frames: the committed view set (`gallery/render-plan.json`) equals the set the build emits, so a shot that VANISHES is a red naming it rather than a shorter directory listing (`DW0721` semantics); every declared view is produced or refused LOUDLY, never merely absent; every produced frame is non-featureless by `detect::is_featureless` — the engine's own answer, computed by the arm that drew the frame and read back from its manifest rather than recomputed here, because a second verdict on one question is a second authority; and every view manifest binds a non-zero target count (`DW0726` discipline), since a camera that framed nothing declared checks nothing by having been taken. Uses the **CPU arms only** (`delvec snapshot`) — no GPU, no client jar, no `nucleation` — so it runs on an ordinary runner. **No pixel is committed or compared**: renderer output is not covered by ADR-0006 across drivers, so the manifests are the machine truth and the frames go up as a CI artifact for eyes. `--write` regenerates the committed view set. States views declared / produced / refused / judged; **declaring zero views or judging zero frames is a red**. On its first run over the gallery it found two: a POV frame of two distinct colours, and one binding zero targets — both cameras standing on wall-adjacent anchors and looking at the wall. Job `gallery (coverage + build + baseline)` |
| `tools/check-whole-map-render.py` | CI + agent | `python3 tools/check-whole-map-render.py --prefabs <generated> [--delvec <bin>] [--work <dir>]` — **the whole-map illustration path runs, on every horizon the engine can build**. `delvec scene` and `delvec panorama` turn the compiler's `render-plan.json` into Chunky scenes — the release illustration and the whole-scene review renders — and nothing in this repository invoked either of them; `check-gallery-render.py` runs `snapshot`, and only on the primary gallery. What that cost, measured: the render layer's `Horizon` enum carried ONE variant while the compiler had been emitting a second for as long as the base existed, and `#[serde(tag = "kind")]` has no fallback, so both commands refused every valley campaign outright with `DW0721 … unknown variant `valley`, expected `ocean``. **The population comes from the engine, not from this file**: the horizon bases are read from `delvec schema --stage all` — the same single authority the coverage gate enumerates units from — and every one must be exercised by a gallery point declaring it, so a base with no point is a red naming the base and there is no list here to go stale. Per point and per arm: exit 0 with the refusal reported verbatim otherwise, at least one scene file emitted, and **the frame contains the ground** — a scene's `chunkList` must cover the landform the horizon declares, because the whole-map camera and the chunk list are both solved from `layout_aabb` (the union of placed AREAS), which on a terrain-building horizon frames a box inside a landform several times its size and loads none of it. That defect renders successfully and produces a picture, which is why it needs an assertion rather than an eye. States bases declared / points found / builds exercised / scene files judged; **a base unaccounted for is a red, and judging zero scenes is a red**. Job `gallery (coverage + build + baseline)` |
| `tools/gallery-baseline.py` | CI + agent | `python3 tools/gallery-baseline.py --prefabs <generated> [--write]` — the gallery's **emission baseline and expected-warnings ledger** (spec-0039 §5). Builds the whole domain (the primary in every declared language, plus each overlay in `en`) and compares each build's `manifest.json` to the copy committed under `gallery/baseline/`. Each point is materialised by `tools/gallery_domain.py`, the one authority on what a build point is, and compiled in a temp directory this tool deletes — it keeps a hash, not a world, and `tools/gallery-build.py` is what writes a point to disk when a tree is what is wanted. The comparison asserts the **header** first — delvec version, `dsl_version`, gallery source hash, generator-input hash — and refuses with its own message on a mismatch instead of diffing noise, because a baseline taken over different inputs is a measurement of two different things. A manifest mismatch gets one of **three** verdicts, because a manifest is not its outputs: **a declared input moved** when no emitted path differs at all and the manifest differs anyway — `content_sha`, the pinned versions and the whole `inputs` index are siblings of `outputs`, so the emitted-path delta correctly reports nothing and `manifest_field_delta` names what did move; **emission change** when the diff touches an input that can reach emission (`gallery/`, `crates/`, `versions.toml`, which the compiler reads at build time and records into every manifest); **determinism finding** (ADR-0006) when it touches none of them, which makes the baseline a standing cross-machine determinism probe for free. Widening that set greens nothing — every verdict refuses — it decides only what the reader is told the finding IS. The emitted warning set must equal the committed ledger exactly, so "still green" can never absorb "warns differently now". `--write` regenerates all of it plus `delta.json`, the review artifact listing every added/removed/changed path grouped by output class. That artifact **names the commit it was measured from** (`base_commit`, the merge base of `--base`, default `origin/main`, with `HEAD`), and verify **recomputes it from that commit** and asserts equality — so a stale or hand-edited delta is a red, which is what spec-0039 §5 asks for and what nothing did. A delta is a statement about a transition and a directory records a state, so the other end has to be named: reading it out of git at a named commit is what makes the file checkable at all, and it is also why `delta.json` is still not a record of the tree. Two live defects fall out of the same repair. The basis used to be whatever sat on disk, so a second `--write` before committing silently re-based the artifact onto its own first output, and a write that moved only the header rewrote it to **empty** — destroying the record of the last real emission change with every gate green. And because `base_commit` is a commit on the base branch, it survives the squash that discards the branch, so the claim stays checkable on `main`. What is established is that the delta is true of the transition it DECLARES; choosing that transition is `--write`'s policy, and the base is asserted to be an ancestor of `HEAD` so it can never name a commit this history does not contain. **Both arms ask one question** — `baseline_matches`: does `gallery/baseline/` already hold, byte for byte, every document a write would PRODUCE (`RECORDED` + `DERIVED`, derived rather than listed)? Verify passes iff yes; `--write` refuses a noise commit iff yes. So no tree is refused by both, for any field a manifest gains and any document the writer gains. The arms used to ENUMERATE what to compare and had no satisfiable union for anything the enumeration had not met: a change moving only `versions.toml [content].sha` moved the recorded `content_sha`, which is in no header, no warning row and no output, so verify refused the tree as a determinism finding over zero differing paths and `--write` refused the regeneration it had just prescribed. Comparing `RECORDED` at the guard was that same shape one level up — it left the review artifact guarded by nothing, and would have made a delta-only defect unrepairable the moment anything did guard it, since `--write` would have called the repair noise. Measured from a commit the delta is idempotent, so weighing it cannot make an ordinary rerun look dirty. The guard runs **before** the write, so a refusal leaves the tree byte-identical and the exit status and the effect agree. States builds compared, paths compared, warning rows checked, recorded manifest values compared, and emitted paths weighed for the review delta against the base it names; **zero of any is a red** — and the delta's own length is never that denominator, since an honest delta is empty whenever a recorded input moves without moving an emitted byte |
| `tools/gallery-build.py` | CI + agent | `python3 tools/gallery-build.py --point <primary\|overlay> --prefabs <generated> [--lang en] [--delvec BIN] [--src DIR] [--out DIR]` — **one point of the gallery domain, materialised and compiled to a tree on disk that anybody can rebuild.** Every build of the domain except the primary in `en` existed only inside `gallery-baseline.py`'s temp directory, which is right for a tool that keeps a hash and wrong for the thing that made the site-plan overlay — the map pipeline's first walkable whole — a tree **no committed act could produce**. The bot ladder walked it and the run was reported against a build nobody could bring back, and a round that went looking for the materialisation read the coverage checker's validate-only copy first. This writes two persistent directories, both gitignored and both replaced on every run: the campaign (`validation/gallery-src-<point>`) and its build tree (`validation/delve-output-gallery-<point>`, the tree `validation/bot-run.sh --output` boots). What a build point IS comes from `tools/gallery_domain.py` and from nowhere here, so this cannot become a third opinion on the question. **Green means reproduced, not compiled**: the build's own `manifest.json` must equal the row `gallery/baseline/manifests.json` already holds for that point — a manifest written by a different tool, on a different machine, at a different time, which is what makes it a second observer rather than the run agreeing with itself. A mismatch refuses and names every differing entry across the WHOLE manifest, inputs included, because the first perturbation it was tested against emitted identical bytes and moved only `inputs`; what KIND of finding it is stays with `gallery-baseline.py`, which decides emission-change vs ADR-0006 determinism-finding from the diff. The gallery is engine-test source and this changes nothing about that — never released, never staged (spec-0039 §2); what it makes regenerable is the tree the BOT walks and CI measures. States its binding count every run — source files materialised, emitted paths compared, and the baseline row compared against; **zero of any, or no baseline row for the point, is a red**. Runs as a step of `gallery (coverage + build + baseline)` — a step, not a job, since every job name is a required status context |
| `tools/gallery_domain.py` | library | Not a gate — the one authority on what a **build point** of the gallery domain is: the primary, or the primary with one overlay's or probe's files over it, materialised into a campaign directory. `gallery/` is not one, carrying `baseline/`, `overlays/` and `probes/` beside its stage documents. It was a function in each caller and the two had come to mean different things — `gallery-baseline.py`'s stripped the three non-campaign directories because it was about to compile the result, `check-gallery-coverage.py`'s did not because it only had to survive a schema walk — so the tree the coverage gate validated was not the tree the baseline compiled, and a round needing to BUILD a point took the validate-only copy and had to recover the difference from the repository by hand. Materialising REPLACES its destination rather than merging onto it (a file left from a previous point is a campaign nobody authored) and refuses a destination that is, or contains, the gallery, since it deletes what it is handed. `tools/tests/test_gallery_domain.py` is what refuses a third copy, keyed to the object — a tool that copies the gallery tree — rather than to the word |
| `tools/gallery_units.py` | library | Not a gate — the shared surface-unit enumeration and the schema-guided binder both halves of the coverage check import. Kept in one file precisely so that *what the DSL declares* and *what a document writes* cannot come to mean different things; its module docstring carries the schema-walk edge semantics (`Option<T>` is optionality not a union; a variant's tag is the variant, never also a property of it; a `$ref` is followed for binding and not for enumeration) that spec-0039 §9 leaves to the implementation |
...
| `tools/check-anchor-providers.py` | CI | `python3 tools/check-anchor-providers.py` — **one authority answers *what anchors does this campaign have*, and nobody else asks.** `AnchorRegistry::anchors_for(prefab)` answers only the narrow question — which names does THIS PREFAB declare — and turning that into the question every check actually has takes a walk over `world.areas`, which eleven checks in `dsl::validate` each wrote for themselves. They were identical, they were correct, and they were correct about a world that no longer exists: when a site plan became a second placement authority a campaign stopped needing prefabs at all, and the derivation synthesizes the whole spatial vocabulary itself. **One** of the eleven walks was taught to ask `siteplan::synthesized_anchors`; the other ten went on enumerating prefabs a derived world does not have, so every stage-5 verb but the ones that happen to walk was unauthorable on a derived map — a `shortcut` naming the very `anchor/unlock-<edge>` the derivation places for it was refused as an invented name, and so were a trap, a shop, a loot chest, a lethal volume, a lane, a timed gate, an actor and a trigger. **Nothing was red, and nothing could have been**: a check resolving against a smaller world than the campaign has refuses CONTENT, and its own binding counts stay truthful about what it was handed. Same shape as `check-effect-roots.py` above, and the same repair — one authority, plus a gate that stops the next copy. It demands that every `anchors_for(` call site under `crates/dsl/src/` sits either in `registry.rs`, where the narrow question is DEFINED, or in `validate.rs` **at most once**, inside `AnchorProviders::build`, which is the one place the broad question is answered; a second call in `validate.rs`, or a call anywhere else, is the eleventh walk arriving. **Known non-proof, stated in its docstring**: it does not prove `AnchorProviders` is correct, nor that a consumer reached for the right accessor — a call to `resolvable` where `for_area` was meant is a different defect and invisible here. What it guarantees is that there is exactly one place to fix when the answer changes. States its binding count every run (currently 3 call sites across 2 of 23 files under `crates/dsl/src`); **zero call sites is a FAILURE, not a pass**, since a renamed trait would otherwise leave it guarding nothing. Deterministic, offline, Python 3 stdlib only. Runs as a step of `docs (local link check)` — a step, not a job, since every job name is a required status context |
...
| `tools/check-unsanctioned-identifiers.py` | CI | `python3 tools/check-unsanctioned-identifiers.py [--tighten]` — **no repository artifact carries an identifier a reader cannot resolve.** The sanctioned ones are ADR numbers, spec numbers and DW codes (CLAUDE.md *Privacy in repo artifacts*); this refuses the four that are not, measured and reported separately because they arrive by different routes — a **task id** (`task #N`, `tasks #N`, `task-#N`, any case), a **pull-request or issue number** (`(#N)`, `PR #N`, `PRs #N/#N`, `PR <n>`, `pull/<n>`; a bare `#N` of one digit is left alone, since single digits are row numbers and numbered sections inside the document that writes them), a **dated attribution**, an ISO date within 120 characters of a person or a decision word inside the same paragraph, and a **role attribution**, which answers the half of the rule the date does not: no artifact records WHO decided something. That one requires a person word (`owner`, `she`, `her`, `his`, `reviewer`) BOUND to a decision word (`ruling`, `ruled`, `decision`, `decided`, `directive`) — adjacent or one word apart, across a line end but never across a blank line. A parenthetical pairing a person with `ruling`, a hyphenated `<person>-ruled` label and a sentence saying that somebody ruled a thing not a defect are all findings, while *an architecture decision record*, *the campaign owner's id* and *a decision taken once at the start* are not — this row can therefore describe the shape but never spell one out, which is the gate binding on its own documentation. The binding is the whole false-positive defence, because either word alone is ordinary English about this software, and a gate that reddened every use of *decision* would teach the people who see it that this red means nothing. Deleting the date is not the repair for either attribution kind: the message says to state the rule impersonally, as a fact about the software. A plain date is deliberately NOT a finding — an ADR's own `Date:` header, an upstream release, the day a licence was verified — because ADRs are the one place history legitimately lives and a licence date is evidence a reader may need to re-check. A **stylesheet's own colour literals** are exempt, and the exemption is narrow in both directions at once, because the obvious form cannot be: "a digit run of a colour's length" would exempt a three-digit `#N` everywhere, which is how a checker gets routed around by the fix for its own false positive. The token must be a syntactically valid CSS hex colour — `#RGB`, `#RGBA`, `#RRGGBB` or `#RRGGBBAA` written with no hex letter, so a two-digit run is never one — in a `.css` file, in stylesheet CODE: comments and quoted strings are masked out first, so a `#N` inside `/* … */` or inside a `content:` string stays a finding. That is a property a citation cannot supply — outside a comment and outside a string, CSS has no syntax in which a bare digit run means anything but a colour or an id selector, so a pull-request reference written into a stylesheet HAS to sit in one of the two places the mask removes. It is a RATCHET, not a one-off sweep: the count refilled twice inside a week of the first tidy because merged branches carry their own citations in and nothing looked. `FLOOR` holds, per file, exactly how many occurrences that file may still contain, compared by **equality in both directions** — above it is a new citation, below it is a floor that must be lowered in the same change, a file with matches and no entry is a red, and an entry on a file with none is a red. `--tighten` rewrites the entries and **refuses to raise one or to add one**, which closes the convenient way to turn this red green; `FLOOR` lives in the checker rather than in a data file so raising a number is a diff to a gate. It is **empty**: nothing is budgeted a citation. It once held three files and five occurrences, and one of the five was not a violation at all — the viewer stylesheet's `#RGBA` shorthand colour, read as a pull-request number. A floor entry that permanently budgets a false positive is the gate paying to hide its own defect: the number stays green and the exemption that should have been fixed never is, so **the repair of a false positive is the exemption, never the floor**. `ALLOWED` carries only the two test files whose SUBJECT is these strings — this gate's own and `check-crates-io-readmes.py`'s — and the property that opt-out demands is one the defect cannot supply: an assertion in the file must FAIL if the string stops being a citation, which a document that merely wants to keep one cannot produce. A finding names the file, line, kind and token, and says what to do with the sentence: keep the OBLIGATION as a plain present-tense fact or delete it, and never replace the citation with a note about what it used to assert — that trades an unresolvable reference for a changelog, which is the same defect one layer out. Reads `git ls-files`, so an untracked scratch file is not a repository artifact. Deterministic, offline, stdlib-only python3. States its binding counts — files examined, bytes examined, per-kind totals, floor and allowlist sizes — and **examining nothing exits 2**, since a check that matched no files must never look like a check that passed. Runs as a step of `docs (local link check)` and again in `engine-release.yml`'s `crates-preflight`, because the tagged tree a release publishes from is not necessarily the tree CI last saw |
...
| `tools/check-source-dupes.py` | CI | `python3 tools/check-source-dupes.py [path …]` — the same merge-artifact gate where the sections are code: no `// ---- name ----` section header repeats within one scope of one `.rs` file, over `crates/`, `prefabs/` and `harness/`. Kills the class that made `plan.rs` invoke the face-mating pass **twice**, as two verbatim copies of one labelled block. That class is invisible to every other gate because the pass is idempotent — the second run recomputes the same answer, the second binding shadows the first, and the only trace anywhere is a zero-binding advisory printed to the operator twice. Two candidate rules were measured over the 397 `.rs` files before one was chosen: a repeated section header is **0** hits on a clean tree and fires on the defect; any repeated 3-line run of code is **3930** hits across 283 files, all correct (builder chains, match arms, per-axis repetition), so the code-shape rule is declined in the script's own docstring rather than shipped behind an allowlist the size of the tree. A `#[cfg(test)] mod` is its own namespace, because a test module conventionally mirrors the implementation's section names so a reader finds a phase's tests under the phase's heading — a narrowing the defect cannot supply, since a copied block lands next to its twin inside one function. States its binding count (files and headers) on every run. Runs as a step of `docs (local link check)` — a step, not a job, since every job name is a required status context |
...
| `tools/check-numbered-doc-index.py` | CI | `python3 tools/check-numbered-doc-index.py` — **an index of numbered documents agrees with the documents it indexes.** `docs/specs/README.md` and `docs/adr/README.md` are tables with a status column, and neither is the authority for a status: each document's own `Status:` line is, so both tables are COPIES, and a copy nothing compares against its source drifts the moment either side is edited alone. Measured before this gate existed: **seven of thirty-seven spec rows disagreed with the file they point at** (three said `Proposed` for a spec the file called `Accepted`, three said `Approved` for a spec the file called `Draft`), **seven spec files had no row at all** — including `spec-0039`, the gallery — and **`ADR-0020` was missing from the ADR index**, while nothing in `tools/` opened either file. The drift is invisible to every neighbouring gate, and not by accident: link checking passes because the links are valid, `check-doc-dupes.py` passes because no key repeats, and `check-numbered-doc-uniqueness.py` passes because no number collides — each asks about the table's SHAPE, and none asks whether what it SAYS is true. Six rules per series: every document has a row; every row points at a document that exists; every row's number matches its link target's number (a row labelled with one spec number and linked to another reads correctly and sends the reader elsewhere); every row's status equals the document's own; every document's status is readable; and — for the spec series only, keyed off the series' own `section` declaration — **every spec carries an Acceptance criteria section**. That last obligation is `docs/specs/README.md`'s own preamble sentence and CLAUDE.md's Conventions line, and before this rule it was a doc line, which is not an invocation: the sentence was true-by-declaration while four specs carried no such section. A heading containing the phrase counts however it is numbered or dressed; a mention in prose does not; the rule demands the section and not its quality — machine-checkability stays with review, and a spec whose subject makes a criterion genuinely unwritable by machine says so inside the section, naming what a human checks instead, rather than earning an exemption in the gate. Statuses are compared as the STATUS and not as the cell text, so an editorial gloss (`Accepted (M4)` against `Accepted (implementation deferred to M4)`) is not a disagreement — a gate that reds on correct prose teaches the people who see it that its red means nothing. Every diagnostic names **which file is the authority**, because a reader who cannot tell which of two disagreeing statements is the source repairs the wrong one half the time. A document with **no readable status is reported and never filled in**: writing a status into a document nobody ruled on forges the fact the gate exists to protect. `STATUSES` is descriptive — CLAUDE.md's ADR words plus the ones the spec corpus uses — and this gate rules on no lifecycle: it refuses exactly a word no document has used and a copy that disagrees with its source, and adding a word is a diff to a checker. A third numbered series is one `SERIES` entry, not a second script (CLAUDE.md: a capability belongs to the object class it acts on). States its binding count per series — documents, rows, statuses compared, and acceptance-criteria sections found where rule 6 binds — and a series empty on either side **exits 2**, since a check that binds to nothing is not a check that passed. Runs as a step of `docs (local link check)` — a step, not a job, since every job name is a required status context |
| `tools/next-numbered-doc.py` | human-in-the-loop (allocation) | `python3 tools/next-numbered-doc.py spec|adr [--list]` — answers **which number to take**, and it is the allocation counterpart to the uniqueness gate above rather than a second gate. That gate's own row records why it cannot close this: it compares a branch against `--base`, and two rounds dispatched at the same time each read a base where NEITHER number is taken, so the collision exists only in the union of two branches that have not merged. Its row says nothing short of the GitHub API would see that; this tool takes the third route it did not consider — `git for-each-ref refs/remotes/`, which sees every branch the checkout has fetched, with no network call and no token. **It is deliberately not a CI gate**: by the time CI runs, the number is already written into a filename and a title, and the cost of the collision has been paid. The allocation has to happen where the concurrent work is known about, which is at dispatch. Prints the next free number and, with `--list`, every claimed number and the ref holding it, so a collision already in flight is visible rather than inferred. **Refuses (exit 1) when it cannot enumerate refs** rather than falling back to the default branch alone, because that fallback IS the reading that produces the collision — a silent answer from the wrong universe is the failure mode, not the absence of an answer. Two collisions in one session are what this replaces, both of them a note in a plan file, which is a doc line and therefore not an invocation |
| `tools/check-demo-levels.py` | CI | `python3 tools/check-demo-levels.py [--base <ref>]` (default `origin/main`) — **a change that adds author-facing command-line surface adds its demo row in the same change.** Every new mechanic owes a demo level, and [`demo-levels.md`](../demo-levels.md) is not a backlog to be worked off later — it is the form an engine capability is confirmed in, so a capability with no row cannot be shown to anyone. That obligation lived in a doc line and behaved the way that shape always does: over 416 changes on main the file was edited six times, sixteen of its rows were seeded retroactively in one sitting, and in the twelve days after that seeding not one row was added while the compiler, the grammar and the render layer all gained surface. Nothing on a diff says "mechanic", and the candidates were measured rather than argued: a new DW code fires on 101 of those changes, a `feat(` subject line on 140, a `docs/reference/` heading on 94, a `Since`-table row on 27 — noise generators, and a check that fires on one merge in three is routed around inside a week. The demand therefore keys off the one clean signal: **a new long flag on a binary under `crates/`**, discovered from the clap derives rather than listed, every such binary being an `agent`- or `human`-class tool on this page. It fires on 16 of 416 (3.8%) and every one of the sixteen landed a capability — fourteen of them with no row, which is the measurement of the defect. **Recall is roughly a third and is stated in the script rather than implied**: a capability with no command-line surface is invisible here (the grammar's guard-exhaustion refusal, which landed the same day as the aimable camera and owes a row just as much, adds no flag). There is deliberately **no opt-out** — "this one needs no demo" is exactly what someone omitting a row already believes, so any hatch would be one the defect supplies for itself. The second half binds on every other change: every `DW` code, spec number and ADR number a queue row cites must resolve in this tree, because the file's own rules allow "covered by" coverage to rot. Both halves are counted off the mechanic-demo table, so the table is parsed the way the renderer that draws this page parses it: a pipe table runs from its header row through its delimiter row to the first blank line or the start of another block-level structure. **A row detached from the table by a blank line is therefore a red naming its line number**, never an entry and never a silent discard — a renderer draws it as a paragraph of literal pipe characters, so counting it would make the demo obligation satisfiable in the letter while void in the reading. States its binding counts (binary crates, source files, flags on both sides, rows on both sides, citations examined, rows no table contains on both sides) every run; zero binaries, zero flags, zero rows or zero citations is a red. When `--base` is missing it refuses and prints how to fetch it, computed from the repository it is running in by `tools/lib/gitbase.py`. Runs as a step of `docs (local link check)` — a step, not a job, since every job name is a required status context |
| `tools/check-version-ledger-uniqueness.py` | CI | `python3 tools/check-version-ledger-uniqueness.py [--base <ref>]` (default `origin/main`) — the numbering defect above, one layer down, on a different kind of number. A **version ledger** is picked exactly the way a spec number is: list the versions that exist, write the next one. So two branches take the same number for DIFFERENT surfaces and both stay green — one writes `MIRROR_SINCE = "1.1.0"` and the other writes `CONTRACT_SINCE = "1.1.0"` into the same new `crates/grammar/src/version.rs`. Unlike two new spec files those two DO conflict textually; the hazard is the **resolution**, because unioning both constants under one `1.1.0` compiles, passes clippy, and ships an engine that accepts a `1.1.0` document written against the construct it does not implement and silently drops it — ADR-0018 §7's failure, reintroduced by the fence's own numbering. A version's surface is named by its **fence anchor** (code, not prose, so it does not drift with wording), and anchors come in two kinds. **Named** anchors are hand-written and not derivable from the number — the `*_SINCE` constants on either ledger, plus each row of that ledger's reservation list (`RESERVED_VERSIONS`, `RESERVED_DSL_VERSIONS`). **Derived** anchors are computed from the number: `dsl_version`'s `is_vNN` predicates resolved through `ordinal()`, which prove a number is in USE and can never say what it MEANS. A version's claim is its named anchors where it has any and its derived ones otherwise — precedence, not union, because a landed reserved surface spells its number twice (`OPEN_WAY_SINCE` and `is_v12`) and that is one surface. Six rules over the claims that stand once this branch merges — the union of the checkout and `--base`, **less what either side withdrew** — one number one surface; one surface one number (a fence may not move once it is on the base); every number past the founding one is claimed by something (so skipping a number does not free you from declaring it); the list is append-only; a reservation is deleted by the change that lands its surface; and a number this branch ALLOCATES — added, or freed by a hold it cancelled — carries a hand-written name. **Reservation** is how a number is held for a sibling change: it claims the version under the NAME of the constant that change will define, so a forward declaration and the change that fulfils it agree, and a forward declaration naming the wrong constant reds. It is what lets an approved spec HOLD a `dsl_version` before any of its surface exists — a reserved number is in the ledger and refused (`is_supported_version` says no, `DW0102` fires), which is the only loud answer an engine has for a surface it does not implement. A hold can also be **cancelled**, and that is a three-way question rather than a two-way one: a row at `--base` that this branch does not carry was either deleted by this branch or added to the base after this branch was cut, and the checkout looks identical either way. So the gate reads the **merge base** of `HEAD` and `--base` (`git merge-base`, local, no network) and subtracts only what this branch INHERITED and dropped. Two restrictions keep that from being an amnesty: only a reservation row is withdrawable — a defined `*_SINCE` or a derived predicate that vanishes is a deletion of shipped surface and stays rule 1's finding — and only while neither tree defines its anchor, since a hold whose surface has landed is no longer a hold. A competitor's base-side claim is either implemented or was never inherited, so it can satisfy neither. Cancelling a hold ALLOCATES the number, so rule 6 follows the withdrawal and the freed number must carry a hand-written name in this checkout exactly as an added one must. Where no merge base exists — an unborn `HEAD`, a shallow checkout — nothing is withdrawn and rule 1 is the union it always was: the gate over-refuses rather than under-refuses, and names the missing instrument in both the finding and the binding line. This is why the `docs (local link check)` job checks out at `fetch-depth: 0` and fetches `origin/main` WITHOUT `--depth=1`. Covers every version ledger in the repo, one row per ledger in `LEDGERS`; the object class is *a version ledger*, not one crate. **Two limitations, both in its own docstring**: it is blind to another open branch that has not merged into `--base` (the CI token stays `contents: read`), so what it guarantees is that once the FIRST of two colliding PRs merges the second goes red; and nothing in a version ledger says what a surface IS, so a change that hangs a new field behind an EXISTING number allocates nothing and is invisible here — the same on both ledgers. Rule 6 closed the limitation that used to sit beside these: `dsl_version`'s implemented anchors are self-naming, so two branches taking `0.12.0` for different surfaces both produced `is_v12` and rule 1 read one claim. Measured, not theorised — an `open-way` `0.12.0` against a horizon-library `0.12.0` ran green with `0 collision(s)` — and both halves are now driven red→green by `tools/tests/test_check_version_ledger_uniqueness.py`. States its binding count (versions, anchors and reservations per ledger on both sides, collisions, how many ADDED versions rule 6 examined for a name, and every withdrawal by version, anchor and the side that cancelled it — or, against the population of inherited holds, that there were none) every run, so a hold cannot leave a ledger without appearing in the output; zero anchors on BOTH sides is a red, and a ledger that parses to zero versions or to zero traceable anchors exits **2** with the pattern to fix named — fix the pattern, never loosen the check. When `--base` is missing it refuses and prints how to fetch it, computed from the repository it is running in by `tools/lib/gitbase.py` — a full clone is told to fetch plainly, an already-shallow one to fetch at depth 1. Runs as a step of `docs (local link check)` — a step, not a job, since every job name is a required status context |
...
| `tools/check-pins.py` | CI + creator | `python3 tools/check-pins.py [--root <repo>] [--registry <path>] [--online --checkout <id>=<path> …]` — **a pin freezes an instrument on purpose; the defect is a pin nobody has looked at since the thing it names moved.** That state is invisible: the file reads the same on the day it is written and a year later, and CI keeps passing with the old instrument. A content-repo audit built its judge from a pinned pipeline commit four hundred commits behind, so the audit arriving on every piece was palette-only and would have admitted a piece failing both settling rules; a second pin sat while the gate it enforced was settled upstream, and a zone was reported red for failing a rule that no longer existed — the repair a reader naturally reaches for there is to build the thing the stale gate asks for, which is the vacuity the gate was written to catch. **Offline half** (the default, stdlib-only, no network, so it runs in a required job and on a creator's clone with nothing installed): discovers pins by SHAPE — 40-hex revisions, `sha256:` and bare-64 digests, `uses: …@ref`, and the `(repository, ref)` pair of a cross-repo checkout with `${{ env.X }}` resolved — across every file kind that can execute or be resolved from. **The examined population is every TRACKED file, with no directory skipped**, and the omission is load-bearing: a skip list over `git ls-files` can only ever subtract authored content, never add safety, because build output and ignored trees are not tracked to begin with. Directory skipping is scoped to `BUILD_OUTPUT_DIRS`, which serves the one walk that reads a RAW upstream checkout for Cargo manifests and so really does meet `target/` and `node_modules/`; every name in it is a fact about a cargo or npm working tree and none names a directory of any Delvewright repository. Confusing the two populations is what once removed 27 tracked files — every campaign stage document — from discovery and from the fetch-verb enumeration in the content repo, with no count anywhere saying so, because a file the enumeration never lists cannot be reported as unexamined. It then asserts each pin is registered in `.github/pins.toml` with a policy, that each entry still matches the files it claims, and that a `cargo build -p <pkg>` out of a pinned checkout appears in that entry's `builds`. **A pin named by a VERSION STRING is the class where an entry's two obligations come apart** — nothing escapes discovery, and every held version carries a recorded decision — because a semantic version has no shape separating it from data, so the decision is owed exactly where discovery is impossible. Both halves are closed and by different means. A value with no shape of its own is still discovered where SOMETHING ELSE fixes what it means, and four schemas do: a manifest whose file kind fixes the key (`rust-toolchain.toml`'s `[toolchain] channel` is a pin because rustup downloads what it names); a Python package manifest (`requirements*.txt`, a `[project] dependencies` array), read on the terms already granted to `Cargo.toml` and `package.json`; an ACTION'S INPUT CONTRACT, because the setup-node and setup-python actions exist in order to fetch the version their input names; and an INSTALL COMMAND in a step the repo runs. Only `==` is read out of a manifest, because every other operator states a RANGE and a range names no version — which keeps a legitimate `>=` floor out by a property of the requirement rather than by an exemption. An install is an ACT rather than a declaration, so a package argument naming no exact version is a FINDING and not a pin: there is no value to record and nothing to hold still, and an unpinned instrument inside a required gate is `CLAUDE.md`'s "a frozen measurement names its instrument literally" inverted. **Membership is decided by discovery, never by importance**: an entry exists because a schema found the value, so appending one by hand for a value no schema reaches is the enumeration-somebody-remembered defect this same tool refuses elsewhere — which is why a `runs-on:` label is out (it selects the machine, fetches nothing, and has no more-frozen form to hold). Two entries may not claim one value: discovery is keyed by the value, so a collision would merge their site sets and stop checking one of them, with the registry still reading complete. An unregistered toolchain reds like any other pin. The sites that merely RESTATE the value — a `rust-version` floor, a manifest line — are reached by no widening ever, so such an entry names `bound_by`, the checker that holds those sites equal, and `bound_key`, the key it reads the value under; the offline half verifies the three things a drifted pin cannot supply (the file really reads that key, it really names every declared site, a workflow really runs it) and then asserts the value stands as a whole token at each site. **Which arm an entry owes is read off the value, never chosen**: a shaped value may not declare `bound_by`, and an unshaped one may not omit it, so the effective obligation is never the weaker of two. The site list is itself checked: any tracked non-prose file carrying a fetch verb (`uses: …@`, `FROM`, `docker run|pull`, `git clone`, a Cargo `git =`) that no site pattern covers is a red, so a new kind of fetch site fails here instead of escaping the registry. **A verb is read in the language of the file it sits in**, because half of them mean nothing outside their own: a *command* (`docker run|pull`, `git clone`) is a process any language can spawn and so is read the same way everywhere, while a *directive* (`uses: …@`, a Dockerfile `FROM`, a Cargo `git =`) is a statement in one configuration language and is prose in a file positively known to be a different one, and a verb may belong to a SET of languages — `pip install` is a shell command line, so it is an invocation in a shell script and in a workflow `run:` block, and in a Python file the identical characters are what a program PRINTS to tell a creator what to install — a Rust diagnostic whose all-caps emphasis wraps `FROM` onto a new line is not a Dockerfile stage. The narrowing is fail-closed and carries no exception list, no allowlist and no per-file opt-out: a file whose kind is unrecognised is read with every verb, so a Dockerfile written under a name nobody anticipated still reds, and the defect this catches cannot change a file's language to escape it. The enumeration states its own binding — how many verb applications over how many uncovered files — and applying no verb to any file is a finding, not a clean tree. **Online half** (`--online`): judges each pin against the history of the repository it names, using the checkout the calling job already makes — so the check runs in the same job, over the same value, immediately before the thing it guards, and there is no arrangement in which a job builds from a pin and skips its own next step. **Four policies, decided by the object rather than by the author.** `release` — a `v<semver>` tag must point at the value, which a commit somebody merely stopped looking at cannot supply; drift is never a finding, because a released delve reproduces through its own pinned engine. `track` — the value must still be an ancestor of the pinned repo's default branch, and the entry carries `reviewed`, the upstream revision it was last judged against; if any commit touching the sources the site BUILDS landed after `reviewed`, the pin has not been looked at since its instrument changed and that is the red. The watched paths are never author-written: they are the package closure of `builds`, read out of the upstream tree. `immutable` — content-addressed third-party bytes, whose currency is a supply-chain question with a different owner and which this gate says it does not answer rather than passing silently. `floating` — a ref that moves on purpose, registered so the count of deliberately-unfrozen refs is visible instead of habitual. A pin naming a repository this project builds may not be declared `immutable` or `floating`, so the one downgrade that would exempt exactly the pins that rot is closed. Every `track`/`release` entry names `judged_by`, and the offline half asserts that file really invokes `--online` and really names the pin — a `judged_by` nobody verifies is the doc line this project has shipped five times. States its binding count; a binding of zero is a finding, not a pass. Runs as steps of `manifest consistency (versions.toml)` — steps, not a job, since every job name is a required status context. The content repo runs this same check against its own registry from its `NBT palette audit` job, because a check a creator can only run by cloning a second repository is not a check that repository has — and it does so with a **verbatim vendored copy of this file**, not a second maintained one. That is possible only because the checker carries no per-repository claim: the copies once differed by one directory-skip constant, inert here and destructive there. The content repo declares the copy in its own registry (`vendors`) and holds it to byte-identity against the pinned checkout with `tools/check-vendored.py`, which also reds when this file moves upstream after that pin was last reviewed. That guard is a separate script there rather than a mode of this one, because this tool ignores registry keys it does not know — so a vendor check living inside it would vanish exactly when an older copy was vendored, which is the moment the copies have drifted |
| `.github/pins.toml` | record | the pin registry `check-pins.py` reads: one entry per pin, with its sites, its policy, and — for a judged pin — the upstream revision it was last reviewed against and the file whose job judges it; for a pin named by a version string, the checker that holds its sites equal (`bound_by`) and the key that checker reads it under (`bound_key`). The reason a pin is held lives here rather than in a comment beside the value, because nothing reads a comment, and a rotted pin and a deliberately frozen one are identical in a diff |
...
| `tools/lib/mdtable.py` | library (CI gates) | Imported, never run. The repo's ONE reader of a markdown pipe table, applying the rule its RENDERER applies and no other: a table runs from a header row, through a delimiter row of the same cell count, to the first blank line or the start of another block-level structure; fenced code holds no tables. Seven gates counted rows out of markdown by matching a regex against lines, so none of them could tell a row inside a table from a line of prose full of pipe characters — and a blank line ends a table, so a detached row was an entry for the gate and a paragraph of literal pipes for every reader. Measured by perturbation on the live documents, `check-dw-codes`, `check-numbered-doc-index`, `check-trial-verdicts`, `check-grammar-ir-compat`, `check-capability-ownership`, `check-stated-counts`, `check-reference-versions` and `check-demo-levels` all stayed green with the last row of their table detached; `compiler.md` was carrying twenty-one such rows and the ownership audit two. `read()` returns the body rows AND the row-shaped lines no table contains, and `rows_matching()` filters both by a gate's own row pattern — so a detached row is a finding naming its line rather than a silent count or a silent discard, and a gate reds only about the rows it reads. Its agreement with the renderer is the half that rots silently, so `tools/tests/test_mdtable.py` compares it against `pandoc`'s GFM reader over every tracked markdown file — a second instrument with an unrelated failure mode, 1930 body rows each, zero files disagreeing — and skips rather than passing quietly where pandoc is absent. Lives here for the same reason `gitbase.py` does: seven private copies of one general mechanism is a defect, not seven implementations |
...
| `tools/check-shell-bash32.py` | CI | `python3 tools/check-shell-bash32.py` — refuses shell syntax that bash **3.2.57** does not have, which is the bash macOS ships and therefore the one a creator runs (`CLAUDE.md`: every validation must be runnable on the creator's own machine). **Why**: `mapfile` is a bash 4.0 builtin, and on 3.2 it does not fail — it prints `command not found`, leaves the array EMPTY and carries on at status 0, which is how `validation/packtest-all.sh`, the script whose whole job is to run every PackTest project, ran none of them and exited 0. Seven scripts already carry a comment saying not to do it; a comment is the bottom rung of the ladder `CLAUDE.md` ranks, and this is the rung above. **The class is the version boundary, not the one string**: `mapfile`/`readarray`, `coproc`, `wait -n`, `declare|typeset|local -A/-n/-l/-u/-g`, `${v,,}`/`${v^^}`, `${v@Q}`, `${a[-1]}`, `&>>`, `|&`, `;&`/`;;&`, `[[ -v ]]`, `printf -v arr[i]`, `printf '%(fmt)T'`, `{fd}<file`, and any `shopt` name outside the **34** bash 3.2 knows — that last one a closed world read out of the shell itself, so it cannot go stale as bash grows. Every member was run on `/bin/bash` 3.2.57 and on `bash:5` in Docker: two binaries on two operating systems, sharing no calibration, and a member that fails on both would be a typo rather than a version boundary. Half the class does not abort at all, which is why nobody ever noticed. **Population derived, never listed**: `.sh`/`.bash` UNION every tracked file whose first line is a shell shebang, plus `run:` blocks in `.github` workflow and action YAML — `engine-release.yml` builds two darwin targets on `macos-latest`, whose `/bin/bash` is 3.2, and which bash an Actions step resolves is a runner-image property this repository neither pins nor controls. `defaults.run` mappings are told from step shell by shape, and the `run:` key count must equal blocks plus mappings or the parser has lost one. **Reads three masks of each line**, because a command inside quotes is text (`echo "no mapfile here"`), an expansion inside double quotes is real (`"${v,,}"`), and a `printf` format is interpreted by bash however it is quoted. Comments are stripped quote-aware — every recorded instance is a comment *saying not to use `mapfile`*, so a naive grep reds exactly the lines that preserve the lesson. Heredoc bodies are data for another program and are not read, with the count printed; a heredoc that writes or feeds a shell script is a finding, and so is an unterminated one, since both mean input went unread. States four counts and refuses on any zero, `docs/experiments/` included — an exclusion matching nothing is a stale constant, not a pass |
...
| `tools/fmt-workspaces.sh` | human + CI | `bash tools/fmt-workspaces.sh [--check]` — **formats every cargo workspace in the tree; the same command CI runs, so a clean run here is what CI will say.** `cargo fmt --all` at the repository root reaches the root workspace and no other: `crates/render` and every `prefabs/*-generator` are separate workspaces on purpose, so the obvious local command reports clean while a generator is unformatted, and the `rust (fmt, clippy, test)` job reddens the pull request instead — the cost paid by whoever pushes rather than by whoever wrote it. Widening the root workspace is refused (those crates are separate for reasons measured in `/Cargo.toml`), so the repair is that one command reaches what CI reaches. **The population is derived, never listed**: `git ls-files -- '*Cargo.toml'`, with each manifest's workspace resolved by asking cargo (`cargo locate-project --workspace`) rather than by grepping for a `[workspace]` table — cargo is the authority on which workspace a manifest belongs to, and a grep would answer wrongly for a lone package that declares no table of its own. That is what the hand-written list it replaces could not do: the tree holds ten cargo workspaces and CI's three fmt steps named **nine**, the tenth being `docs/experiments/…/generator`, reached by no fmt step anywhere. Exclusions are properties rather than names — `target/`, `node_modules/` and vendored manifests are absent because they are untracked, and `campaigns/` contributes nothing because it is a symlink into another repository. There is exactly **one** prefix exclusion, `docs/experiments/`, which preserves an experiment as it was run so that rewriting it would falsify the record (the same words and the same prefix as `tools/check-shell-pipe-shortcircuit.py` and `tools/check-shell-redirect-dirs.py`), and it is **counted every run** — an exclusion matching zero tracked manifests is a refusal, since a stale exemption is a finding. That constant is also read from the outside: `tools/check-structure-emitters.py` reds if it ever swallows a `prefabs/*` generator workspace, so widening it cannot quietly truncate the population this sweep is trusted to cover. A population of zero, or a manifest whose workspace cargo cannot resolve, are refusals too rather than a quietly smaller sweep. States the swept count against the tracked-manifest population on every run. It runs `fmt` only: `clippy -D warnings` on the generators is somebody else's pull request, because `prefabs/generator` carries two pre-existing style lints. It needs no registry, no lockfile and no network — measured with an empty `CARGO_HOME` and `CARGO_NET_OFFLINE=true`, the whole sweep including the git-dependency workspace exits 0 and leaves that `CARGO_HOME` empty — which is why one step early in the `rust` job can replace three, still ahead of the render workspace's one named fetch. Bound as that step, and as the closing advice of `tools/worktree-new.sh`; a person who runs neither still has one command to run rather than three to remember |
| `tools/worktree-new.sh` | planner operations (**not** an authoring tool — a creator building a delve never meets it; it is run at dispatch, and it is the counterpart of the row below, which runs at the merge) | `tools/worktree-new.sh --path DIR (--branch NAME [--base REV] \| --detach REV) [--donor DIR] [--no-clone] [--no-lease] [--lease-holder WHO]` — **creates a worker's worktree that does not recompile what it can clone.** Every dispatched worker gets its own worktree, which is not up for revision: two workers in one checkout is one `git add -A` away from sweeping three authors into one commit. What was avoidable is that each of those trees then cold-compiled the whole dependency graph, because `git worktree add` copies nothing and `cargo build` compiles fresh, so **nothing in the chain ever asked the filesystem to clone**. APFS clone support was present and unused; 24 worktrees held 149.8 GiB of `target/` with no block sharing at all, confirmed by deleting one — `du` said 15 G and `df` gave back 14 GiB, which is what no sharing looks like. This script clones the donor's `target/` with `cp -c`. What that buys is **not mainly disk — it is compilation**: registry dependencies live at one absolute path in `~/.cargo` for every worktree, so their fingerprints survive the move intact, and only this workspace's own path dependencies are invalidated, because their source path is part of what they are fingerprinted against and that path is exactly what a new tree changes. Measured on a warm donor with `cargo build --workspace --all-targets`, two runs of each condition alternated so machine load could not be attributed to one of them: **6 packages of 140 rebuilt, 134 reused**, the clone moving 5.8 GiB in about 5 s at a `df` cost indistinguishable from zero. **The saving in time is much smaller than that ratio suggests, and the tool says so rather than letting a reader infer otherwise: 19.0% of CPU time** (859.2 s → 695.7 s, the two cold runs agreeing to 0.03% — CPU time is the instrument here because wall clock under concurrent workers measures the machine, not the build). The 134 reused packages are the cheap ones; what dominates this workspace is its own six crates and the forty-odd test binaries that link them, which is exactly what a new worktree must rebuild. Without `--all-targets` the figure is 11.5%. Worth making — it costs one `cp` and five seconds — and it is a fifth off, not a dependency graph for free. `debug/incremental/` is deliberately **not** carried: it is 3.06 GiB of the donor's 5.77 GiB, and incremental state serves only crates compiled from a path, which are precisely the units a move invalidates — measured, dropping it changed nothing (6 packages either way, 691.3 s against 695.7 s) and left the tree 3 GiB smaller. **The toolchain pin is resolved from the CURRENT DIRECTORY, and that is the trap this script exists to close**: `rustc`'s version is part of every fingerprint, rustup finds `rust-toolchain.toml` by walking up from the CWD and not from `--manifest-path`, so a build launched from outside a checkout silently uses the user's default toolchain — the measurement run for this very script did exactly that, built with a nightly instead of the pinned release, invalidated every cloned unit, rebuilt all 140 packages and reported that cloning saves nothing, with nothing erroring and the number merely plausible and wrong. So the script names the instrument literally on both sides and **refuses to clone when the two trees disagree about anything a fingerprint depends on that can be cheaply established**, and says so rather than copying output the tree cannot use. `rustc` was the first such input and is not the only one: **profile settings are fingerprint inputs too, and this repository moves them** — `[profile.dev]` gained `debug = "line-tables-only"` after `opt-level = 1`, and a rustc-only refusal passes the first tree cloned from a donor whose `target/` predates such a change, then invalidates every cloned unit anyway. The wasted build is the small half; the large half is that this reproduces, exactly, the misdiagnosed symptom above, wearing the costume of a regression in this script. The comparison itself is `tools/lib/cargo-fingerprint-inputs.py` (row below), which also states what it deliberately does not compare; a differing `Cargo.lock` remains reported and not refused, since the units still shared are still reused. It closes by naming `tools/fmt-workspaces.sh`, because the tree it just made is one a person will format. It also links `campaigns` (two `analyze` tests fail on a tree without it) and takes a `--lease`, so a live dispatch has removed itself from the sweep's question before it starts rather than depending on someone remembering, and **prints the command that will end that lease** — `--after-merge BRANCH --apply` for a branch tree, `--release PATH` for a detached one — rather than the instruction *release it at the merge*, which was the entire release mechanism for as long as this script existed and which nothing invoked. It does **not** share `CARGO_TARGET_DIR`: cargo locks a target directory for the duration of a build, so a shared one serialises every concurrent worker, and two `cargo test --workspace` runs against one target tree are already a recorded cause of reds that read as content defects. **Its invocation is not yet bound to an event** — a dispatch that types `git worktree add` by hand still gets a cold tree, and that gap is the UNRUN shape; what would close it is the dispatch step itself calling this script |
| `tools/lib/cargo-fingerprint-inputs.py` | planner operations (the comparison behind the row above; independently runnable) | `python3 tools/lib/cargo-fingerprint-inputs.py (--json DIR \| --diff DONOR NEW)` — **what two trees must agree about before one's `target/` is worth cloning into the other.** Compares **7** inputs, each established per tree: `rustc -vV` and `cargo -vV` run with the tree as their working directory (which is how a build resolves the pin — rustup walks up from the CWD and has no manifest flag — so the resolved answer is compared rather than `rust-toolchain.toml`, which is only the question); the `[profile.*]` tables of the tree's root `Cargo.toml`, and only those tables, since hashing the manifest would refuse on any ordinary dependency edit and degrade the tool to `--no-clone`, a loosening wearing a refusal's clothes; and the `profile`, `build`, `target` and `env` sections of every `.cargo/config.toml` on the tree root and each ancestor directory. That ancestor chain is kept as an **ordered list of tables, nearest first, never merged** — cargo's merge rules are their own subject and re-implementing them would be a second instrument with no calibration — and paths are dropped from it on purpose, because a donor and a worktree sit at different depths and refusing on depth would refuse on where the tree was put. **What it deliberately does not compare, with the reason, is in its own header**: environment overrides (`RUSTFLAGS`, `CARGO_PROFILE_*`) belong to a build that has not happened yet and are equal on both sides at creation time by construction, so comparing them would be a green that measures nothing; `$CARGO_HOME/config.toml` is identical for both; `Cargo.lock` differs legitimately on any branch; and the environment variables a `build.rs` declares with `cargo:rerun-if-env-changed` cannot be enumerated without running the build scripts — which is where "everything a fingerprint depends on" genuinely stops being enumerable, and a stated limit beats a pretence. Exit **0** agreeing, **1** differing (naming every differing input and rendering both sides), **2** when agreement could not be *established* — a missing `rustc`, an unreadable manifest. Two is a refusal, not a pass: the shape it replaces answered `unknown` on both sides and let them compare equal. Guarded by `tools/tests/test_cargo_fingerprint_inputs.py` in both directions |
| `tools/worktree-reclaim.py` | planner operations (**not** an authoring tool — it enters no skill, because a creator building a delve never meets it; it is run by the hook below and at a merge) | `python3 tools/worktree-reclaim.py [--apply] [--after-merge BRANCH] [--tree PATH] [--lease PATH \| --release PATH] [--targets-only] [--free-below GIB] [--repo PATH]…` — **reclaims the worktrees whose work has landed, in both checkouts, and refuses to touch any other**. A worktree is created by a dispatch and destroyed by the merge that lands its work; while that obligation lived in a sentence in a document it held for about one dispatch in four, and the disk filled twice — each tree carrying a full `cargo target/`, until `df` itself was unrunnable. So it is **bound to the events, not to a checklist** (the UNRUN vacuity mode): `tools/planner-state.sh` runs it with `--apply` on `SessionStart` and on every stale `UserPromptSubmit`, which is the one thing a merge typed by hand, a pull request closed in a browser and a stopped worker whose branch someone else pushed all pass through; `--after-merge BRANCH` is the same proof applied to one tree in the same breath as the merge. **What counts as permission to delete** is the question `CLAUDE.md`'s sixth vacuity mode asks — could the thing this is meant to exclude produce it? Quiet cannot be evidence, nor mtime, nor "no process has its cwd there" (an agent between tool calls has no process), nor the commit being reachable from the remote (a worker reading pinned content at a detached commit has exactly that). The one key a live dispatch cannot forge is the **remote's own pull-request state**: merged or closed is asserted by an authority off this machine about work that has already landed there. That key is only worth what the QUESTION is worth, so the remote is asked **about the branch being decided, one query per branch**, never by fetching a list and looking the branch up in it: a bulk listing returns the most recent N and never says it stopped, so a branch whose request falls outside the window reads as "no pull request on the remote" — a fact about a fetch wearing the clothes of a fact about the remote, which is the UNTRAVERSED vacuity mode, and it also made the tool structurally unable to reclaim anything older than one page. Each per-branch answer proves it is not itself cut off: an answer that fills its own row limit is treated as no authority at all. The run reports how many branches it **asked**, which is the only figure that is coverage rather than the size of a fetch. It is necessary and never sufficient — every reclamation also demands a clean tree, **no** commit absent from every remote, that **nothing outside the tree links into it**, no lease, and git's own re-check (`worktree remove` is called without `--force`). Dirty or unpushed outranks everything, by any path, for any reason. **Reachability is part of liveness**: `campaigns` inside one worker's tree is how it reads content, so a sweep judging a tree in isolation deletes a checkout two live workers are reading and neither fails — the workers keep running and measure zero over a directory that stopped existing. A reverse-reference index over the whole scratch area is therefore built before any verdict, and every **dangling** symlink it meets is reported as a finding of its own, because that failure is silent by construction. A **detached** checkout is never swept: a pinned-content read tree and a spent verification tree are indistinguishable by every local signal, so it is reclaimed only when an operator names it with `--tree`, which prints what it overrode and still enforces every other key. A live dispatch removes itself from the question with `--lease` (stored in the worktree's git admin directory, so it cannot make the tree dirty and cannot outlive it); a lease is honoured **even when its window has elapsed** — an expiry that voided it would be "quiet means dead" wearing a timestamp — and a lease over a merged branch is reported, never silently resolved — and the report is not a courtesy, it is the rung stating its own binding, because a top rung that claims every object and prints one indistinguishable line per claim cannot be told apart from one that is working. Each run states **how many leases it holds and how many of those the remote has already landed**, and the question is asked of every lease rather than of the ones whose tree reaches that rung — whether a lease is spent is a fact about the lease, not about which rung answers about the tree first, and asking at the rung undercounted 8 as 2 on a machine where five of the eight sat over trees carrying commits on no remote. **What ENDS a lease is the merge**: `--after-merge BRANCH` drops it, gated on the same un-forgeable key the reclaim rung rests on — the remote's own MERGED or CLOSED verdict for that exact branch, never the operator having named it, which only selects — and then re-runs every other rung, each of which still outranks. Before that, `tools/worktree-new.sh` claimed every tree it made and printed *release it at the merge*, nothing invoked `--release`, and the sentence was the whole mechanism: the top rung held all 36 leases on the machine and every rung below it was unreachable for those trees and for the 45 of 55 `target/` directories they hold, `--after-merge` included, since it was blocked by the lease dispatch had handed out. A **sweep still releases nothing, ever** — it reports and names the act. A **detached** tree has no branch for a remote to hold a verdict about, so no merge can end its lease and only an operator's `--release` does; a narrowed run says so rather than exiting zero having done nothing. **Build output is judged on every run, by liveness rather than by free space.** A `target/` holds no work — it is a regenerable cache and says so itself, through the `CACHEDIR.TAG` signature this tool requires before touching anything, which a directory holding real work cannot produce. So the loss is bounded at "a worker pays for a rebuild", and the key must ask the matching question: is anything going to build here again? The key it used to have was free disk space, which asks whether the machine is about to fall over — a different question, and it never opened: nineteen dead trees held 100 GiB with 258 GiB free, and the accumulation was found by a hand sweep. Lowering the number would only move the date. `decide_target` asks instead, in order: does a build hold the directory's lock — cargo takes an exclusive `flock` on `<target>/<profile>/.cargo-lock` for the whole of a build, and a running build cannot present its own lock as free, which is the one un-forgeable key at this layer, with an unopenable lock counted as HELD; then the same three liveness keys as the tree ladder (lease, inbound link, self), plus the **main checkout of each repository, protected because it is the donor `tools/worktree-new.sh` clones from**; then **LANDED** — the remote holds a MERGED or CLOSED pull request for the branch, which needs no threshold at all because the work is on the remote and this output rebuilds from it; and last **IDLE**, nothing touched under it for `--target-idle-hours` (default 72), for trees the remote holds no verdict on. Those last two are a disjunction, so the obligation is only as strong as IDLE, the weaker — which is why IDLE is reached only after the kernel has said no build is running, and why which arm applies is decided by the object (whether the remote holds a terminal state) and never chosen by whoever runs the tool. `--free-below` GiB (default 25) survives as an **alarm, not a gate**: below it the run additionally sizes and lists what it is protecting, so an operator in real trouble can see what is held and by which rung, and release it deliberately. Every deletion reports `df` recovery beside the `du` figure, because `du` counts a cloned block once per file that names it and cannot say what a deletion gives back — a large gap between the two means the output was sharing blocks with a tree that still holds them. Also reports the harness's `worktree-agent-*` branches already contained in `origin/main`. Default mode is a **dry run**; every run states how many repositories it enumerated, worktrees it examined, symlinks it resolved, and the reason per kept tree, and a run that examined nothing says so as a FINDING. Nothing in it ever changes directory — a `cd` in a compound command persists through the rest of it, which is how `gh` and `git worktree` have been made to answer confidently about the wrong repository |
...
| `validation/staging-admission.sh` | CI + agent (runs inside the compose `staging-admission` service) | `validation/staging-admission.sh <build-tree>` — refuses a build tree that carries no valid staging-gate admission token, or one minted for a DIFFERENT tree (it recomputes the `manifest.json` sha256). Announces an overridden admission loudly at boot, and a pre-detail blockout's admission with its OUT-OF-STAGE count — the walk's scope is stated where the session starts. Dependency-free bash+coreutils on purpose: it runs inside the delve image, which must never gain tooling (ADR-0003). Both 25565-publishing services `depends_on` it with `service_completed_successfully`, so compose will not start them when it exits non-zero — verified live: the server container stays `State=created`, never starts, and binds no port. |
| `validation/packtest-run.sh` | agent (**the PackTest ladder entry**) | `EULA=TRUE validation/packtest-run.sh --project dw-<id> [--output <tree>]` — the `packtest` profile end to end, same contract as `bot-run.sh`: `--project` REQUIRED, own teardown, exit code = failed tests. `--output` selects the build tree (default `./delve-output`), which is how CI proves per-campaign template classes hello-world cannot emit. **To run the whole surface, use `packtest-all.sh`** — this script runs ONE project, and choosing which is how a round covers a strict subset of the gate without noticing; `packtest-matrix.py` is the authoritative list of what the gate actually runs. There is no `PACKTEST_CONTAINER` any more — the runner pins no container name. It also calls `server-bootstrap-cache.sh` (idempotent) and **copies the bootstrap overlay into this project's world volume before booting**, so the suite performs no live Mojang/Fabric fetch — measured: the whole suite runs green under `--network none`. It then asserts that binding on the boot log (the locally-provisioned-launcher line present, no download lines) and reds if the seed missed, because a seed that silently missed would leave the ladder exactly as fragile while reporting success |
| `validation/packtest-all.sh` | agent (**required for any change to PackTest emission**) | `EULA=TRUE validation/packtest-all.sh [--prefix <id>] [--only <project>]` — runs **every** PackTest project the tier-2 gate runs, building each campaign's tree first. One command instead of a choice: a round that verified only the project it happened to find shipped a red on one it did not, and every project it ran was green. The matrix is DERIVED from `ci.yml` (see the row below), so a project added to the job is covered by the next local run with nothing to remember. States the project count before and after, and enforces an accounting identity — every row ran, failed or was explicitly skipped — so a row lost to a future `continue` cannot be absorbed into a green summary. `--only` prints what it skipped and **exits non-zero**, so a partial sweep can never be reported as a pass. Each project goes through `packtest-run.sh` and owns its own compose project |
| `validation/packtest-matrix.py` | agent + `packtest-all.sh` | `validation/packtest-matrix.py [--list] [--expect <n>]` — the enumeration of every PackTest project the gate runs, with its build tree, campaign, prefab dir and (where one exists) its prefab generator, read out of `.github/workflows/ci.yml` itself. **One authority, not two**: there is no second list to drift and no lockstep checker to own, which is the same "both walks derive from one traversal" rule `DW0497` states for emitters. A zero parse is a refusal, never an empty pass; `--expect` refuses when the job's count is not the one a caller believes it is covering; and a project whose build step it cannot find is a refusal rather than a dropped row. Run it bare to see the surface a PackTest-affecting change owes |
...
| `validation/fresh-volumes.sh` | agent | `validation/fresh-volumes.sh --project <compose-project>` — tear ONE compose project down and **prove** its containers, volumes **and networks** are gone. The network is the third class and the one that only ever bites the creator: a runner is fresh every job, a workstation is not, and leaked `<project>_default` networks accumulate until Docker's address pool is exhausted — at which point a ladder fails with `all predefined address pools have been fully subnetted`, on whichever project happens to be running, reported as a test failure. `--project` is REQUIRED: no default, and `COMPOSE_PROJECT_NAME` is deliberately not honoured, because an invisible default's cost is somebody else's live world. The old daemon-wide `--all` is GONE — it matched `server-data$` across every project and force-removed the pinned `delvewright-*` names, i.e. an outage rather than a teardown. It additionally refuses a project whose container publishes host 25565 (an owner-facing session, human possibly inside). Run it before any re-run of the bot ladder — the entry scripts do it for you: `docker compose -p <proj> … down -v` silently leaves `<proj>_server-data` behind whenever an exited container of that project still holds it, and the stale volume carries the scoreboard, so the re-run starts with objectives already complete and the bot reports a **false CONTENT failure** (three misattributed red runs, island round 13) |
...
| `validation/check-versions.sh` | CI (tier 1) | fails if any Dockerfile/compose/workflow disagrees with `versions.toml`. It is also the registered `bound_by` of every pin named by a version string: `[engine].rust_toolchain` (the channel, both published `rust-version` floors and the manifest), and `[ci]` + `[skin]` (the Node and Python lines every job selects, the mecha / beet / pytest instruments of the required checks, and the skin library's four statements — two manifests, the provenance stamp `delve_skin/catalog.py` writes into every emitted record, and the manifest). Those assertions state a declared COUNT as well as the value, because the drift is one of five statements moving while the other four sit still and a `grep -q` stops at the first and calls that agreement — so a new site for a pinned toolchain is a decision rather than a detail. It also holds the mecha cross-check's interpreter and its beet at or above the floors mecha itself declares, which is why the two Python lines in one workflow disagree: the higher one is entailed by the pinned validator, not chosen. `check-pins.py` verifies that binding, so dropping the key or a consumer path from this script reds the pin registry too |
...
keeps the gallery buildable from this repository alone, so its piece is
regenerated on every run and no `.nbt` is committed. It emits the piece's
**metadata beside its blocks**, which is what lets an anchor inventory be checked
against the geometry (`assert_anchors_are_standable`) instead of kept in sync by
a doc comment. And `--skins <dir>` emits the mannequin PNGs the gallery's
`skin.texture_id` declarations name, for the same reason: a generated byte is a
byte nothing can let drift.
...
(ADR-0006). `tools/check-structure-emitters.py` holds that job's enumerated lists
equal to the `prefabs/*/Cargo.toml` workspaces on disk, and separately proves the
derived `cargo fmt` sweep still reaches each of them, so a generator CI does not
run is a red.
