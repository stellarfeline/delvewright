...
| `tools/check-approval-gates.py` | CI | `python3 tools/check-approval-gates.py` — every job in `.github/workflows` that declares `environment:` must carry `tools/assert-run-approved.sh <its own environment>` as its **first** `run:` step, and must grant `actions: read`. Exists because the crates.io one-way door was vacuous on 2026-08-08: `publish-crates` declared `environment: crates-io`, was named "(owner approval)", and ADR-0017 §4 named that reviewer as THE control — but the environment had `protection_rules: []`, so a tag push published `delvewright-dsl 0.1.0` and `delvec 1.1.0` unreviewed, and crates.io versions cannot be deleted, only yanked. Everything that made the gate look real lived in the repo; the one setting that made it BIND lived in GitHub's settings, where nothing in the repo could see it — the same out-of-band shape as branch protection (`check-required-contexts.py`), one door further in. The obligation attaches to `environment:` itself rather than to the job that got burned, so a second gated job inherits it the day it is written. Reads only the repo, no token. States its binding count; **zero environment-gated jobs is a red** — it would mean the release workflow moved or the door lost its gate |
