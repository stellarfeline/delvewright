...
| [0016](0016-three-layer-versioning.md) | Three-layer versioning: format, engine, skill | Accepted; §Decision item 1 (old formats always compile) superseded by ADR-0024; item 2's tag shape and item 3's unreleased skill version superseded by ADR-0028 |
| [0017](0017-toolchain-distribution.md) | Toolchain distribution: `cargo install delvec`, release shelf, CI-only publishing | Accepted (§3 and the musl targets superseded by ADR-0023; install default superseded by ADR-0023; §4's mechanism half superseded by ADR-0026; §4's tag shape superseded by ADR-0028) |
...
| [0028](0028-three-things-released-by-name.md) | Three things, each released by its name and its version: `<name>--v<semver>` tags for `delvec`, `delvewright-dsl` and the `delvewright` plugin | Proposed |
