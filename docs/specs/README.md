...
| [spec-0071](spec-0071-a-creator-types-judgements-only.md) | A creator types judgements only — a purchase that does not add up is refused, an effect's happening is about the effect's object, and a translating agent hands over a table | Proposed |
| [spec-0073](spec-0073-a-boss-shows-its-health.md) | A boss shows its health — the health bar as a declared property of a wave or an actor: a named bar over the fight's total health for every player in range, refused where nothing can move it or title it, advised where a boss-billed fight has none | Draft |
| [spec-0075](spec-0075-a-shop-sells-enchanted-things.md) | A shop sells enchanted things — a handed-over stack carries its enchantments and a book stores them, a trap's trigger block stands at its cell, and a piece may carry an anvil and a trapped chest | Accepted |
| [spec-0074](spec-0074-a-kill-pays.md) | A kill pays — an `on_kill` bundle on a wave or an actor runs as the player credited with each body's death, with the creator stating whether a fight that comes back after a rest pays again | Draft |
