...
**No step checks something a later step supplies**, and the table's precondition
column is what that rule is held to: a check whose input arrives later belongs at
the step where the input arrives. Getting this wrong is not a cosmetic defect. A
stop that fires for a reason the page itself created is worse than no check,
because it is confidently wrong about the creator's machine — and the creator
cannot tell, because the tool's refusal is the same one it gives for the real
cause. The GPU arms are the case that proves it: `delvec render` draws with the
client jar, so asked before I5 it refuses with `DW0723` on every machine that has
never run this page, and `DW0723` is also what it says when the hardware genuinely
has no adapter.

**The skill root is a value, not an assumption.** Every bundled path — the pin
and the three scripts — is addressed from the directory that carries `SKILL.md`,
which is the plugin's installation directory in creator mode and
`"$DELVEWRIGHT_ENGINE/.claude/skills/delvewright/skills/new-delve"` in dev, and
in neither mode the working directory (which is where the *campaign* goes, §2).
No standard substitution reaches page content — `${CLAUDE_PLUGIN_ROOT}` resolves
inside `plugin.json`, in `command`, `args`, `env` and hook commands, and nowhere
else (§2b) — so I1 resolves it once, as `DELVEWRIGHT_SKILL`, from Claude Code's
own install register, and **refuses rather than guesses** when that register
names anything but exactly one Delvewright: an upgrade leaves the superseded
version in the cache beside the current one, so a glob over the cache resolves by
a name that is not unique and yields a candidate, not a match. This is the one
algorithm the page carries inline rather than in `scripts/`, because it is the
one that finds `scripts/`.
...
| **I3c · the binary, answering about itself** | I3a or I3b | `delvec --version` answers and the `dsl` number it prints is written down for step 1. The GPU arms are **not** asked here — their textures arrive at I5 — and the page says so, so that nobody restores the check without moving the jar | no answer at all: the install is broken, go back to I3a's table |
| **I4 · the environment** | I3 | `~/.delvewright/env.sh` written with `JAVA_HOME`, `DELVEWRIGHT_MODE`, `DELVEWRIGHT_ENGINE`, `DELVEWRIGHT_PYTHON`, `DELVEWRIGHT_SKILL`, `PATH` (the bin or the target directory first), and `DELVEWRIGHT_PREFABS` once I6 sets it; every later command on the page runs as `. ~/.delvewright/env.sh && <command>` | — |
| **I5 · the client jar** | I4 | the hand-over as today: download by default (`scripts/fetch-client-jar`, Mojang's manifest, the version the engine's `versions.toml [minecraft]` pins, the sha1 Mojang publishes checked on the bytes) or a copy from a directory the user names; lands at `~/.chunky/resources/minecraft.jar` and is looked at there, which is a postcondition this step's own inputs can carry. That the jar READS is I8's line: it takes `delvec`, a scratch directory and a piece `delvec grammar expand` writes into `.out/` — never a library piece — and the library is not named until I6 | a sha1 that does not match: refuse. A directory the user did not name: never searched |
...
| **I7 · named, not installed** | I6 | Chunky's source answers (`curl -I` on the launcher URL the engine's `versions.toml [render]` names); path A is decided by enumerating `campaigns/*/design/` against the campaign the user's prompt names, never by a campaign id (step 1 decides that one); on the drawing path, `refimg.py --dry-run` exits 0 against a `[refimg]` section in `"$DELVEWRIGHT_ENGINE/delvewright.local.toml"`, written now, whose `api_key_env` names a variable the machine already carries — and when it carries none, the page asks the user for a provider and a key by name and exports it from `env.sh`, because the agent's shell keeps nothing between commands; the skin toolchain is not mentioned here at all | Chunky unreachable: not a stop, said out loud, a stop at step 12. No provider: a stop on the drawing path before step 1 |
| **I8 · finished when** | I0–I7 | the checklist on the page answers, through `env.sh`: `java -version`, `echo "$DELVEWRIGHT_ENGINE"`, `delvec --version`, `delvec grammar list`, `delvec render fidelity-gate`, the texture ladder over a piece `grammar expand` writes, the Chunky probe, `docker info`. **This is where the GPU arms and the texture ladder are proved**, and it is the only step whose precondition holds every input they need; the page numbers it, and `references/init.md` gives each line its failure meaning | any line wrong: Init is not finished, and a run that continues authors against a half-built toolchain. `render fidelity-gate` says three different things here and the reference separates them: `DW0723 no textures` is I5 undone, any other `DW0723` is the hardware, `DW0720` at exit 4 is a jar that is not 1.21.11 |
