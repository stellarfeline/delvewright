...
| 12 | `happening` rides 11 of 26. The vocabulary anticipates consumers it was never given: `HappeningVerb` has `gains`/`loses` — "the party gains a thing" — while **`give-item` carries no `happening`**. The contradiction proof `DW0485` reasons only over beats that declare one. | `stages.rs:1146`, `3234` | L |
