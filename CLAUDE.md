...
tools/               # auxiliary Python/shell tooling — never shipped in delves; one
                     #   directory per reader: creator/ (what the /new-delve page runs),
                     #   ci/ (the gates), planner/ (dispatch, session state, numbering),
                     #   maintenance/ (regenerators a pin bump calls), lib/ (shared modules)
...
- **Every command's response is read.** Live commands go through the shared rejection rule (`tools/lib/rcon.{sh,mjs}`); emitted commands are checked against the pinned command tree by the emitter; both bound by `tools/ci/check-live-commands.py`. A shared rule is extracted, never copied per call site.
