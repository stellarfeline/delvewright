...
- **A bot plays it.** A mineflayer bot joins the shipped server and plays the critical path to the end: `critical path 'doune-castle-tour' PASSED (11 steps, 2 advisory finding(s))`. The two advisories say there is no combat and no death to test. At 8 of the guide's 9 conversations the bot also printed that it could not confirm which body it clicked (`acquisition unproven`).
- **The server log.** `shipped server log is error-free.`
- **Determinism.** Same documents and same seed, byte-identical output. The published image carries its datapack's digest as a label, `datapack-sha256=dba45276efbaabea53ee261489921ead3925d2b3363cf0155ed0a8548b5666e0`, so anyone who rebuilds can compare. The campaign's [generation record](https://github.com/stellarfeline/delvewright-campaigns/blob/release/doune-castle-tour/v1.0.0/campaigns/doune-castle-tour/GENERATION.md) records two builds of the tree as byte-identical on the engine revision it was authored against (`d1a83dbe`). On every pull request to this repository, CI builds a generated campaign on Linux and on macOS and refuses if one byte differs.
