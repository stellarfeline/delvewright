...
fn horizon_rules(c: &Campaign, d: &mut Vec<Diagnostic>) {
    ...
    if h.needs_v09() && !is_v09(c.world.dsl_version.as_str()) {
        d.push(Diagnostic::error(
            codes::RESERVED,
            "world",
            path,
            "this `horizon` form requires dsl_version 0.9.0 (spec-0026): below it, `horizon` \
             accepts only the strings `\"void\"` and `\"ocean\"`. Raise the world stage's \
             `dsl_version` to 0.9.0, or use one of those strings"
                .to_string(),
        ));
        return;
    }
        ...
        for (ei, edit) in batch.edits.iter().enumerate() {
            ...
            match edit {
                ...
                WorldEdit::Flood { region } => {
                    ...
                    if !is_v09(env.dsl_version.as_str()) {
                        d.push(Diagnostic::error(
                            codes::RESERVED,
                            stage,
                            epath.clone(),
