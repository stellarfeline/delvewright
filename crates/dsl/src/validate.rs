...
/// DSL v0.11 reserved-feature gating. **Two independent surfaces land in this
/// version**, and this is the one fence for both:
///
/// * the per-body `traversal` declaration (spec-0033) — what a body can do when
///   it moves — on the stage-2 NPC and the stage-5 actor;
/// * the **press-answer lift** — the `narrate` `actionbar` style and a trigger's
///   `audience: presser`. One fence for that pair, because they are one
///   capability: without the channel a press answer cannot be written where a
///   player reads replies, and without the addressee it is broadcast to a party
///   three quarters of whom are elsewhere. Together they are what makes
///   `close-gate.sealed_hint` expressible as an ordinary trigger.
///
/// **Fenced per stage, because the surface is per stage.** The stage-2 NPC's
/// traversal declaration is gated on the `npcs` document's own `dsl_version`;
/// the stage-5 actor's, and both halves of the press-answer lift, on the
/// `quests` document's. A campaign may therefore adopt the surface one stage at
/// a time — which is what keeps every 0.2–0.10 campaign compiling untouched.
///
/// This function is the **new-surface** half only: declaring any of it below
/// 0.11.0 is `DW0141`. `traversal` has no requirement half at all (the derived
/// class is exactly what every pre-0.11 build used, so a campaign that declares
/// none emits byte-for-byte what it emitted before); the press answer's
/// requirement half is `DW0429`, fenced on the code itself rather than here —
/// see [`press_obligation_checks`].
fn reserved_v11(c: &Campaign, d: &mut Vec<Diagnostic>) {
    ...
    if !is_v11(c.quests.dsl_version.as_str()) {
        ...
        // The press-answer lift. The style rides `for_each_campaign_effect`, so a
        // `narrate` nested inside a `sequence` step or an `on_arrive` bundle is
        // fenced exactly like a top-level one — the fence is as total as the
        // surface.
        crate::stages::for_each_campaign_effect(c, &mut |path, _site, eff| {
            if matches!(
                eff,
...
///
/// **Fenced on the code, not here.** This is a tightening, not new surface, and
/// the fence is what makes an already-approved design keep its verdicts: the
/// same declared version yields the same behaviour (owner ruling on version
/// semantics, 2026-08-11). Below 0.11.0 a silent door still compiles and still
/// emits nothing, and an unauthored `close-gate` still takes the compiler's
...
///
/// The version scope is task #51's general form:
/// [`codes::SEALED_BODY_UNANSWERED`] declares
/// [`Since(11)`](crate::Binds::Since) and [`crate::fence`] carries it, so this
/// walk runs **unconditionally** and reports what it finds. That is deliberate
/// and it is the stronger arrangement: a verdict is read off `Fenced`, and the
/// diagnostics the fence withholds are its binding count — a grandfathered set
/// that came out empty would say the proof examined nothing (CLAUDE.md: a green
/// gate that binds to nothing is vacuous, not a pass). A local `is_v11` guard
/// here would make that count permanently zero.
