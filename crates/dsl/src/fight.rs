...
/// Which of the two fight classes a [`Fight`] is — the value a consumer that
/// only needs the class (a diagnostic's word, a body tag's prefix) compares.
...
/// One fight the campaign declares: a wave or an actor. The one type every rule
/// over "a fight" reads — the `health_bar` (spec-0073) and the `on_kill`
/// (spec-0074) alike.
...
impl<'a> Fight<'a> {
    /// Which class this fight is.
    pub fn kind(&self) -> FightKind {
        match self {
            Fight::Wave(_) => FightKind::Wave,
            Fight::Actor(_) => FightKind::Actor,
        }
    }
    ...
    pub fn tier(&self) -> Option<EncounterTier> {
        match self {
            Fight::Wave(w) => w.tier,
            Fight::Actor(a) => a.tier,
        }
    }

    /// The fight's `health_bar`, if it declares one (spec-0073).
    pub fn health_bar(&self) -> Option<&'a HealthBar> {
        match self {
            Fight::Wave(w) => w.health_bar.as_ref(),
            Fight::Actor(a) => a.health_bar.as_ref(),
        }
    }
    ...
    pub fn own_name(&self) -> Option<&'a str> {
        match self {
            Fight::Wave(w) => match w.mobs.as_slice() {
                [only] => only.name.as_deref(),
                _ => None,
            },
            Fight::Actor(a) => a.name.as_deref(),
        }
    }
    ...
    pub fn entries(&self) -> usize {
        match self {
            Fight::Wave(w) => w.mobs.len(),
            Fight::Actor(_) => 1,
        }
    }

    /// Whether the fight's standing body can be hurt before any `unleash`: an
    /// actor's `vulnerable`; always for a wave, whose bodies are never
    /// invulnerable.
    pub fn vulnerable(&self) -> bool {
        match self {
            Fight::Wave(_) => true,
            Fight::Actor(a) => a.vulnerable,
        }
    }
