...
fn cast_validates_clean_under_v07() {
    let diags = check_campaign(&campaign_with(quests("0.24.0")));
    assert!(
        diags.is_empty(),
...
fn a_ledger_root_is_a_dialogue_entry_point() {
    ...
    let with_root = quests("0.24.0").replace(
    ...
    let mut orphan = campaign_with(quests("0.24.0"));
...
fn bark_lines_enter_the_l10n_inventory() {
    let barks = quests("0.24.0").replace(
        r#""dialogue": "dlg/greeting""#,
