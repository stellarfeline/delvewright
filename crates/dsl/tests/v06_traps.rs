...
fn v06_trap_validates_clean() {
    let diags = check_campaign(&campaign_with_traps("0.20.0", VALID_TRAP));
    assert!(
        diags.is_empty(),
...
fn v06_trap_bad_anchor_is_dw0340() {
    ...
    let diags = check_campaign(&campaign_with_traps("0.20.0", &trap));
    assert!(
...
fn a_disarm_flag_can_gate_the_rest_of_the_campaign() {
    ...
    let mut raw = campaign_with_traps("0.20.0", trap);
    ...
    let mut unproduced = campaign_with_traps("0.20.0", trap);
