...
fn campaign_with_traps(traps: &str) -> RawCampaign {
...
fn v06_trap_validates_clean() {
    let diags = check_campaign(&campaign_with_traps(VALID_TRAP));
    assert!(
        diags.is_empty(),
...
fn v06_trap_bad_anchor_is_dw0340() {
    ...
    let diags = check_campaign(&campaign_with_traps(&trap));
    assert!(
...
fn a_disarm_flag_can_gate_the_rest_of_the_campaign() {
    ...
    let mut raw = campaign_with_traps(trap);
    ...
    let mut unproduced = campaign_with_traps(trap);
