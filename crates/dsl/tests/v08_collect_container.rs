...
fn the_adoption_surface_validates_clean_at_v08() {
    let diags = check_campaign(&campaign_with(quests("0.21.0", ADOPTED)));
    assert!(
        diags.is_empty(),
...
fn item_name_enters_the_l10n_inventory() {
    let c = parse_campaign(&campaign_with(quests("0.21.0", ADOPTED))).expect("parses");
...
fn an_unadopted_collect_serializes_without_the_new_fields() {
    let c = parse_campaign(&campaign_with(quests("0.21.0", ""))).expect("parses");
