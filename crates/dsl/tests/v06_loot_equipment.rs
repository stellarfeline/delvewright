...
fn valid_loot_validates_clean() {
    let diags = check_campaign(&with_loot("0.23.0", VALID_LOOT));
    assert!(diags.is_empty(), "valid loot must be clean: {diags:#?}");
}

#[test]
fn a_fully_enchanted_actor_validates_clean() {
    let diags = check_campaign(&with_actor("0.23.0", VALID_ACTOR));
    assert!(
        diags.is_empty(),
...
fn plain_and_enchanted_slots_coexist() {
    assert!(check_campaign(&with_actor("0.23.0", VALID_ACTOR)).is_empty());
}
...
fn an_unknown_loot_item_is_dw0143() {
    ...
    assert!(codes(&with_loot("0.23.0", &l)).contains(&"DW0143".to_string()));
}

#[test]
fn a_loot_anchor_no_prefab_provides_is_dw0142() {
    let l = VALID_LOOT.replacen("anchor/exit", "anchor/nowhere", 1);
    assert!(codes(&with_loot("0.23.0", &l)).contains(&"DW0142".to_string()));
}
...
fn two_loot_entries_on_one_anchor_is_dw0435() {
    ...
    assert!(codes(&with_loot("0.23.0", &both)).contains(&"DW0435".to_string()));
}
...
fn more_stacks_than_a_container_has_slots_is_dw0432() {
    ...
    assert!(codes(&with_loot("0.23.0", &l)).contains(&"DW0432".to_string()));
}
...
fn an_unknown_enchantment_id_is_dw0433() {
    ...
    assert!(codes(&with_actor("0.23.0", &a)).contains(&"DW0433".to_string()));
}
...
fn the_curse_id_trap_is_dw0433_and_the_real_id_is_clean() {
    ...
    assert!(codes(&with_actor("0.23.0", &bad)).contains(&"DW0433".to_string()));
    let good = VALID_ACTOR.replacen("minecraft:protection", "minecraft:binding_curse", 1);
    assert!(check_campaign(&with_actor("0.23.0", &good)).is_empty());
}
...
fn a_zero_enchantment_level_is_dw0434() {
    ...
    assert!(codes(&with_actor("0.23.0", &a)).contains(&"DW0434".to_string()));
}
...
fn a_level_above_the_survival_max_is_allowed() {
    ...
    let diags = check_campaign(&with_actor("0.23.0", &a));
...
fn loot_enchantments_share_the_equipment_rules() {
    ...
    assert!(codes(&with_loot("0.23.0", l)).contains(&"DW0433".to_string()));
}
