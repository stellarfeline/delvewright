...
fn runtime_state_validates_clean() {
    let raw = campaign_with(&quests_doc("0.21.1"), None);
...
fn an_undeclared_datum_is_rejected() {
    let doc = quests_doc("0.21.1").replace(
...
fn datum_ids_follow_the_ordinary_id_rules() {
    ...
    let doc = quests_doc("0.21.1").replace(r#""id": "state/ride""#, r#""id": "state/Ride""#);
    assert!(
