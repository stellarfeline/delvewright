...
fn a_region_write_validates_clean_without_naming_a_gate() {
    let d = check_campaign(&raw(quests_doc("0.21.1", &format!("{FILL}, {CLEAR}"))));
...
fn unknown_fill_block_is_dw0193() {
    ...
    let c = codes(quests_doc("0.21.1", body));
    assert!(
...
fn a_region_write_carries_the_whole_gate() {
    ...
    let c = parse_campaign(&raw(quests_doc("0.21.1", body))).expect("campaign parses");
...
fn region_write_accessor_answers_for_both_verbs() {
    let c = parse_campaign(&raw(quests_doc("0.21.1", &format!("{FILL}, {CLEAR}"))))
