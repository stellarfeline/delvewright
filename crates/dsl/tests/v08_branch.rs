...
const NPCS: &str = r#"{
  "dsl_version": "0.22.1", "campaign_id": "hello-world", "stage": "npcs",
  "content": { "npcs": [
...
const DIALOGUE: &str = r#"{
  "dsl_version": "0.22.1", "campaign_id": "hello-world", "stage": "dialogue",
  "content": { "dialogues": [
...
fn green() -> Vec<Diagnostic> {
    diags(plan("0.22.1", POINTS), quests("0.22.1"), DIALOGUE)
}
...
fn malformed_branch_id_is_dw0110() {
    ...
    assert!(has(
        &diags(plan("0.22.1", &bad), quests("0.22.1"), DIALOGUE),
        "DW0110"
    ));
}
...
fn leads_to_without_a_known_prefix_is_dw0110() {
    ...
    let d = diags(plan("0.22.1", &bad), quests("0.22.1"), DIALOGUE);
    let hit = d
        .iter()
...
fn dangling_happening_subject_is_dw0112() {
    let bad =
        quests("0.22.1").replace("\"subject\": \"npc/keeper\"", "\"subject\": \"npc/nobody\"");
    let d = diags(plan("0.22.1", POINTS), bad, DIALOGUE);
    let hit = d
        .iter()
...
fn item_subject_is_a_free_namespace() {
    let ok = quests("0.22.1").replace(
        "\"subject\": \"npc/keeper\"",
        "\"subject\": \"item/warden-token\"",
    );
    let d = diags(plan("0.22.1", POINTS), ok, DIALOGUE);
    assert!(d.is_empty(), "{d:#?}");
}
