...
fn a_well_formed_economy_validates_clean() {
    assert!(
        codes(&campaign(DSL_VERSION, GOOD)).is_empty(),
        "{:#?}",
        validate_campaign(&campaign(DSL_VERSION, GOOD))
    );
}
...
fn dw0520_a_stake_needs_a_player_scoped_datum() {
    ...
    assert!(
        codes(&campaign(DSL_VERSION, &party)).contains(&"DW0520".to_string()),
    ...
    assert!(
        codes(&campaign(DSL_VERSION, &missing)).contains(&"DW0520".to_string()),
...
fn dw0521_drop_stake_must_name_a_declared_stake() {
    ...
    assert_ne!(bad, GOOD);
    let got = codes(&campaign(DSL_VERSION, &bad));
...
fn dw0522_a_stake_nothing_drops_is_a_finding() {
    ...
    let got = codes(&campaign(DSL_VERSION, &orphan));
...
fn dw0523_an_offer_that_cannot_answer_is_a_finding() {
    ...
    let got = codes(&campaign(DSL_VERSION, &inert));
    ...
    assert!(
        codes(&campaign(DSL_VERSION, no_offers)).contains(&"DW0523".to_string()),
...
fn dw0524_a_proportion_above_one_hundred_is_a_finding() {
    ...
    assert_ne!(over, GOOD);
    let got = codes(&campaign(DSL_VERSION, &over));
...
fn dw0527_a_gate_read_after_a_conditional_write_is_a_finding() {
    ...
    assert!(
        codes(&campaign(DSL_VERSION, &hazard)).contains(&"DW0527".to_string()),
        "purchase-then-apology is the hazard"
    );
    assert!(
        !codes(&campaign(DSL_VERSION, GOOD)).contains(&"DW0527".to_string()),
    ...
    assert!(
        !codes(&campaign(DSL_VERSION, sequenced)).contains(&"DW0527".to_string()),
...
fn a_shop_offer_is_a_gate_consumer() {
    ...
    let c = campaign(DSL_VERSION, GOOD);
...
fn every_new_player_visible_string_is_inventoried() {
    let mut c = campaign(DSL_VERSION, GOOD);
