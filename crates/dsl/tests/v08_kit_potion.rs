...
fn a_contents_less_potion_is_dw0487() {
    let c = classes_with("0.21.2", json!("minecraft:potion"), Value::Null);
...
fn every_potion_bearing_item_owes_contents() {
    for item in [
        ...
        let c = classes_with("0.21.2", json!(item), Value::Null);
        assert_code(&c, "DW0487");
    }
}
...
fn a_non_potion_kit_item_owes_no_contents() {
    let c = classes_with("0.21.2", json!("minecraft:bread"), Value::Null);
    assert!(
        codes_for(&c).is_empty(),
...
fn empty_contents_is_dw0486() {
    let c = classes_with("0.21.2", json!("minecraft:potion"), json!({}));
    assert_code(&c, "DW0486");
}
