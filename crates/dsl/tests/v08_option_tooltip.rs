...
fn an_option_tooltip_validates_clean_at_v08() {
    let d = check_campaign(&with_tooltip("0.21.2"));
    assert!(
        d.is_empty(),
...
fn an_option_tooltip_enters_the_l10n_inventory() {
    let c = parse_campaign(&with_tooltip("0.21.2")).expect("parses");
...
fn an_absent_tooltip_contributes_no_key() {
    let c =
        parse_campaign(&raw_with_dialogue(dialogue_with_tooltip("", "0.21.2"))).expect("parses");
    assert!(
