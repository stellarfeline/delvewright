//! DSL v0.9 (spec-0026 horizon library): the stage-1 `horizon` object form
//! `{base, …params}` and the new base/shorthand names.
//!
//! - The v0.6 strings (`"void"`/`"ocean"`) stay valid and byte-identical.
//! - The new surface is reserved (`DW0141`) below 0.9.0.
//! - `valley`/`summit`/`sky` (and `cherry-valley`) parse at 0.9.0 but are
...
/// A v0.9 stage-1 world document with a pluggable `horizon` value and an
/// always-on boundary.
...
/// The object form — even for a base 0.6 already ships — is v0.9 surface:
/// `DW0141` below it.
#[test]
...
/// A new shorthand name below 0.9.0 is `DW0141` too.
#[test]
...
/// `flatland` (with a boundary) is the landed new base: clean at 0.9.0, both
...
fn flatland_without_boundary_is_dw0320() {
    let diags = diags_for("0.9.0", "\"flatland\"", false);
    assert!(
