...
fn both_body_classes_accept_a_declaration_at_v11() {
    for locomotion in ["ground", "climber", "flier"] {
        let r = raw(npcs("0.21.1", Some(locomotion)), quests("0.21.1", None));
        assert!(
            !codes(&r).iter().any(|c| c == "DW0455"),
            "npc `{locomotion}`: {:#?}",
            check_campaign(&r)
        );
        let r = raw(npcs("0.21.1", None), quests("0.21.1", Some(locomotion)));
        assert!(
            !codes(&r).iter().any(|c| c == "DW0455"),
            "actor `{locomotion}`: {:#?}",
            check_campaign(&r)
...
fn declaring_aquatic_is_dw0455_on_either_body_class() {
    for (label, r) in [
        (
            "npc",
            raw(npcs("0.21.1", Some("aquatic")), quests("0.21.1", None)),
        ),
        (
            "actor",
            raw(npcs("0.21.1", None), quests("0.21.1", Some("aquatic"))),
        ),
    ] {
...
fn an_unknown_locomotion_or_field_does_not_parse() {
    for bad in [
        ...
        let r = raw(doc.to_string(), quests("0.21.1", None));
        assert!(
...
fn declaring_nothing_at_v11_raises_nothing() {
    let r = raw(npcs("0.21.1", None), quests("0.21.1", None));
    let d = check_campaign(&r);
    assert!(
