...
//! (`DW0367`), the empirical flood-level proof (`DW0364` — the #149 tide-mill
...
/// guess a tileset datum (the #149 folklore that flooded the tide mill).
#[test]
...
/// The #149 tide-mill class as a permanent fixture (spec-0026 acceptance
...
/// 0.9.0, ships the pinned bedrock/dirt/grass `generator-settings` with the
...
fn flatland_builds_on_the_datum_byte_identical() {
    ...
    let camp = campaign_with_horizon("flatland-camp", "0.9.0", "\"flatland\"");
...
/// string shorthand (`{base:"ocean"}` ≡ `"ocean"`), so the v0.9 surface adds
...
fn ocean_object_form_emits_byte_identical_to_the_string() {
    let pf = common::prefabs_dir();
    let camp_s = campaign_with_horizon("ocean-str", "0.9.0", "\"ocean\"");
    let camp_o = campaign_with_horizon("ocean-obj", "0.9.0", "{ \"base\": \"ocean\" }");
