...
/// The **fenced** verdict — what `delvec` reports and derives its exit code
/// from. `DW0429` is an obligation fenced on its own code
/// (`DwCode::since("DW0429", 11)`), so the raw list is an intermediate no user
/// ever sees: reading it here would report a 0.9 fixture for a rule it never
/// opted into.
...
fn a_pre_0_11_campaign_with_a_silent_door_still_compiles() {
    ...
    let fenced = diagnostics(&c);
    assert!(
        fenced.reported().is_empty(),
        "a pre-0.11 campaign is not held to the obligation: {:#?}",
        fenced.reported()
    );
    // The binding count. The assertion above would pass just as well if
    // `press_obligation_checks` had never looked at this campaign at all, and a
    // gate that binds to nothing is vacuous, not a pass (CLAUDE.md). What makes
    // it a proof is that the fence had a `DW0429` to withhold.
    assert!(
        fenced.grandfathered().iter().any(|d| d.code == "DW0429"),
        "the fence must have a DW0429 to grandfather here, or this test proves \
         nothing: {:#?}",
        fenced.grandfathered()
    );
    let out = build(&c);
    assert!(
