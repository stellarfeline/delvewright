...
/// **The test this whole feature exists to pass.** The spec-0026 flooded-interior fixture —
...
fn flood_cannot_silence_the_149_class() {
    ...
    let bare = campaign("flood-drowned-bare", Some("\"ocean\""), None);
    let b = build(&bare, &tmp("flood-drowned-bare-out"), &prefabs);
    let stdout = said(&b);
    assert_eq!(
        code(&b),
        3,
        "the flooded-interior fixture must be red:\n{stdout}"
    );
    ...
    assert_eq!(
        code(&d),
        3,
        "a declared flood must not make the flooded interior green:\n{stdout}"
    );
...
/// The v0.12 version fence (`DW0141`): declaring `flood` in a world-edits stage
