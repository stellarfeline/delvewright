...
/// (exit 3).
pub const DW_FLOOD_INERT: DwCode = DwCode::every_version("DW0394");
...
/// (exit 3).
pub const DW_FLOOD_ESCAPES: DwCode = DwCode::every_version("DW0395");
            ...
            return Err(EditError {
                ...
                message: format!(
                    "after world-edits batch `{bid}`: the water admitted by `flood` on region \
                     ...
                     that cell while every compile-time proof reads it as dry air (the flooded-interior \
