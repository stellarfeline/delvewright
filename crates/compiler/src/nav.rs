...
impl Ambient {
    ...
    /// spec-0026 to one branch per horizon base). Purely the generator's own
    /// facts: **where the content ends is not one of them** — that is
    /// [`built_volume`], a property of the assembled world under every horizon
    /// (see [`World::built`]).
...
impl World {
    ...
    /// [`Ambient::Void`] and [`Ambient::Flatland`], neither of which has water
    /// anywhere — flatland's pinned superflat is bedrock/dirt/grass, so a cell
    /// the content did not build is either that ground or open air.
    fn ambient_water(&self, c: [i32; 3]) -> bool {
        match &self.ambient {
            Ambient::Void | Ambient::Flatland => false,
...
///
/// This closes the `DW0344` gap that made that class invisible: `DW0344`
...
pub const DW_WALK_FLOODED: DwCode = DwCode::every_version("DW0364");
...
/// (the tide-mill class) an interior room floods on first boot. Cells the
    ...
    Err(NavError {
        ...
        message: format!(
            "flood-level proof (spec-0026): {total} standable cell(s) in placed pieces sit at or \
             ...
             (the flooded-interior class). First: area `{area_id}`, prefab `{}` placed at y={}:{listing}\n\
