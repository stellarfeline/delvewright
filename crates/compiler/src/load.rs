...
/// The five stage filenames a campaign directory must contain.
...
pub struct LoadedCampaign {
    /// The five raw stage documents, ready for `parse_campaign`.
    pub raw: RawCampaign,
    /// Filename → exact file bytes, for `manifest.json` input hashes.
    pub inputs: BTreeMap<String, Vec<u8>>,
}

/// Read all five stage files from `dir`. Fails if any is missing/unreadable.
