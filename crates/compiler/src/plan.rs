...
/// place the area with, and guessing one is how the flooded-interior flooded-interior
/// class happened. Build error (exit 3).
///
/// [`Binds::EveryVersion`]: this rule judges a document — the prefab library's
/// metadata and the geometry it assembles into — and every other rule over that
/// document (`DW0344`, `DW0346`, `DW0543`) is classified the same way. The cost
/// is real and is stated rather than hidden: a library that has not adopted the
/// datum reds an already-written campaign at its own declared `dsl_version`.
pub const DW_WALK_Y_MISSING: DwCode = DwCode::every_version("DW0367");
            ...
            PlanError::new(
                ...
                format!(
                    "area `{area_id}` is placed in a `{}` horizon, whose per-area datum is \
                     ...
                     (spec-0026 §2 — the flooded-interior class)",
                    h.base.token(),
                    meta.base(),
                ),
            )
...
/// are not island pieces and are not checked **here** — their walk cells are
/// still proved dry empirically by `DW0364` (`nav::check_flood_level`), which
/// has no exemption (spec-0026 §2, closing the gap this clause used to be).
///
        ...
        Some(Diagnostic::warning(
            ...
            format!(
                "the ocean-datum check examined ZERO of {placed} placed piece(s): this world \
                 ...
                 top authored water block, and `walk_y − 1` for a piece authored to a tileset \
                 convention. If this world really authors no shore, the per-area datum \
                 (`walk_ref_y ({walk_ref}) − walk_y`, spec-0026 §2) is the lever that lifts its \
                 pieces clear of the sea: raise their declared `walk_y` until no walk cell sits \
                 at or below y={SEA_LEVEL}, which `DW0364` proves empirically. A world that \
                 wants neither the shore nor the lift wants a horizon that is not `ocean`",
                placed = self.placed,
                reaching = self.reaching_sea,
                walk_ref = crate::horizon::OCEAN_WALK_REF_Y,
            ),
        ))
    }
}
...
impl<'a> Plan<'a> {
    ...
    pub fn build(campaign: &'a Campaign, prefabs: &PrefabRegistry) -> Result<Self, PlanError> {
        ...
        for (i, area) in campaign.world.content.areas.iter().enumerate() {
            ...
            // constant (the flooded-interior class).
            let origin = [
