...
/// those functions emit) and both halves of [`crate::flow`] — the producer scan
/// in `Flow::new` and the gate-flag inventory [`crate::flow::gate_flags`] (→
...
/// each. It is **not** finished: eight further campaign-wide walk sites (fourteen
/// distinct walkers) still enumerate three or four roots — see "Known spec ↔ code
/// drift" in `docs/reference/compiler.md` for the list and what each feeds.
