...
pub fn build_with_warnings(
    ...
    // spec-0026 §5 valley un-climbability proof (DW0369): a nav walk flood
    // from the gap floor must never stand outward of the crest line —
    // empirical assembled geometry (surround tiles included; edited model when
    // a stage-7 script exists, since an edit can carve steps), never a
    // slope-angle promise.
    if let Some(surround) = &plan.surround {
        ...
        if let Err(cell) = surround.valley.verify_unclimbable(&world) {
            return Err(BuildFailure::Diagnostic {
                code: crate::surround::DW_VALLEY_CLIMB,
                message: format!(
                    "the valley's inner slope grew a standable staircase: a walk from the gap \
                     floor stands at [{}, {}, {}], outward of the crest line — the surround no \
                     longer bounds the delve. The generator quantizes every surface to 2-block \
                     steps and keeps trees off the inner wall, so something later (an edit \
                     batch, a settle, a palette change) re-introduced a 1-block riser on the \
                     inner slope; remove it or move it off the surround",
    ...
    // several placed pieces — the insert is idempotent, same bytes). Includes
    // the horizon surround tiles (spec-0026 §5): generated bytes, same channel.
...
fn emit_functions(
    ...
    // Horizon biome paint (spec-0026 §1 biome layer): vanilla-native
    // `/fillbiome` over the surround's columns — the same channel vanilla uses
    // for grass/foliage tint, ambience and sky; no resource pack. Runs in
    // setup_finish because the chunks provably exist here (place_verify
    // gated). `max_block_modifications` (1.21.11's renamed
    // commandModificationBlockLimit, default 32768) is raised to the largest
    // band's volume for the pass and restored — a band is painted in ONE
    // command, so the paint can never be half-applied. Empty for a
    // surround-less horizon → byte-identical.
    if let Some(surround) = &plan.surround
        && !surround.biome.is_empty()
    {
        let volumes = surround.biome.iter().map(|r| {
            (r.max[0] - r.min[0] + 1) as i64
                * (r.max[1] - r.min[1] + 1) as i64
                * (r.max[2] - r.min[2] + 1) as i64
        });
        let limit = volumes.max().unwrap_or(0).clamp(32768, i64::from(i32::MAX));
