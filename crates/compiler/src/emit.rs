...
fn check_trigger_bodies(plan: &Plan) -> Result<crate::pressable::PressLedger, BuildFailure> {
    ...
    // The **emitted** trigger list, not the authored one: the compiler
    // synthesizes press answers of its own (`plan::press_answer_sites`), and a
    // trigger the build ships is a trigger the build must prove clickable.
