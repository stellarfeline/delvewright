...
| `DW0311` | (v0.4.1 addendum) The **critical path** has a consecutive pair of player-visited anchors that no walkable A* path connects over the assembled geometry, with no inter-area transport between them — the player would be stranded. This is the compile-time counterpart to the runtime critical-path bot: for each consecutive pair of visited steps (`talk`/`reach`/`kill`/`collect`/`interact`), the endpoints are snapped to standable floor and routed through `crate::nav`'s occupancy model (which treats every non-air block as an obstacle, so a plant curtain or eroded shell wedged across a doorway seam counts, exactly as it does for the bot). Legs the player rides across an inter-area teleport (a `critical_path_transport` hop) are skipped. Turns a whole bug class — a prefab regen that quietly wedged a mated socket seam shut or opened a floor into the void — from a bot-run surprise into a build failure. |
