//! `delve-schem` — Sponge schematic (`.schem`) -> vanilla structure `.nbt`.
//!
...
use delvewright_schem::diag::{DW_INPUT, Diagnostic};
...
#[command(
    name = "delve-schem",
    about = "Convert Sponge schematics (.schem, v2/v3) to vanilla structure .nbt",
    version
)]
struct Cli {
...
fn main() -> ExitCode {
    ...
    match cli.command {
        Command::Convert {
            ...
            split,
            palette_report,
        } => run_convert(&input, &out, split, palette_report, cli.json),
    }
}
