...
//!   `edit::hash01`) and [`crate::compiler::Splitmix64`]. No wall clock, no unseeded RNG,
...
impl ValleySurround {
    ...
    ///
    /// [`Plan::ground_columns`]: crate::compiler::plan::Plan
    ///
    ...
    pub fn verify_unclimbable(&self, world: &crate::compiler::nav::World) -> Result<(), [i32; 3]> {
...
/// fully determined by the seeded [`crate::compiler::Splitmix64`] stream.
fn poisson_columns(
    seed: u64,
    bounds: &SceneRect,
    r: f64,
...
pub fn generate_valley(
    ...
    let inner_seed = crate::compiler::edit::mix64(seed ^ 0xB105_50F7);
...
fn serialize_tile(
    ...
    let structure = Structure {
        data_version: crate::compiler::DATA_VERSION,
        size,
        palette,
        blocks,
        entities: vec![],
...
fn assert_inner_slopes_unclimbable(
    ...
    for (c, name) in tree_cells.iter().chain(decor_cells.iter()) {
        if crate::compiler::assembled::is_thin_decoration(name) {
