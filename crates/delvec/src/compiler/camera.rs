...
//! expansion. Shared between emission ([`crate::compiler::emit`]) and validation
//! ([`crate::compiler::nav`]) so the checks prove exactly the camera path the client
//! renders.
//!
...
/// A shot's full keyframe plan: what [`crate::compiler::emit`] turns into commands and
/// what [`crate::compiler::nav`] ray-checks.
#[derive(Clone, Debug, PartialEq)]
...
pub fn plan_shot(pts: &[[f64; 3]], subject: Option<[f64; 3]>, ticks: i32) -> ShotFrames {
    ExpandedShot {
        ...
        path: CameraPath::Polyline(if pts.is_empty() {
            vec![[0.0, crate::compiler::plan::BASE_Y as f64, 0.0]]
        } else {
            pts.to_vec()
        }),
        aim: match subject {
...
fn track_at(track: &[[f64; 3]], t: i32) -> [f64; 3] {
    if track.is_empty() {
        return [0.0, crate::compiler::plan::BASE_Y as f64, 0.0];
    }
...
pub fn expand_shot(
    ...
    let aim = match &shot.look_at {
        Some(t) => AimTrack::Static(crate::compiler::nav::camera_look_point(plan, t)),
        None => match &subject {
    ...
    if !shot.path.is_empty() {
        return ExpandedShot {
            ticks,
            path: CameraPath::Polyline(crate::compiler::nav::camera_points(plan, &shot.path)),
    ...
    let Some(style) = shot.shot_style else {
        ...
        return ExpandedShot {
            ticks,
            path: CameraPath::Polyline(vec![[0.0, crate::compiler::plan::BASE_Y as f64, 0.0]]),
            aim,
        };
    };
...
fn subject_pos(subject: &Option<SubjectGeom>, t: i32) -> [f64; 3] {
    match subject {
        ...
        None => [0.0, crate::compiler::plan::BASE_Y as f64, 0.0],
    }
}
...
fn resolve_subject(
    ...
    let (is_actor, id, offset) = match subject {
        CameraSubject::Anchor(s) => {
            return SubjectGeom::Static(crate::compiler::nav::anchor_offset_point(
                plan,
                s.anchor.as_str(),
                s.offset,
            ));
...
pub fn cutscene_units(
    ...
    // walks them".
    let mut out = Vec::new();
    crate::compiler::plan::for_each_effect_root(campaign, &mut |_site, list| {
        scan(list, &[], &mut out);
    });
    out
}
...
/// ([`crate::compiler::render_plan`], `yaw = atan2(-dz, dx)`, `0` = +X): pitch agrees, yaw
