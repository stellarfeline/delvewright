...
//! (`DW0410`).
//!
//! `crate::compiler::nav`'s existing seal model reasons about the **quest DAG**: which
//! gates are shut while the *player* walks a critical-path leg, rooted in
//! objective causality ([`crate::compiler::plan::Plan::gate_fired_before`]). That model
...
//! timeline is:
//!
//! * one **effect root** — every list [`crate::compiler::plan::for_each_effect_root`]
...
//! ## No false certainty
//!
//! The stance is [`crate::compiler::continuity`]'s: state only what the structure proves.
//!
...
/// [`crate::compiler::plan::for_each_effect_root`]'s five roots in their fixed order, each
/// list in declaration order, each effect yielded before its own nested effects.
/// [`crate::compiler::nav::all_effects`] is defined as this walk with the states dropped,
...
/// [`crate::compiler::plan::EffectRoot`] plays in ([`crate::compiler::plan::effect_root_area`]) —
...
/// destination the same [`crate::compiler::plan::BodyScope::Beat`] scope the cast
/// ledger's per-beat station already asks [`crate::compiler::plan::body_station`] for,
...
fn walk_campaign_with_beat_area<'a>(
    ...
    // [`crate::compiler::plan::for_each_effect_root`] fixes. They are concatenated to match
...
/// gate effects carry no area, exactly as `crate::compiler::plan`'s collector resolves them.
