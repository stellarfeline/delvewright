...
//! [`crate::compiler::clearance`] proves where a body IS; this module proves what a body
//! DID to get there.
//!
...
//!   up (a step-up's [`crate::compiler::nav::resample`] L is a vertical translation in
...
//!
//! [`crate::compiler::nav::World::standable_fp`] requires **solid** ground below, and
//! [`crate::compiler::nav::World::is_occupied`] counts a 1.5-tall barrier as impassable, so
...
//!    right-click ([`crate::compiler::nav::World::without_gate_use`] exists precisely
...
//! campaign has any NPC or actor ([`crate::compiler::clearance::has_bodies`]) — exactly the
...
//!   from the vendored registry ([`crate::compiler::registry::entity_in_tag`]), never a
...
//! ([`crate::compiler::nav::check_critical_path`]) stays visibly the one caller that has it.
//!
//! **What is deliberately NOT modelled**: per-entity jump reach. The router
//! measures every rise against the *player's* apex
//! ([`crate::compiler::nav::MAX_JUMP_RISE_16`]'s 1.25 blocks); a sheep matches it, but a
...
use crate::compiler::nav::{ActorMovePlan, MovePlan, World};
...
/// Vanilla's own aquatic tag, vendored (`crate::compiler::registry::entity_tags`).
...
impl Traversal {
    ...
    pub fn of_entity(entity: &str) -> Traversal {
        let id = crate::compiler::registry::namespaced_entity(entity);
        let locomotion = if CLIMBERS.contains(&id.as_str()) {
            Locomotion::Climber
        } else if FLIERS.contains(&id.as_str()) {
            Locomotion::Flier
        } else if crate::compiler::registry::entity_in_tag(&id, AQUATIC_TAG) {
            Locomotion::Aquatic
        } else {
            Locomotion::Ground
        };
...
fn barrier_course(world: &World, support: [i32; 3]) -> Option<[i32; 3]> {
    if world.solid_top_16(support) != Some(crate::compiler::assembled::FULL_HEIGHT_16) {
        return None;
    }
    [(-1, 0), (1, 0), (0, -1), (0, 1)]
        .into_iter()
...
fn declared_bodies(plan: &Plan) -> BTreeMap<String, DeclaredBody> {
    ...
    for site in delvewright_dsl::body_traversal_sites(plan.campaign) {
        ...
        let entity = match site.body {
            BodyRef::Npc(n) => crate::compiler::nav::npc_body_entity(n),
            BodyRef::Actor(a) => crate::compiler::nav::actor_body_entity(a),
        };
