...
//!   ([`crate::compiler::nav::entity_dims`] — the one dims table in the compiler), or the
...
//! will not guess one; [`crate::compiler::continuity`] takes the same no-false-certainty
...
//! [`crate::compiler::blockout`]'s `footing` and `station_cell` — for a reason that
...
use crate::compiler::nav::entity_dims;
...
fn bodies(plan: &Plan) -> Vec<Body> {
    ...
    for (i, n) in c.npcs.content.npcs.iter().enumerate() {
        ...
        let entity = crate::compiler::nav::npc_body_entity(n);
        out.push(Body {
            kind: "npc",
...
/// [`crate::compiler::affordance`], which `DW0420`/`DW0421` read for visibility and
...
/// ([`crate::compiler::teleport`]) and by [`affordance_cells`] — so an affordance added to
...
/// needed a test: it was registered with `crate::compiler::affordance` (visibility and
...
pub(crate) fn affordances(plan: &Plan) -> Vec<Affordance> {
    ...
    // on ground the runtime does not rewrite (`crate::compiler::stake`, `DW0525`/`DW0526`).
    out
}
...
/// [`crate::compiler::emit::SEAL_MARGIN`] on every side so the entity is strictly nearer
