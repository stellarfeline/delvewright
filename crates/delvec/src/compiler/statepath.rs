...
//! has an order: [`crate::compiler::flow::Flow`]'s path replay. So this rule is not a new
...
//! as [`crate::compiler::analyze`] is the diagnostic surface over the reachability
//! fixpoint.
//!
//! ## What is walked
//!
...
//! ## Where this runs
//!
//! Beside [`crate::compiler::promise::check`] and [`crate::compiler::branch::check_branches`], in
...
use crate::compiler::flow::{DW_STATE_GATE_CLEARED, Flow, StateWalk};
...
pub fn check(c: &Campaign) -> (Vec<Diagnostic>, StatePathBinding) {
    ...
    let mut push =
        |flow: &Flow<'_>, path: &crate::compiler::flow::Playthrough, branch: Option<&str>| {
            ...
            for f in found {
                ...
                d.push(Diagnostic::error(
                    ...
                    "quests",
                    crate::compiler::analyze::objective_path(c, &f.objective),
                    f.message(),
                ));
            }
        };
