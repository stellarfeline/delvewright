...
//!
//! Hand-rolled for the same reason [`crate::compiler::resourcepack`]'s ZIP/SHA-1 is: the
...
//!   all. Used by the `delve:art` font atlas ([`crate::compiler::atmos`]), whose PNG bytes
...
//!   level. Used by the visual-authoring-loop renders ([`crate::compiler::snapshot`]),
