...
//! pillar 3).
//!
//! The viewport ([`crate::compiler::snapshot`]) answers "what does it look like from
...
//! height — flat-shaded with [`crate::compiler::snapshot::block_color`] and lightened with
...
//! arrive pre-sorted from [`crate::compiler::snapshot::collect_targets`], label placement
...
use crate::compiler::snapshot::{Target, block_color};
...
fn area_walkable(
    ...
    let starts: Vec<[i32; 3]> = plan
        ...
        .map(|(_, r)| match r {
            crate::compiler::plan::ResolvedAnchor::Point { pos, .. } => *pos,
            crate::compiler::plan::ResolvedAnchor::Gate { from, to, .. } => [
                (from[0] + to[0]) / 2,
                (from[1] + to[1]) / 2,
                (from[2] + to[2]) / 2,
            ],
...
fn render_slice(
    ...
    let png = crate::compiler::png::encode_rgba(width, height, &c.rgba);
    Slice {
