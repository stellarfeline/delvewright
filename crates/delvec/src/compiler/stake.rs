...
//!
//! * **walkable** is [`crate::compiler::nav::World`], the same navigation model the
...
//!   a deliberate ruling rather than an omission. Closed by `DW0545`, one
//!   layer away from this file.** A teleport moves *entities*, not blocks, so
...
//!   [`crate::compiler::emit`]'s `stk_gc_<s>` finds nobody holding a wager at the marker's
...
//!   ([`crate::compiler::affordance`], `DW0545`). **This module holds no part of that
...
//!   [`crate::compiler::emit`]'s `stk_fill_<s>`, where the marker is made.
//!
...
pub fn runtime_mutable_regions(plan: &Plan) -> Vec<LabelledBox> {
    ...
    for ((area, name), resolved) in &plan.anchors {
        if let crate::compiler::plan::ResolvedAnchor::Gate { from, to, .. } = resolved {
            push(
    ...
    for (eff, _) in crate::compiler::timeline::walk(plan) {
...
fn unsafe_footing(regions: &[LabelledBox]) -> BTreeSet<[i32; 3]> {
    ...
    for (_, r) in regions {
        for cell in crate::compiler::assembled::region_cells(r.0, r.1) {
            out.insert(cell);
...
/// [`crate::compiler::lethal::check_respawn_seats`] takes it.
///
/// Public because [`crate::compiler::deathplan`] hands the same list to the bot tier: "the
...
pub fn seats(plan: &Plan, world: &World, entry: Option<[i32; 3]>) -> Vec<Seat> {
    ...
    if let Some(pos) = entry
        && let Some(cell) = world.snap(pos, crate::compiler::nav::SNAP_RADIUS)
    {
        out.push(Seat {
            cp: -1,
    ...
    for cp in &plan.checkpoints {
        let Some(cell) = world.snap(cp.pos, crate::compiler::nav::SNAP_RADIUS) else {
...
pub fn build(
    ...
    for seat in &seats {
        let (n, r) = crate::compiler::nav::reachable_under_every_quest_state(
            plan,
            world,
            seat.cell,
            seat.from_step,
