...
//! perspective viewport ([`crate::compiler::snapshot`]) and the orthographic blocking
//! chart ([`crate::compiler::blocking`]) need — alpha blending, rectangles, and bitmap
...
impl Canvas {
    ...
    ///
    /// Glyphs come from [`crate::compiler::atmos::glyph`] — the same original bitmap font
    ...
    pub fn stamp_text(&mut self, x: i64, y: i64, text: &str, scale: i64, color: [u8; 3]) {
        ...
        for ch in text.chars() {
            if ch != ' '
                && let Some(g) = crate::compiler::atmos::glyph(ch)
            {
