...
fn prop<'a>(name: &'a str, key: &str, default: &'a str) -> &'a str {
    crate::compiler::assembled::state_value(name, key).unwrap_or(default)
}
...
/// `emission_matches_the_pinned_game` (`crates/delvec/tests/emission_table.rs`),
/// over `crates/delvec/tests/fixtures/light/emission-1.21.11.tsv` — a dump of
/// `BlockState.getLightEmission()` for all 29,671 blockstates of the pinned
/// 1.21.11 server jar, regenerated by `tools/maintenance/dump-block-light.py`. The test
...
impl LightModel {
    ...
    /// assembled-world model ([`crate::compiler::assembled`]): placed pieces, solver seals,
    ...
    ///
    /// Public because the daylight-burn proof ([`crate::compiler::daylight`], `DW0496`)
...
pub fn relight(plan: &Plan, structures: &BTreeMap<String, Vec<u8>>) -> Relight {
    relight_over(
        plan,
        &crate::compiler::assembled::assemble(plan, structures),
    )
}
...
mod tests {
    ...
    /// ([`crate::compiler::assembled::occupancy_of`]), so nav-passable blocks (pressure
    ...
    fn nav_occ_of(map: &BTreeMap<[i32; 3], String>) -> World {
        World::from_occupancy(
            crate::compiler::assembled::occupancy_of(map.clone(), &BTreeSet::new()),
            crate::compiler::nav::Premises::geometry_only(),
        )
    }
    ...
    /// [`crate::compiler::assembled::occupancy_of`] leaves a block's cell player-occupiable
