...
impl World {
    ...
    /// route.
    fn modelled_seals(&self) -> impl Iterator<Item = &crate::compiler::assembled::GateSeal> {
    ...
    fn gate_seals_over(&self, cells: &[[i32; 3]]) -> Vec<&crate::compiler::assembled::GateSeal> {
        self.modelled_seals()
            .filter(|s| {
                let (lo, hi) = s.region;
                cells
...
mod tests {
    ...
    fn premise_declines_are_enumerated() {
        ...
        // Order by file name, as EXPECTED is.
        found.sort();
    ...
    fn aabb_clips_body(world: &World, p: [f64; 3], width: f64, height: f64) -> bool {
        let span = |c: f64| {
            (
