...
//!
//! [`crate::compiler::eclipse`] (`DW0359`) is the closest rule — a body in front of a
...
//! ([`crate::compiler::cast`], spec-0020) makes every quest declare, for every live NPC,
...
//!   is a crosshair target. Width comes from [`crate::compiler::nav::entity_dims`] over
//!   the body that actually ships ([`crate::compiler::nav::npc_body_entity`] — a skinned
...
//! non-pickable to let clicks pass through it — [`crate::compiler::eclipse`] states the
...
use crate::compiler::nav::{entity_dims, npc_body_entity};
...
pub fn check_crosshair_contests(plan: &Plan) -> Result<Vec<Diagnostic>, Failure> {
    ...
    for qid in crate::compiler::cast::quest_dag_order(c) {
        let Some((qi, quest)) = c
            .quests
            .content
            .quests
