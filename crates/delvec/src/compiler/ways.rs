...
//!   (`crate::compiler::faces`, `DW0780`) and spec-0042 keeps ways off them entirely;
...
///
/// Reads the same `spatial_contract` block `crate::compiler::faces` reads, through the
        ...
        for (index, placement) in area.pieces.iter().enumerate() {
            ...
            for edge in &contract.edges {
                ...
                for b in &way.boxes {
                    ...
                    entry.cells += crate::compiler::assembled::region_cells(lo, hi).count();
...
/// A local cell, placed and rotated — the transform `crate::compiler::faces` and
/// `crate::compiler::solver` both use, so a way's cells land on the piece's blocks.
