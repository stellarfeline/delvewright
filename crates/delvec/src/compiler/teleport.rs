...
//! and five machinery types by name ([`crate::compiler::emit`]'s `LETHAL_EXEMPT_TYPES`),
...
//! the engine.
//!
//! ## The one narrowing, and why it is not an exemption list (`DW0545`)
//!
...
//! it: *my position IS engine state*. See [`crate::compiler::affordance`] for the class and
...
//! [`crate::compiler::eclipse::affordances`], the same single authority `DW0359` measures
//! bodies against, plus the seal shells `close-gate` arms
//! ([`crate::compiler::plan::SealHintPlan::shell_cells`], which `DW0422` owns). A future
...
//! from the selector instead (`DW0545`). The split is the whole design: **a place
...
pub fn check_bound_affordances(plan: &Plan) -> Result<TeleportGate, Failure> {
    ...
    let mut posts: Vec<(&'static str, String, [i32; 3])> =
