...
//!    position [`crate::compiler::continuity`]'s replay actually produces when the quest
...
//!    [`crate::compiler::continuity`]'s stance that an indeterminate history is reported,
//!    never guessed at.
//!
...
/// [`crate::compiler::branch`]'s `DW0483` and [`station`] both read it, so "which
...
/// ([`crate::compiler::gates::certain_anchor_providers`]) — a bare `prefab` that declares
...
/// cannot make — [`crate::compiler::gates::anchor_ambiguity_remedy`] is the one writer for
    ...
    for (qi, q) in c.quests.content.quests.iter().enumerate() {
        ...
        for (npc, entry) in &q.cast {
            ...
            for (i, p) in placements.iter().enumerate() {
                ...
                let providers =
                    crate::compiler::gates::certain_anchor_providers(c, prefabs, anchor.as_str());
                ...
                diags.push(Diagnostic::error(
                    ...
                    format!(
                        ...
                        remedy = crate::compiler::gates::anchor_ambiguity_remedy(&named),
                    ),
                ));
            }
        }
...
/// [`crate::compiler::plan::build_critical_path`], which is the only thing in the compiler
...
/// Stage-5 quest ids in the same DAG linearization [`crate::compiler::continuity`] replays,
