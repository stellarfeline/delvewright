...
//! ([`crate::compiler::camera::stand_in_open_air`], the mechanism `delvec snapshot --at`
...
//! every corner-thinned critical-path waypoint (the same [`crate::compiler::waypoints::thin`]
...
//! in open air; the DW0724 self-check ([`crate::compiler::nav::verify_camera_eyes`]) enforces
...
use crate::compiler::nav::{CameraEye, LegRoute, World};
...
impl<'c> Shots<'c> {
    ...
    fn push(&mut self, shot: Shot) {
        ...
        let eye = if shot.kind == "pov" || self.world.is_clear(eye_cell(requested)) {
            ...
            crate::compiler::camera::stand_in_open_air(
...
pub fn render_plan(
    ...
    crate::compiler::nav::verify_camera_eyes(world, &out.eyes)?;
    let mut warnings = Vec::new();
    if out.eyes.is_empty() {
        warnings.push(Diagnostic::warning(
            crate::compiler::nav::DW_CAMERA_EYE_OCCLUDED,
            "world",
            "/shots",
...
fn horizon_fact(c: &Campaign, plan: &Plan) -> Option<Value> {
    ...
    match r.base {
        HorizonBase::Ocean => Some(json!({
            "kind": "ocean",
            "sea_level": crate::compiler::plan::SEA_LEVEL,
        })),
        ...
        HorizonBase::Valley => Some(json!({
            "kind": "valley",
            "gap_floor_y": crate::compiler::horizon::VALLEY_GAP_FLOOR_TOP_Y,
