...
//!   endpoints to a standable cell ([`crate::compiler::nav::World::snap`], `SNAP_RADIUS`)
//!   and A* only ever steps through passable cells, so a walk *cannot* end
//!   inside a wall.
//! * A **placed** body is not. [`crate::compiler::nav::check_actor_placement`] (`DW0325`)
...
//!
//! `DW0359` ([`crate::compiler::eclipse`]) is the neighbouring proof and does not cover
...
//! ## The model
//!
//! * **Body.** The entity's standing hitbox from [`crate::compiler::nav::entity_dims`] —
...
//!   because [`crate::compiler::nav::plan_moves`] / [`crate::compiler::nav::plan_actor_moves`]
...
//!   ([`crate::compiler::nav::World::solid_top_16`]). Water is not geometry — a body in
...
use crate::compiler::nav::{ActorMovePlan, BARRIER_HEIGHT, MovePlan, World, entity_dims};
...
fn npc_body(plan: &Plan, npc_id: &str) -> Option<(String, String)> {
    plan.campaign
        ...
        .map(|(i, n)| {
            (
                crate::compiler::nav::npc_body_entity(n),
...
fn actor_body(plan: &Plan, actor_id: &str) -> Option<(String, String)> {
    plan.campaign
        ...
        .map(|(i, a)| {
            (
                crate::compiler::nav::actor_body_entity(a),
...
mod tests {
    ...
    /// An [`crate::compiler::assembled::Occupancy`] with only its `solid` set filled.
    fn occ(solid: BTreeSet<[i32; 3]>) -> crate::compiler::assembled::Occupancy {
    ...
    fn a_bottom_slab_below_the_feet_is_not_an_obstruction() {
        ...
        let world = World::from_occupancy(o, crate::compiler::nav::Premises::geometry_only());
        let v = giant_at([3, 69, 3]);
        let (w, h) = entity_dims(&v.entity);
        assert!(verdict(&world, &v, w, h).is_none());
    }
