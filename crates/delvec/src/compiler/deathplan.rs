...
//! runtime check on: [`crate::compiler::stake`]'s placement table, as (seat, region) → anchor.
//!
...
fn currency(c: &Campaign, state_id: &str) -> Value {
    ...
    json!({
        ...
        "objective": crate::compiler::plan::state_score(state_id),
...
///
/// `table` is [`crate::compiler::stake::build`]'s answer, `None` for a campaign that declares
...
pub fn build(
    ...
    // same way when there is not, which is [`crate::compiler::stake::seats`]'s whole reason
    // for being public.
    let seats_src = match table {
        Some(t) => t.seats.clone(),
        None => crate::compiler::stake::seats(plan, world, entry),
    };
