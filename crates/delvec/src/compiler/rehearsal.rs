...
//! [`crate::compiler::nav::anchor_offset_point`] resolves it to that cell's **centre**
...
use crate::compiler::camera::{self, AimTrack, MoveCtx};
...
fn bundles(campaign: &Campaign) -> Vec<(String, &[QuestEffect])> {
    ...
    crate::compiler::plan::for_each_effect_root(campaign, &mut |site, list| {
...
///
/// A world point `p` produced by [`crate::compiler::nav::anchor_offset_point`] is exactly
...
fn default_path(expanded: &camera::ExpandedShot) -> Vec<[i32; 3]> {
    ...
    if pts.is_empty() {
        return vec![[0, crate::compiler::plan::BASE_Y, 0]];
    }
