...
//! [`crate::compiler::emit::SEAL_MARGIN`] so it protrudes past the block it stands in
//! ([`crate::compiler::plan::SealHintPlan`]). That machinery was never available to
...
//! ([`crate::compiler::emit::env_trigger_setup`]) and the collision proof
//! ([`crate::compiler::eclipse`]) read it, so the two can no longer disagree about whether
...
///    body (see [`crate::compiler::wrongside`] for why that placement is also the side
///    test);
...
/// knows the kind ([`crate::compiler::emit`]).
