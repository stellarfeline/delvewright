...
//! 4. **Boundary safety** (`DW0322`, [`crate::compiler::nav::verify_boundary_safety`]):
...
use crate::compiler::assembled::{self, Assembled};
use crate::compiler::plan::{Plan, ResolvedAnchor};
use crate::compiler::solver::stream_seed;
...
pub const DW_EDIT_UNRESOLVED: DwCode = DwCode::new("DW0323", ExitTier::Build);
...
pub const DW_EDIT_TRAP_HARDWARE: DwCode = DwCode::new("DW0352", ExitTier::Build);
...
pub const DW_EDIT_GATE_REGION: DwCode = DwCode::new("DW0353", ExitTier::Build);
...
pub const DW_EDIT_SUPPORT: DwCode = DwCode::new("DW0354", ExitTier::Build);
...
pub const DW_PLACEMENT_SHORTFALL: DwCode = DwCode::new("DW0864", ExitTier::Build);
    ...
    for batch in &env.content.batches {
        ...
        for (ei, edit) in batch.edits.iter().enumerate() {
            ...
            match edit {
                ...
                // (`crate::compiler::massing`, inside `Plan::build`) — the assembly this
...
fn check_batch_invariants(
    ...
    if let Some(e) = crate::compiler::nav::measure_fluid_escape(&with_fixtures).finding() {
        return Err(ctx(e));
    }
    let starts = anchor_starts(plan);
    crate::compiler::nav::verify_boundary_safety(&with_fixtures, &starts).map_err(ctx)?;
    Ok(())
}
...
/// [`crate::compiler::nav::AnchorRoot`]). `plan.anchors` is a `BTreeMap`, so the order here
/// is deterministic (ADR-0006).
pub fn anchor_starts(plan: &Plan) -> Vec<crate::compiler::nav::AnchorRoot> {
    plan.anchors
        .iter()
...
/// side set [`crate::compiler::assembled::occupancy_of`] reads to tell a *closed* fence
...
fn resolve_shape(
    plan: &Plan,
    area: &crate::compiler::plan::AreaPlacement,
    bid: &str,
...
fn fragment(
    ...
    let rot = match rotation {
        delvewright_dsl::FragmentRotation::None => crate::compiler::solver::Rotation::None,
        delvewright_dsl::FragmentRotation::Clockwise90 => crate::compiler::solver::Rotation::Cw90,
        delvewright_dsl::FragmentRotation::Clockwise180 => crate::compiler::solver::Rotation::Cw180,
        delvewright_dsl::FragmentRotation::Counterclockwise90 => {
            crate::compiler::solver::Rotation::Ccw90
        }
    };
...
fn relight_region(
    ...
    let sky = crate::compiler::light::darkest_effective_sky(c);
    ...
    let reachable: BTreeSet<[i32; 3]> = nav
        ...
        .into_iter()
        .filter(|cell| crate::compiler::light::in_bounds(*cell, amin, amax))
        .collect();
