...
//! provider scale ([`crate::compiler::atmos::ART_SCALE`]). Those two multiply — which is why
...
//! sentence. [`crate::compiler::emit`]'s `build_node_dialog` emits each node as a
...
use crate::compiler::atmos::{ART_GLYPH_ADVANCE, ART_SPACE_ADVANCE};
...
/// [`crate::compiler::emit`]'s `build_node_dialog` emits no `width` — so this is the width of
