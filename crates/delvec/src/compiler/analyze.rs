...
//! ## Model
//!
//! The reachability fixpoint itself lives in [`crate::compiler::flow`], which solves it
...
//! to the compiler's assembled-world light model ([`crate::compiler::light`]), which measures
...
//! ([`crate::compiler::flow::Flow::divide`]).
//!
...
pub fn analyze_campaign(c: &Campaign, prefabs: &dyn AnchorRegistry) -> Vec<Diagnostic> {
    ...
    if !completed.contains(finale) {
        ...
        if let Err(f) = flow.replay(&path) {
            ...
            let n = crate::compiler::plan::min_players(c) as usize;
            if n >= 2
    ...
    // compiler's assembled-world light model (`crate::compiler::light`), which measures real
