...
/// already claims it.
///
/// Takes the resolved-anchor map rather than the [`crate::compiler::plan::AnchorTable`]
...
pub fn container_anchors(
    ...
    for ((area, name), resolved) in anchors {
        let cell = match resolved {
            ...
            // furniture — and `crate::compiler::assembled` clears the region outright, so
            ...
            crate::compiler::plan::ResolvedAnchor::Gate { from, .. } => *from,
        };
        let found = blocks
            .get(&cell)
            .map(String::as_str)
...
mod tests {
    ...
    fn mk(id: &str, cell: [i32; 3], n: usize) -> LootPlan {
        LootPlan {
            ...
            cell,
            items: (0..n)
                .map(|_| crate::compiler::plan::LootItemPlan {
    ...
    fn container_anchors_reads_the_world_and_the_claims() {
        use crate::compiler::plan::ResolvedAnchor;
