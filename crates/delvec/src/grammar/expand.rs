...
//!   a sibling's scope. Here [`crate::grammar::ir::Split::children`] states the match,
...
use crate::grammar::eval::{Env, EvalError, Scope};
use crate::grammar::explain::{self, GuardLeaf, axis_name, render_cond, render_expr};
...
/// [`Program`](crate::grammar::ir::Program), so it reaches the bytes through the
...
pub struct Anchor {
    ...
    /// nearly all of them.
    pub role: Option<crate::grammar::export::AnchorRole>,
...
pub struct ResolvedEdge {
    /// A declared space name, or [`crate::grammar::ir::EXTERIOR`].
    pub a: String,
    /// A declared space name, or [`crate::grammar::ir::EXTERIOR`].
    pub b: String,
    /// The class keyword.
...
/// ([`crate::grammar::orient::FrameSet`]) holds another frame, and that frame would land
...
struct ScopeState<'e> {
    ...
    /// program could be expanded at ([`crate::grammar::orient::FrameSet`]).
    ///
...
struct Expander<'a> {
    ...
    /// Per-stem occurrence counter, so [`crate::grammar::ir::MarkIndex::Auto`] numbers in
    /// expansion order.
