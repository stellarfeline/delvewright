...
//!
//! [`crate::grammar::compose::include_renaming`] composes one `Program` into another and
...
//! ```
//!
//! Everything a composition *means* is [`crate::grammar::compose`]'s, unchanged: the same
...
//! (`crate::grammar::rng`), so a sibling that draws earlier shifts every later draw:
...
use crate::grammar::compose::{self, AnchorRenames, ComposeError};
use crate::grammar::ir::{Include, Program, ProgramError};
...
/// of [`crate::grammar::compose::include_renaming`] calls produces — so everything
