...
//! program's [`Node::Mark`](crate::grammar::ir::Node::Mark) declarations produced, in the
...
//! ([`crate::grammar::gates`]), the palette legality check below runs over the whole
...
pub fn contract_metadata(expansion: &Expansion) -> Option<SpatialContract> {
    ...
    out.faces = crate::grammar::contract::exterior_faces(&expansion.model, &out)
        .into_iter()
        .map(|f| ContractFace {
            space: f.space,
            class: f.class,
...
/// [`crate::grammar::contract::exterior_faces`] reads, so the derivation cannot depend on
/// its own output.
...
mod tests {
    ...
    use crate::grammar::ir::{Material, Node};
