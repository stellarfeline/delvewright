...
//! vocabulary of [`super`], brought in by [`crate::grammar::compose::include`]. The only
...
//! one round later than the zone itself: [`crate::grammar::library::hearth_ward`] landed
...
//! hearth — are built as [`crate::grammar::library::bait_stand`],
//! [`crate::grammar::library::disarm_stand`] and [`crate::grammar::library::hearth_ward`], and the
...
//! [`crate::grammar::library::drop_shaft`] and [`crate::grammar::library::dumbwaiter`] both assert
//! that their landing does **not** reach their hatch under the plain step, and
//! [`crate::grammar::library::boulder_stair`] is flat.
//! [`crate::grammar::library::stair_flight`] is the way up: a walled shaft, a landing at
...
//! [`crate::grammar::library::lift_shaft`], written for this zone and against the shipped
...
//!    [`crate::grammar::compose::include`] still never renames an anchor on its own,
...
//!    [`crate::grammar::compose::include_renaming`] takes an explicit per-anchor rename
...
//!    odd. See [`crate::grammar::library::causeway`].
...
//!    vocabulary, not a new node kind: [`crate::grammar::library::tee_passage`] is a
...
//! ([`crate::grammar::geom::Orientation::is_rotation`]). [`crate::grammar::ir::Reorient::turned`]
...
use crate::grammar::compose::{AnchorRenames, include_renaming};
...
/// same prefix — and it is caught by [`crate::grammar::ir::Program::validate`] in the
