...
//! *along* its own path, deliberately ([`crate::grammar::library::cliff_path()`]); the
...
//! a reflection ([`crate::grammar::geom::Orientation::is_rotation`]). A frame carries a
//! sign per local axis ([`crate::grammar::geom::Mirror`]), so the far leg is literally
//! the near leg under [`crate::grammar::ir::Reorient::turned`]: same rule, same
...
//!
//! [`crate::grammar::library::cliff_path()`] guarantees the ledge is one wide and its
...
//! the prefixes (`crate::grammar::compose` never qualifies an anchor):
...
use crate::grammar::compose::{AnchorRenames, entry};
use crate::grammar::geom::Axis;
use crate::grammar::ir::{ArithOp, AxisSpec, CmpOp, DimRef, Program, Reorient};
use crate::grammar::library::cliff_path;
...
fn qualified(prefix: &str, param: &str) -> String {
    format!("{prefix}{}{param}", crate::grammar::compose::SEPARATOR)
}
