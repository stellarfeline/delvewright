...
//! [`crate::grammar::library::tee_passage`] landed. Every other §5b rule walls both of its
...
//!   ([`crate::grammar::library::dumbwaiter`], §4 entry **L**) — the landing the zone is
//!   named for;
//! * one bay along, a nook off the west side of the lane carries the ward's
//!   rest point ([`crate::grammar::library::hearth_ward`], **BF3**): a detour, not a
//!   room on the route — the lane walks whether or not you take it;
//! * they walk out along the junction's lane
//!   ([`crate::grammar::library::tee_passage`]), which carries one doorway in its side;
//! * through that doorway is the sealed half of a souls shortcut
//!   ([`crate::grammar::library::far_side_bar`], §4 entry **F**, spec-0016 §2): a room
...
//! mechanism, and [`crate::grammar::library::hearth_ward`] is it — somewhere off the road
...
use crate::grammar::compose::entry;
use crate::grammar::geom::Axis;
use crate::grammar::ir::{ArithOp, AxisSpec, CmpOp, DimRef, Expr, Program, Reorient};
