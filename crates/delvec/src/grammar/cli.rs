//! `delvec grammar` — the box-split grammar back end's command line
//! (spec-0027 §3), mounted by the `delvec` binary (ADR-0023 §3).
//!
//! The division of labour the spec is about — **the model authors rules, the
//! deterministic expander does geometry, machine gates judge the result** —
//! needs an entry point a creator can invoke; this is it.
//!
...
//!
//! ```text
//! delvec grammar list                                  # what can be built
//! delvec grammar show --program store-room > scene.json # start from the corpus
//! delvec grammar check --file scene.json                # does the program hold together
//! delvec grammar expand --file scene.json --region 11x6x13 -o out/   # build + judge + freeze
//! delvec grammar coverage                              # what no example demonstrates
//! delvec grammar audit --library --campaign-root ../content  # judge EVERY program
//! ```
//!
...
//! hand-assembled.
//!
//! Exit codes (mirroring `delvec schem` and `delvec render`): `0` ok · `2`
...
use crate::grammar::coverage;
use crate::grammar::document::{self, Loaded};
use crate::grammar::gates;
use crate::grammar::ir::{Paint, Program};
use crate::grammar::{Axis, Box3, ExpandOptions, Overrides, expand, export, library};

const EXIT_INPUT: u8 = 2;
const EXIT_OUTPUT: u8 = 3;
const EXIT_GATE: u8 = 4;

/// `delvec grammar`: the command line, as a type.
#[derive(Clone, Args)]
...
/// The six verbs of `delvec grammar`.
...
pub enum GrammarCommand {
    ...
    Check {
        /// The program: `--program <id>` from the library or `--file <ir.json>`.
        #[command(flatten)]
        source: Source,
    },
    ...
    fn load(&self) -> Result<(String, Loaded), String> {
        match (&self.program, &self.file) {
            (Some(id), None) => match library::by_id(id) {
                ...
                None => Err(format!(
                    "no library program {id:?} — `delvec grammar list` names them all"
                )),
            },
            (None, Some(path)) => {
...
fn run_list() -> ExitCode {
    ...
    println!(
        "\nThe region above is the one the corpus demonstrates the piece at and the one \
         `delvec grammar audit --library` judges it at; a program expands over any region that \
...
/// Run `delvec grammar`.
pub fn run(args: GrammarArgs) -> ExitCode {
    match args.command {
        GrammarCommand::List => run_list(),
        GrammarCommand::Audit {
            library,
            campaign_roots,
            exclusions,
