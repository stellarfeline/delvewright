...
use crate::grammar::contract;
use crate::grammar::expand::Expansion;
use crate::grammar::export;
use crate::grammar::geom::Axis;
use crate::grammar::model::VoxelModel;
use crate::grammar::nav;
...
pub fn judge(expansion: &Expansion, options: Options) -> Report {
    ...
    // (`crate::grammar::orient`), so a literal `facing`/`axis`/connection state inside
...
mod tests {
    ...
    fn the_traversability_gate_separates_a_stair_from_a_one_way_drop() {
        ...
        let stair = crate::grammar::expand(
    ...
    fn the_reachability_measurement_is_deterministic_and_order_independent() {
        ...
        let model = &crate::grammar::expand(
            &two_storey(false),
            Box3::at_origin([9, 13, 9]),
            &ExpandOptions::seeded(0),
        )
