...
///
/// Unset axes are filled in by [`crate::grammar::orient::reorient`].
///
...
impl Reorient {
    ...
    /// ([`crate::grammar::geom::Orientation::is_rotation`]), so a chiral piece stays the
...
/// ([`crate::grammar::geom::Orientation`]), and the resolution reads **both**: a
...
/// `anchor/<kebab>` ids.
///
/// `pub(crate)` because [`crate::grammar::compose`] applies the same test to a rename's
...
///
/// This is [`crate::grammar::compose::include_renaming`] made writable in the artifact of
...
pub struct Include {
    ...
    /// [`crate::grammar::compose::AnchorRenames`](crate::grammar::compose::AnchorRenames).
    ///
...
pub struct Program {
    ...
    /// compiling to the same bytes forever. See [`crate::grammar::version`].
    pub version: String,
    ...
    /// [`crate::grammar::document::load`] is what turns one into the other.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
pub enum ProgramError {
    ...
    /// does not implement (`crates/delvec/src/grammar/version.rs`).
    UnsupportedVersion {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ProgramError::UnsupportedVersion { version } => {
                let accepted: Vec<&str> = crate::grammar::version::accepted_versions().collect();
                match crate::grammar::version::reserved_for(version) {
                    Some(anchor) => write!(
                        f,
...
impl Program {
    ...
    /// [`crate::grammar::compose::include`] instead — this is the *document's* form, and
    ...
    /// claims each.
    ///
    /// Deliberately syntactic, like [`crate::grammar::compose::declared_anchors`]: a
    ...
    /// can be decided without expanding. [`crate::grammar::expand`] runs this first.
