...
//! reading `crates/delvec/src/grammar/*`, and a stated fact with no test is the drift
...
use delvec::grammar::library::{self, idioms};
use delvec::grammar::{Box3, ExpandError, ExpandOptions, Expansion, expand};
...
/// library's programs, and `tests/library.rs` carried a second one
...
fn cases() -> Vec<Case> {
    library::PROGRAMS
        ...
        .map(|p| Case {
            ...
            // The rose window IS the mirror plane, and the registry is where
            // that claim lives, so the gate reads it here AND in `audit` —
            // rather than only in a table this file keeps by hand.
            symmetric: p.gates.symmetric,
        })
        .collect()
}
...
fn arguments_states_one_recursion_and_calls_it_four_ways() {
    let case = &case_by_id("idiom-arguments");
...
fn fact_a_world_cardinal_state_does_not_turn_with_the_scope() {
    ...
    let turned = expand(
        ...
        &ExpandOptions {
            orientation: delvec::grammar::geom::Orientation::from_axes([Axis::Z, Axis::Y, Axis::X]),
            ..ExpandOptions::seeded(1)
        },
    )
    .unwrap();
