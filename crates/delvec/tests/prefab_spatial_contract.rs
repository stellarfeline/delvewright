...
//!
//! `delvec grammar expand` judges a contract against the blocks it just wrote.
//! `delvec prefab audit` judges the same contract against the same blocks read
...
/// **The green.** The piece `delvec grammar expand` passed, admitted through the
...
/// **The green, through the way spelling.** The piece `delvec grammar expand`
...
/// process, and the grid `delvec prefab audit` rebuilds from an `.nbt` it read off
