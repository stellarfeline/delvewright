...
use delvec::grammar::geom::{Axis, Box3};
use delvec::grammar::ir::{Contract, EXTERIOR, EdgeClass, Envelope, Expr, Node, Program, Reorient};
use delvec::grammar::{ExpandOptions, Expansion, expand};
...
fn arm() -> Node {
    Node::Split(delvec::grammar::ir::Split {
        axis: Axis::X,
        sizes: vec![
            delvec::grammar::ir::Size::Absolute {
                blocks: Expr::param("band"),
            },
            delvec::grammar::ir::Size::Relative {
                weight: Expr::int(1),
            },
        ],
...
fn a_binding_reads_the_same_extents_under_a_reflection_as_without_one() {
    fn one_arm(reflect: bool) -> Program {
        let body = Node::call("arm").with_params([(
            "band",
            Expr::dim(delvec::grammar::ir::DimRef::X)
                .arith(delvec::grammar::ir::ArithOp::Div, Expr::int(3)),
        )]);
        let body = if reflect {
            Node::Reorient {
                orient: Reorient::KEEP.flip(Axis::X),
...
fn every_fence_this_document_crosses_refuses_it_at_the_version_below() {
    use delvec::grammar::ir::ProgramError;
    use delvec::grammar::version::{BIND_SINCE, CONTRACT_SINCE, MIRROR_SINCE};
