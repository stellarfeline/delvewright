...
use delvewright_dsl::{Campaign, DSL_VERSION, Diagnostic, RawCampaign, parse_campaign};
...
fn quests_doc(on_complete: &str) -> String {
    quests_doc_with(on_complete, "")
}

/// As [`quests_doc`], plus a raw `triggers` array body (no surrounding brackets).
fn quests_doc_with(on_complete: &str, triggers: &str) -> String {
    format!(
        r#"{{
...
/// `cp_on_respawn_<i>`).
static DIALOGUE_SEALS_ON_RESPAWN: LazyLock<String> = LazyLock::new(|| {
    common::at_dsl_version(
        r#"{
  "dsl_version": "%dsl_version%",
  "campaign_id": "hello-world",
...
fn a_sealed_gate_answers_a_right_click() {
    let c = parse_hw(&quests_doc(SEAL_IT));
    let prefabs = PrefabRegistry::load_dir(&common::prefabs_dir()).unwrap();
...
fn an_unanswered_seal_is_dw0429() {
    let c = parse_hw(&quests_doc(SEAL_IT_SILENT));
...
fn a_dialogue_nested_seal_is_armed() {
    let quests = quests_doc(r#"{ "type": "campaign-complete" }"#);
    let c = parse_hw_with_dialogue(&quests, DIALOGUE_SEALS_ON_RESPAWN.as_str());
    let prefabs = PrefabRegistry::load_dir(&common::prefabs_dir()).unwrap();
    let out = build(&c, &prefabs);
...
fn no_seal_no_machinery() {
    let c = parse_hw(&quests_doc(r#"{ "type": "campaign-complete" }"#));
    let prefabs = PrefabRegistry::load_dir(&common::prefabs_dir()).unwrap();
    let out = build(&c, &prefabs);
    assert!(
