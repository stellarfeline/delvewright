...
fn a_sealed_gate_answers_a_right_click() {
    let c = parse_hw(&quests_doc("0.23.0", SEAL_IT));
    let prefabs = PrefabRegistry::load_dir(&common::prefabs_dir()).unwrap();
...
fn an_unanswered_seal_is_dw0429() {
    let c = parse_hw(&quests_doc("0.23.0", SEAL_IT_SILENT));
...
fn a_dialogue_nested_seal_is_armed() {
    let quests = quests_doc("0.23.0", r#"{ "type": "campaign-complete" }"#);
...
fn no_seal_no_machinery() {
    let c = parse_hw(&quests_doc("0.23.0", r#"{ "type": "campaign-complete" }"#));
    let prefabs = PrefabRegistry::load_dir(&common::prefabs_dir()).unwrap();
    let out = build(&c, &prefabs);
    assert!(
