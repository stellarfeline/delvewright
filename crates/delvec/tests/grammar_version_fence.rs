...
use delvec::grammar::geom::{Axis, Mirror};
use delvec::grammar::ir::{Alternative, Cond, Node, Program, ProgramError, Reorient};
...
fn the_fence_is_on_the_reflection_and_not_on_the_frame() {
    let plain = with_body(Node::Reorient {
        orient: Reorient::default().x(delvec::grammar::ir::AxisSpec::WorldZ),
        body: Box::new(Node::fill("stone")),
    })
    .at_version("1.0.0");
