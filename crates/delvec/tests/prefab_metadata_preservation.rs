...
//! by `delvec grammar expand` and removed by `delvec prefab lighting --write`, the
...
//! The fixture is therefore a real `delvec grammar` export — a fixture written by
...
/// Every `delvec prefab` subcommand, and whether it rewrites a prefab's metadata.
///
...
fn run(args: &[&str]) {
    let out = prefab().args(args).output().unwrap();
    assert!(
        out.status.success(),
        "delvec prefab {args:?} failed: {}",
...
fn assert_only_touches(
    ...
    for (path, value) in &b {
        ...
        assert_eq!(
            a.get(path),
            Some(value),
            "`delvec prefab {command}` changed or dropped `{}`, which it does not write. It \
             declares {owned:?}",
            path.join("/")
        );
    }
    for path in a.keys() {
        assert!(
            ...
            "`delvec prefab {command}` invented `{}`, which it does not write",
            path.join("/")
        );
    }
}
...
fn every_subcommand_is_classified_as_writing_metadata_or_not() {
    let help = prefab().arg("--help").output().unwrap();
    ...
    for c in &commands {
        assert!(
            ...
            "`delvec prefab {c}` is not classified: if it rewrites prefab metadata add it to \
    ...
    for c in WRITES_METADATA.iter().chain(DOES_NOT_WRITE_METADATA) {
        assert!(
            ...
            "`{c}` is classified but `delvec prefab` no longer has it"
        );
    }
}
...
fn every_step_classified_as_not_writing_metadata_leaves_it_byte_identical() {
    ...
    for command in DOES_NOT_WRITE_METADATA {
        ...
        let _ = prefab().args(&argv).output().unwrap();

        assert_eq!(
            std::fs::read(&json).unwrap(),
            before,
            "`delvec prefab {command}` is classified as not writing prefab metadata and wrote it"
        );
        checked += 1;
    }
    assert!(
