...
//! [`PrefabCommand`]'s clap tree, every discovered command is classified exactly once, and a
...
fn prefab() -> Proc {
    let mut cmd = Proc::new(env!("CARGO_BIN_EXE_delvec"));
    cmd.arg("prefab");
    cmd
}

fn tmp(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("delvec prefab-doors-{}-{name}", std::process::id()));
...
fn a_door_that_takes_a_manifest_refuses_one_that_does_not_tile_its_zone() {
    ...
    for door in PIECE_DOORS {
        ...
        assert_ne!(
            out.status.code(),
            Some(0),
            "`delvec prefab {}` accepted a manifest covering half the zone it declares. It has \
        ...
        if takes_a_manifest.iter().any(|d| d.path == door.path) {
            assert!(
                text.contains("cover"),
                "`delvec prefab {}` reads zone manifests, so its refusal must be that this one does \
                 not tile its zone:\n{text}",
                door.path.join(" ")
            );
            bound += 1;
