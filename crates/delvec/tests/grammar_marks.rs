...
use delvec::grammar::expand::Anchor;
use delvec::grammar::ir::{AxisSpec, Node, Reorient};
use delvec::grammar::library::castle;
use delvec::grammar::{
...
fn offsets_are_read_through_the_scopes_orientation() {
    ...
    program.rules.insert(
        "root".to_string(),
        vec![delvec::grammar::ir::Alternative::new(Node::Reorient {
...
/// The same, once axes are **reversed**: every position stated in the rule's own
/// frame is measured from the far end instead, and every one is measured from
/// the *same* far end — the whole point of routing them through one
/// `Orientation::offset`.
///
/// `floor_center` is the deliberate exception and is checked here beside the
/// others rather than left to a doc line: gravity is a world fact, so a floor
/// centre does not move when a piece is turned round. Nothing else asserts that
/// special case, so folding it back into the general path would otherwise be a
/// silent change.
#[test]
