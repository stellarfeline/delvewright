...
fn version_line() {
    ...
    // byte-identically.
    assert!(s.contains("dsl 0.22.1"), "{s}");
...
fn move_unroutable_exits_3_with_dw0307() {
    ...
    common::patch_file(&camp.join("quests.json"), |d| {
        d["dsl_version"] = serde_json::json!("0.22.1");
...
fn ocean_areas_sit_on_the_sea_level_datum_void_unchanged() {
    ...
    let place_line = |horizon: Option<&str>, name: &str| -> String {
        ...
        if let Some(h) = horizon {
            world["dsl_version"] = serde_json::json!("0.22.1");
...
fn beach_camp_campaign(name: &str, ocean: bool) -> std::path::PathBuf {
    ...
    let edit = |file: &str, f: &dyn Fn(&mut serde_json::Value)| {
        ...
        f(&mut doc);
        if ocean {
            doc["dsl_version"] = serde_json::json!("0.22.1");
        }
...
fn dw0330_warning_reports_but_does_not_fail_the_build() {
    ...
    quests["dsl_version"] = serde_json::json!("0.22.1");
...
// ---------------------------------------------------------------------------
// `delvec build --perturb` — the observer, shown its own defect
// ---------------------------------------------------------------------------
//
// The stage-5 blockout battery claims to OBSERVE the built bytes rather than
// replay the arithmetic that laid them (`compiler.md`, "why the battery is an
// observer and not a replay"). Until this flag existed that claim could only be
// read off a `cargo test` transcript: `blockout::Perturb` is reachable from Rust
// and from nowhere else, and a doctored PLAN cannot stand in for a doctored
// derivation, because the observer compares the plan against bytes derived from
// that same plan — a plan edit reds at stage 4 before a block is laid.
//
// The subject is the METRICS GYM, generated by the engine under test from its
// own metrics table. It needs no prefab content, no content checkout and no
// pin — the build is handed an empty prefab library, exactly as
// `tools/determinism-subject.sh` does — so what these tests measure cannot
// depend on a second repository.

/// The gym, and the empty prefab library it is built against — generated once
/// for the whole test binary, because it is a pure function of the engine.
fn gym() -> &'static (std::path::PathBuf, std::path::PathBuf) {
    static GYM: std::sync::OnceLock<(std::path::PathBuf, std::path::PathBuf)> =
        std::sync::OnceLock::new();
    GYM.get_or_init(|| {
        let dir = tmp("perturb-gym");
        let prefabs = tmp("perturb-gym-prefabs");
        let out = delvec(&["metrics", "--gym", dir.to_str().unwrap()]);
        assert_eq!(
            code(&out),
            0,
            "the gym must generate: {}",
            String::from_utf8_lossy(&out.stderr)
        );
        assert!(dir.join("site-plan.json").is_file(), "the gym has a plan");
        (dir, prefabs)
    })
}

/// A digest of every file under `dir`, so a run that wrote something beside its
/// input cannot pass for one that wrote nothing.
fn tree_fingerprint(dir: &Path) -> BTreeMap<String, u64> {
    fn walk(dir: &Path, base: &Path, out: &mut BTreeMap<String, u64>) {
        for e in std::fs::read_dir(dir).expect("a directory under test is readable") {
            let p = e.expect("a directory entry").path();
            if p.is_dir() {
                walk(&p, base, out);
            } else {
                let bytes = std::fs::read(&p).expect("a file under test is readable");
...
/// Run one perturbed build over the gym and return what it said.
///
/// The invariants that hold for EVERY knob are asserted here rather than in each
/// test, so a knob added later gets them without anybody remembering: the run
/// exits at the build tier, it names the knob it was asked for, it says it wrote
/// no tree, and the campaign directory it read is byte-identical afterwards.
fn perturbed(knob: &str, place: Option<&str>) -> String {
    let (dir, prefabs) = gym();
    ...
    assert_eq!(
        code(&out),
        3,
        "a perturbed build exits at the build tier whatever the observer says: {err}"
    );
    assert!(
        err.contains(&format!("`{knob}`")),
        "the run must name the defect it asked for: {err}"
    );
    assert!(
        err.contains("NO output tree"),
        "the run must say it writes nothing: {err}"
    );
    assert_eq!(
        tree_fingerprint(dir),
        before,
        "a perturbed build wrote into the campaign directory it was reading"
    );
    err
}

/// **The structural guarantee, at the argument level**: a perturbed build has
/// nowhere to write.
///
/// `--out` and `--perturb` are declared as conflicting arguments, so this is not
/// a decision the program makes after deriving — the invocation carrying both is
/// refused by the parser and no code runs at all. That is what makes a perturbed
/// tree unadmittable rather than merely unadmitted: `tools/staging-gate.py`
/// fingerprints a build by hashing its `manifest.json`, and a tree that was
/// never written has no manifest and therefore no identity to bind a token to.
#[test]
fn a_perturbed_build_is_refused_an_output_directory() {
    let (dir, prefabs) = gym();
    let out_dir = tmp("perturb-forbidden-out");
    // `tmp` creates the directory; the point is that nothing is written INTO it.
    let r = delvec(&[
        "build",
        dir.to_str().unwrap(),
        "--prefabs",
    ...
    assert_eq!(code(&r), 2, "clap refuses the combination: {err}");
    assert!(
        err.contains("--out") && err.contains("--perturb"),
        "the refusal must name both arguments: {err}"
    );
    assert!(
        tree_fingerprint(&out_dir).is_empty(),
        "the refused invocation wrote an output tree"
    );
    assert!(!out_dir.join("manifest.json").exists());
}

/// The knobs whose defect this campaign's own shape gives the observer nothing
...
fn a_defect_this_campaign_has_nothing_for_is_still_refused() {
    for (knob, place, why) in [
        ...
        let err = perturbed(knob, place);
        assert!(
            err.contains("NOTHING refused"),
            "`{knob}` ({why}) must report that no observer fired: {err}"
        );
    }
}

/// `DW0836`: every hole cut one cell over.
#[test]
fn perturb_slide_openings_reddens_dw0836() {
    let err = perturbed("slide-openings", None);
    assert!(err.contains("DW0836"), "{err}");
    assert!(
        err.contains("the built world does not have the opening the plan allocated"),
...
fn perturb_sink_reddens_dw0836_on_the_realized_rise() {
    ...
    assert!(
        ...
        "the realized rise must be what disagreed: {err}"
    );
}
...
/// `DW0838`: walls a course tall, and both observers that see it.
///
/// The build STOPS at its first refusal — `emit::BuildFailure` carries one code
/// and one message — and for this defect that is `DW0836`, because walls a
/// course tall also leave every wall open above its allocation. `DW0838` is the
/// code the knob is documented to redden and the one row 60 of
/// `docs/demo-levels.md` promises a reader will see naming the wall course, so
/// the battery states its whole refusal set and prints the messages after the
/// first. Asserting both is what keeps that from silently regressing to one.
#[test]
fn perturb_short_walls_reddens_dw0838_beside_dw0836() {
    let err = perturbed("short-walls", None);
    assert!(
        err.contains("DW0836") && err.contains("DW0838"),
        "one defect, two observers: {err}"
    );
    assert!(
...
/// The arity and the place are refused before anything derives, and each refusal
/// says what the flag would have decided.
#[test]
fn the_perturbation_flag_refuses_what_it_cannot_attribute() {
    let (dir, prefabs) = gym();
    let base = |extra: &[&str]| -> Output {
        let mut args = vec![
            "build",
            dir.to_str().unwrap(),
            "--prefabs",
    ...
    // Two knobs at once: the code that fires would say nothing about which
    // defect it saw.
    let r = base(&["--perturb", "short-walls", "--perturb", "brick-up"]);
    assert_eq!(code(&r), 1);
    assert!(
        String::from_utf8_lossy(&r.stderr).contains("exactly one knob"),
        "{}",
        String::from_utf8_lossy(&r.stderr)
    );

    // A place-taking knob with no place would derive a clean map and report an
    // observer that failed to observe — true about the wrong thing.
    let r = base(&["--perturb", "brick-up"]);
    assert_eq!(code(&r), 1);
    assert!(
        String::from_utf8_lossy(&r.stderr).contains("--perturb-place"),
        "{}",
        String::from_utf8_lossy(&r.stderr)
    );

    // A place on a knob that damages the whole derivation decides nothing.
    let r = base(&["--perturb", "short-walls", "--perturb-place", "node/pit"]);
    assert_eq!(code(&r), 1);
    assert!(
        String::from_utf8_lossy(&r.stderr).contains("not one place"),
        "{}",
        String::from_utf8_lossy(&r.stderr)
    );

    // A place no box declares, checked against the site plan's own boxes.
    let r = base(&["--perturb", "brick-up", "--perturb-place", "node/nowhere"]);
    assert_eq!(code(&r), 1);
    let err = String::from_utf8_lossy(&r.stderr);
    ...
    // An unknown knob is refused by the parser, which lists the value set it
    // takes from `Knob::ALL` — so a seventh defect is offered here for free.
    let r = base(&["--perturb", "no-such-defect"]);
    assert_eq!(code(&r), 2);
    let err = String::from_utf8_lossy(&r.stderr);
    for knob in [
        "slide-openings",
...
/// An ordinary build is untouched: no `--perturb`, an output tree, exit 0.
///
/// The pair matters more than either half — the demonstration is only worth
/// anything beside a run of the same campaign that passes, and row 60 of
/// `docs/demo-levels.md` shows exactly that pairing.
#[test]
fn the_unperturbed_gym_builds_and_writes_its_tree() {
    let (dir, prefabs) = gym();
    let out_dir = tmp("perturb-clean-out");
    let r = delvec(&[
        "build",
        dir.to_str().unwrap(),
        "--prefabs",
    ...
    assert_eq!(
        code(&r),
        0,
        "the gym builds clean: {}",
        String::from_utf8_lossy(&r.stderr)
    );
    assert!(
        out_dir.join("manifest.json").is_file(),
        "the tree a staging admission token binds to"
    );
    let err = String::from_utf8_lossy(&r.stderr);
    assert!(
        err.contains("blockout battery binding:"),
        "the observer states what it examined even when it finds nothing: {err}"
    );
    assert!(
        !err.contains("perturbed build"),
