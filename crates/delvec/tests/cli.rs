...
fn version_line() {
    ...
    // byte-identically.
    assert!(s.contains(&format!("dsl {DSL_VERSION}")), "{s}");
...
fn v06_ocean_boundary_builds_byte_identical_and_wires_return() {
    ...
    let world = common::at_dsl_version(
        r#"{
...
fn a_missing_skin_png_is_dw0309() {
    ...
    .unwrap();
    npcs["dsl_version"] = DSL_VERSION.into();
...
// `tools/ci/determinism-subject.sh` does — so what these tests measure cannot
...
/// tree unadmittable rather than merely unadmitted: `tools/creator/staging-gate.py`
