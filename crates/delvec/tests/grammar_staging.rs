...
use delvec::grammar::geom::{Axis, Mirror, Orientation};
use delvec::grammar::ir::{AxisSpec, Node, Program, Split};
use delvec::grammar::library::bait_stand::{self, bait_stand};
use delvec::grammar::library::causeway::MIN_GATE_RISE;
use delvec::grammar::library::disarm_stand::{self, disarm_stand};
use delvec::grammar::library::elite_ground::MIN_RADIUS;
use delvec::grammar::library::hearth_ward::{self, hearth_ward};
use delvec::grammar::library::lift_shaft::{self, lift_shaft};
use delvec::grammar::library::rafter_hall::FLOOR_CELLS_PER_PERCH;
use delvec::grammar::library::stair_flight::{self, stair_flight};
use delvec::grammar::library::tee_passage::{self, tee_passage};
...
use delvec::grammar::{Anchor, Box3, ExpandOptions, Expansion, VoxelModel, expand};
// W3: the palette/prop family (W + S + M + X).
use delvec::grammar::library::boulder_stair::{self, boulder_stair};
use delvec::grammar::library::broken_grate::{self, broken_grate};
use delvec::grammar::library::threshold_motif::{self, threshold_motif};
...
// (`docs/reference/grammar.md` §7; `crates/delvec/src/grammar/mod.rs`'s own "not
...
fn the_palette_mirror_still_fires_on_a_genuine_accent_in_the_tread() {
    ...
    restyled
        .set_role(
            "smooth",
            delvec::grammar::ir::Paint::block(BlockState::simple("gold_block")),
        )
        .unwrap();
    ...
    for (base, region) in [
        ...
        for (i, role) in roles.iter().enumerate() {
            restyled
                .set_role(
                    role,
                    delvec::grammar::ir::Paint::block(BlockState::simple(SWATCH[i % SWATCH.len()])),
                )
                .unwrap();
        }
