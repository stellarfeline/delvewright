...
use delvec::grammar::compose::{AnchorRenames, entry, include_renaming};
use delvec::grammar::geom::Axis;
use delvec::grammar::ir::{Alternative, Node, Paint, Program, Size, Split};
use delvec::grammar::library::bell::bell_tower::{CLIMB, RISE, SILL};
use delvec::grammar::library::bell::cliff_road::{MIN_DROP, MIN_GULF};
...
use delvec::grammar::{Box3, ExpandOptions, Expansion, VoxelModel, expand};
...
const CLIFF_REGION: Box3 = Box3::at_origin([11, 13, 40]);
...
/// also the first thing that would break if [`delvec::grammar::compose`]
...
/// [`delvec::grammar::library::bell::barrow_shore`] says so plainly rather
/// than dressing it up.
///
...
fn a_piece_run_shorter_than_the_zone_is_refused_not_turned() {
    ...
    let turned = expand_at(&delvec::grammar::library::ambush_door(), wide, KEEP_SEED);
    assert_eq!(
    ...
    let turned_motif = expand_at(
        &delvec::grammar::library::threshold_motif(),
        flat_box,
        TOWER_SEED,
    );
    assert_eq!(
...
fn branch_chain() -> Program {
    ...
    let tee = delvec::grammar::library::tee_passage();
...
fn a_zone_can_compose_a_route_a_player_walks_up() {
    ...
    let approach = delvec::grammar::library::tee_passage();
