...
fn run_expand(
    ...
    // Also an input-only property: a region no structure template can hold is
    // knowable from the three numbers, so it is refused here rather than after
    // expanding it.
    if size.iter().any(|&s| s > export::MAX_STRUCTURE_AXIS) {
        return bad_input(format!(
            "{}\n  nothing was expanded and nothing was written.",
            export::ExportError::TooLarge {
                size,
                cap: export::MAX_STRUCTURE_AXIS
            }
        ));
    }
