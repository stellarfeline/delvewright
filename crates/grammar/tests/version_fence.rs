...
use delvewright_grammar::version::{
    LATEST_PROGRAM_VERSION, MIRROR_SINCE, SUPPORTED_PROGRAM_VERSIONS, has_mirror,
};
...
fn an_unknown_version_is_refused_rather_than_parsed_best_effort() {
    for unknown in ["0.9.0", "1.2.0", "2.0.0", "", "latest"] {
...
/// accepts and validates under it; and every program that actually writes a
/// reflection is refused when its version is lowered below the fence.
///
/// Both counts are printed and both are asserted non-zero. A fence proved only
/// on a hand-written fixture is a fence that binds to one file: this is what
/// says the corpus is behind it.
#[test]
fn the_fence_binds_to_the_corpus_and_not_only_to_a_fixture() {
    let mut declared = 0usize;
    let mut reflecting = 0usize;
    for (id, build) in library::PROGRAMS {
        ...
        match lowered.validate() {
            Err(ProgramError::FencedConstruct { since, .. }) => {
                assert_eq!(since, MIRROR_SINCE, "{id}");
                reflecting += 1;
            }
            Ok(()) => {}
    ...
    println!(
        "version fence   bound {declared:3}  library program(s) at {LATEST_PROGRAM_VERSION}, \
         {reflecting} of them refused at 1.0.0"
    );
    assert!(declared > 0, "the corpus sweep examined zero programs");
    assert!(
        reflecting > 0,
        "ZERO library programs write a reflection, so lowering the version refuses nothing \
         and this sweep is a green that binds to nothing"
    );
}
