//! `delve-render` — the **GPU arms** of the Delvewright render layer
//! (spec-0007 / spec-0003, M3).
//!
...
//! CPU arms are `delvec` subcommands (ADR-0021 §1): `delvec viewer`,
//! `delvec scene`, `delvec panorama`, `delvec contact-sheet`, `delvec palette`,
//! `delvec index`.
//!
//! This binary is **built from a checkout, never shipped on the shelf**
//! (ADR-0021 §3), and that is a distribution statement rather than a capability
//! one: the shelf's Linux targets are static-musl, which cannot `dlopen` a
//! Vulkan loader, and `nucleation` carries a C build script no musl
//! cross-compiler in the release recipe can run. Every validation the pipeline
//! needs stays runnable on the creator's own machine because the source build is
//! the guarantee — the skill's `Init` section builds this crate at the step that
//! needs it.
//!
...
#[command(
    name = "delve-render",
    about = "Delvewright render layer, GPU arms: per-prefab shot sets and the fidelity gate",
